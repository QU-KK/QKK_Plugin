...
class PunchIt(bpy.types.Operator):
    ...
    def get_selection(self, context, bm, debug=False):
        ...
        if self.is_mesh_non_manifold and not get_prefs().non_manifold_extrude:
            bpy.ops.machin3.draw_punchit_labels(text='Mesh is non-manifold!', text2='Try enabling non-manifold mesh support in the addon preferences', coords=coords, color=red, color2=yellow, color3=white, alpha=1, alpha2=0.8, time=3)
            return
        ...
        if not faces and (tuple(bpy.context.scene.tool_settings.mesh_select_mode) == (True, False, False) or tuple(bpy.context.scene.tool_settings.mesh_select_mode) == (False, True, False)):
            ...
            if verts:
                ...
                if len(sequences):
                    ...
                    bpy.ops.machin3.draw_punchit_labels(text='Illegal Selection!', text2='You need to select at least one edge', coords=coords, color=red, color2=yellow, alpha=1, alpha2=0.5, time=3)
                    return

            else:
                bpy.ops.machin3.draw_punchit_labels(text='Illegal Selection!', text2='Make a Selection, will ya...', coords=coords, color=red, color2=yellow, alpha=1, alpha2=0.5, time=3)
                return

        islands = get_selection_islands(faces, debug=debug)

        if not islands:
            bpy.ops.machin3.draw_punchit_labels(text='Pay attention, numbnuts!', text2='Uh, try to select a face, maybe. Or an edge at least.', coords=coords, color=red, color2=yellow, alpha=1, alpha2=0.5, time=3)
            return

        faces = max(islands, key=lambda x: len(x[2]))[2]

        if set(faces) == {f for f in bm.faces}:
            bpy.ops.machin3.draw_punchit_labels(text='Illegal Selection!', text2="Let's think this through. You want to select all the faces? At the same time?", coords=coords, color=red, color2=yellow, alpha=1, alpha2=0.5, time=3)
            return
        ...
        if has_non_manifold_verts:
            bpy.ops.machin3.draw_punchit_labels(text='Selection is next to non-manifold edges!', text2='Fix your shit 💩', text3='No soup for you', coords=coords, color=red, color2=yellow, color3=white, alpha=1, alpha2=0.5, alpha3=0.5, time=3)
            return
        ...
        if len(sequences) > 1:
            bpy.ops.machin3.draw_punchit_labels(text='Illegal Selection!', text2="Face Selection can't be cyclic 🚴 ", coords=coords, color=red, color2=yellow, alpha=1, alpha2=0.5, time=3)
            return
