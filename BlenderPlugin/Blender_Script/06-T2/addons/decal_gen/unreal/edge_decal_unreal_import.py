...
def _package_path(value):
    ...
    # Accept Unreal's full Copy Reference form, for example:
    # Material'/Game/Materials/M_Master.M_Master'
...
def _load_asset(path, label, required=True):
    ...
    if required and asset is None:
        raise RuntimeError(f'{label} was not found at "{path}".')
    return asset
...
def _create_edge_decal_master(path):
    """Create the bundled NormalTexture + Opacity deferred-decal template."""
    package_path = _package_path(path)
    if not package_path.startswith("/Game/"):
        raise RuntimeError(
            "An automatically created decal master must be inside /Game. "
            f'Configured path: "{path}".'
        )
    destination_path, asset_name = package_path.rsplit("/", 1)
    if not asset_name:
        raise RuntimeError(f'Invalid decal master material path: "{path}".')
    ...
    if material is None:
        raise RuntimeError(f'Could not create decal master material at "{package_path}".')
    ...
    default_normal = _load_asset(
        "/Engine/EngineMaterials/BaseFlattenNormalMap.BaseFlattenNormalMap",
    ...
    if not unreal.MaterialEditingLibrary.connect_material_property(
        ...
        raise RuntimeError(
            "Could not connect NormalTexture to the master material Normal input."
        )
    ...
    default_opacity = _load_asset(
        "/Engine/MaterialTemplates/LayerBlends/T_DefaultMaskTexture.T_DefaultMaskTexture",
        "Default opacity mask texture",
        required=False,
    )
    ...
    if not unreal.MaterialEditingLibrary.connect_material_property(
        ...
        raise RuntimeError("Could not connect Opacity to the master material Opacity input.")

    compile_errors = list(unreal.MaterialEditingLibrary.recompile_material(material) or [])
    if compile_errors:
        raise RuntimeError(
            "The automatically created decal master failed to compile: "
            + "; ".join(str(error) for error in compile_errors)
        )
    material.modify()
    unreal.EditorAssetLibrary.save_loaded_asset(material, only_if_is_dirty=False)
    _log(
        f"Created bundled Deferred Decal / Translucent master material at "
...
def _load_or_create_master(path, create_if_missing):
    ...
    if not create_if_missing:
        raise RuntimeError(
            f'Decal master material was not found at "{path}". '
            "Enable Create Master if Missing or provide an existing Unreal material path."
        )
...
def _import_texture(
    ...
    existing = _load_asset(
        texture_package,
        f'Texture "{asset_name}"',
        required=False,
    )
    if (
    ...
    texture = imported[0] if imported else _load_asset(
        texture_package,
        f'Texture "{asset_name}"',
    )
...
def _material_instance(record, master_material, material_path, texture_path):
    ...
    instance_signature = (
        hashlib.sha256(
            f"{record_signature}|{master_path}".encode("utf-8")
        ).hexdigest()
        if record_signature
        else ""
    )
    if (
        ...
        and _metadata_value(instance, "EdgeDecalAssetSignature")
        == instance_signature
    ):
        return instance, []
    ...
    if instance is None:
        ...
        instance = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            instance_package.rsplit("/", 1)[-1],
            instance_package.rsplit("/", 1)[0],
            unreal.MaterialInstanceConstant,
            factory,
        )
    if instance is None:
        raise RuntimeError(f"Could not create material instance {instance_package}.")
    ...
    for texture_record in record.get("textures", []):
        texture_name = _safe_name(
            os.path.splitext(os.path.basename(texture_record["file"]))[0],
            "T_EdgeDecal",
        )
        texture = _import_texture(
            ...
            texture_name,
            texture_path,
            texture_record.get("role", ""),
            str(texture_record.get("content_hash") or ""),
        )
        ...
        if parameter:
            ...
            if actual is None or actual.get_path_name() != texture.get_path_name():
                _warn(
                    f'Master material has no texture parameter named "{parameter}" '
                    f'for {record["instance_name"]}.'
                )
    unreal.MaterialEditingLibrary.update_material_instance(instance)
    instance.modify()
...
def _import_static_mesh(record, destination_path):
    ...
    import_ui.set_editor_properties(
        {
    ...
    static_data.set_editor_properties(
        {
            "auto_generate_collision": record.get("role") != "decal",
            "build_nanite": bool(record.get("nanite")),
            "combine_meshes": bool(record.get("combine_meshes", True)),
            "convert_scene": True,
            "convert_scene_unit": True,
            "generate_lightmap_u_vs": record.get("role") != "decal",
    ...
    task.set_editor_properties(
        {
            "filename": os.path.realpath(record["file"]),
            "destination_path": destination_path,
            "destination_name": asset_name,
            "automated": True,
            "replace_existing": True,
    ...
    mesh = imported[0] if imported else _load_asset(
        f"{destination_path}/{asset_name}",
        f'Static Mesh "{asset_name}"',
    )
    if not isinstance(mesh, unreal.StaticMesh):
        raise RuntimeError(f'Imported asset "{asset_name}" is not a Static Mesh.')
    return mesh
...
def _assign_static_mesh_materials(mesh, role, source_slot, blank_material, decal_materials):
    ...
    if assigned < expected_minimum:
        ...
        raise RuntimeError(
            f'Could not map imported material slots for {mesh.get_name()}. Slots: {names or "none"}.'
        )
    mesh.modify()
...
def _find_blueprint_handle(subsystem, blueprint, variable_name):
    ...
    for handle in subsystem.k2_gather_subobject_data_for_blueprint(blueprint):
        ...
        normalized = {
            name.removesuffix("_GEN_VARIABLE")
            for name in names
        }
...
def _blueprint_parent_handle(subsystem, blueprint):
    ...
    for handle in handles:
        ...
        if library.is_default_scene_root(data):
            return handle
    if not handles:
        raise RuntimeError("The Blueprint has no subobject root.")
    return handles[0]
...
def _ensure_blueprint_mesh_component(subsystem, blueprint, variable_name):
    ...
    if handle is None:
        params = unreal.AddNewSubobjectParams(
            parent_handle=_blueprint_parent_handle(subsystem, blueprint),
            new_class=unreal.StaticMeshComponent,
            blueprint_context=blueprint,
        )
        handle, fail_reason = subsystem.add_new_subobject(params=params)
        if str(fail_reason):
            raise RuntimeError(f"Could not add {variable_name} component: {fail_reason}")
        if not subsystem.rename_subobject(handle, unreal.Text(variable_name)):
            raise RuntimeError(f"Could not rename Blueprint component to {variable_name}.")
    component = _blueprint_component_object(subsystem, handle)
    if component is None:
        raise RuntimeError(f"Could not resolve Blueprint component {variable_name}.")
    return component
...
def _create_or_update_blueprint(blueprint_name, destination_path, source_mesh, decal_mesh):
    ...
    if blueprint is None:
        ...
        blueprint = unreal.AssetToolsHelpers.get_asset_tools().create_asset(
            blueprint_package.rsplit("/", 1)[-1],
            blueprint_package.rsplit("/", 1)[0],
            unreal.Blueprint,
            factory,
        )
    if blueprint is None:
        raise RuntimeError(f"Could not create Blueprint {blueprint_package}.")

    subsystem = unreal.get_engine_subsystem(unreal.SubobjectDataSubsystem)
    source_component = _ensure_blueprint_mesh_component(
        subsystem,
        blueprint,
        "SourceMesh",
    )
    ...
    decal_component = _ensure_blueprint_mesh_component(
        subsystem,
        blueprint,
        "DecalMesh",
    )
...
def import_edge_decal_manifest(manifest_path):
    ...
    if not os.path.isfile(manifest_path):
        raise RuntimeError(f"Edge Decal manifest does not exist: {manifest_path}")
    with open(manifest_path, "r", encoding="utf-8") as handle:
        manifest = json.load(handle)
    if int(manifest.get("schema_version", 0)) != 1:
        raise RuntimeError("Unsupported Edge Decal Unreal manifest version.")
    ...
    # Materials and textures are shared by every source below destination_root.
    # Per-source meshes and Blueprints remain isolated below asset_root.
    ...
    master_material, master_created = _load_or_create_master(
        manifest.get("master_material") or "/Game/EdgeDecals/M_EdgeDecal_Master",
        bool(manifest.get("create_master_if_missing", True)),
    )
    blank_material = _load_asset(
        manifest.get("blank_material")
        or "/Engine/EngineMaterials/DefaultMaterial.DefaultMaterial",
        "Source material",
    )
    ...
    for material_record in manifest.get("materials", []):
        instance, textures = _material_instance(
            material_record,
            master_material,
            material_path,
            texture_path,
    ...
    for mesh_record in manifest.get("meshes", []):
        ...
        _assign_static_mesh_materials(
            mesh,
            role,
            str(manifest.get("source_slot") or "EDG_SOURCE"),
            blank_material,
            decal_materials,
        )
    ...
    if manifest.get("workflow") == "nanite_blueprint":
        ...
        if source_mesh is None or decal_mesh is None:
            raise RuntimeError("Nanite workflow requires both source and decal meshes.")
        result_asset = _create_or_update_blueprint(
            manifest.get("blueprint_name") or f"BP_{asset_key}",
            asset_root,
            source_mesh,
            decal_mesh,
        )
    ...
    if result_asset is None:
        raise RuntimeError("The Unreal import completed without a result asset.")
    try:
        unreal.EditorUtilityLibrary.sync_browser_to_objects([result_asset])
    except Exception:
        pass
    _log(
        f'Imported {manifest.get("source_name", asset_key)} as '
        f'{result_asset.get_path_name()} ({manifest.get("workflow")}).'
    )
    return {
        "result_asset": result_asset.get_path_name(),
