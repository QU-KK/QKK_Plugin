...
def surface_voronoi_polygon_normal(points, fallback=None):
    """Return a stable Newell normal for a world-space polygon."""
    points = list(points or ())
    if len(points) < 3:
        return safe_normalized(
            fallback or Vector((0.0, 0.0, 1.0)),
            Vector((0.0, 0.0, 1.0)),
        )
...
def surface_voronoi_polygon_centroid(points):
    """Return the arithmetic centroid used only for orientation decisions."""
    points = list(points or ())
    if not points:
...
def surface_voronoi_polygon_area(points, normal=None):
    """Return the unsigned area of a planar world-space polygon."""
    points = list(points or ())
    if len(points) < 3:
        return 0.0
    normal = safe_normalized(
        normal or surface_voronoi_polygon_normal(points),
        Vector((0.0, 0.0, 1.0)),
    )
...
def surface_voronoi_float32_polygon_area(points):
    """Return polygon area after Blender mesh-coordinate quantization."""
    from struct import pack, unpack

    quantized = [
        Vector(tuple(
            unpack("f", pack("f", component))[0]
    ...
    for index in range(1, len(quantized) - 1):
        area_vector += (
            quantized[index] - origin
        ).cross(quantized[index + 1] - origin)
    return area_vector.length * 0.5


def surface_voronoi_clean_polygon(points, tolerance=SURFACE_VORONOI_TOLERANCE):
    """Remove repeated and collinear vertices without changing winding."""
    cleaned = []
    for point in points or ():
        point = point.copy()
    ...
    while changed and len(cleaned) >= 3:
        ...
        for index, point in enumerate(cleaned):
            ...
            if (
                previous_vector.length <= tolerance
...
def surface_voronoi_clip_halfspace(
    polygon,
    plane_normal,
    plane_offset,
    tolerance=SURFACE_VORONOI_TOLERANCE,
):
    """Clip a convex 3D polygon to ``dot(normal, point) <= offset``."""
...
def surface_voronoi_face_is_planar(
    points,
    normal,
    tolerance=SURFACE_VORONOI_TOLERANCE,
):
    """True when all source points lie on one support plane."""
    points = list(points or ())
    if len(points) < 3:
        return False
    origin = points[0]
    scale = max(
        max((point - origin).length for point in points),
        1.0,
    )
    return all(
        abs(normal.dot(point - origin)) <= tolerance * scale
        for point in points[1:]
    )


def surface_voronoi_face_is_convex(
    points,
    normal,
    tolerance=SURFACE_VORONOI_TOLERANCE,
):
    """True when a planar polygon has one consistent turn direction."""
    points = list(points or ())
    if len(points) < 3:
        return False
    turn_sign = 0
    scale = max(
        max((points[index] - points[(index - 1) % len(points)]).length
...
def surface_voronoi_site_data(face_points, face_normal, site):
    """Return one inward-oriented boundary-line site record."""
    start = site["start"].copy()
    end = site["end"].copy()
    tangent = end - start
    length = tangent.length
    if length <= EPSILON:
        raise ValueError("Surface Voronoi sites must have non-zero length.")
    tangent /= length
    ...
    if supplied_inward is None:
        ...
        # Chart sites are oriented once from the source side face.  Keeping
        # that orientation fixed is what lets ownership propagate through
        # adjacent coplanar faces instead of flipping per target face.
        inward = safe_normalized(
            supplied_inward,
            face_normal.cross(tangent),
        )
    ...
    return {
        **site,
        ...
        "length": length,
        "inward": inward,
        "line_offset": inward.dot(start),
    }


def surface_voronoi_endpoint_join(
    site,
    endpoint,
    sites,
    face_normal,
    ...
    """Return the shared offset-line join for one finite segment endpoint."""
    ...
    for other in sites:
        if other is site:
            continue
        # Pair corners by source topology, never merely by coincident world
        # positions.  At a manifold vertex several faces and side sites share
        # the same coordinate; mixing them creates the long fan triangles seen
        # on arches and applied-bevel meshes.
        if (
            endpoint_vertex is not None
            ...
            other_away = -other["tangent"]
        elif endpoint_vertex is None and (
            other["start"] - endpoint
        ).length <= tolerance * 8.0:
            other_away = other["tangent"]
        elif endpoint_vertex is None and (
            other["end"] - endpoint
        ).length <= tolerance * 8.0:
        ...
        denominator = site["tangent"].cross(other["tangent"]).dot(
            face_normal
        )
        ...
        factor = (
            (other_origin - site_origin).cross(other["tangent"]).dot(
                face_normal
            )
            / denominator
        )
        ...
        if distance <= tolerance:
            continue
        # Prefer the geometrically adjacent incident segment.  The directional
        # score disambiguates branch vertices while the distance keeps a remote
        # collinear site from winning.
        turn = abs(site_away.cross(other_away).dot(face_normal))
        same_curve = (
            site.get("curve_id") is not None
            and site.get("curve_id") == other.get("curve_id")
        )
        candidates.append((
            0 if same_curve else 1,
            -turn,
            distance,
            str(other.get("id")),
            join,
    ...
    # The reference splits excessively sharp corners instead of allowing a
    # needle-like miter.  Both incident sites compute the same clamped point.
    delta = join - endpoint
...
def surface_voronoi_clip_endpoint_divider(
    polygon,
    endpoint,
    join,
    site_midpoint,
    ...
    """Clip a finite segment cell at its shared endpoint/miter divider."""
    ...
    return surface_voronoi_clip_halfspace(
        polygon,
        plane_normal,
        plane_offset,
        tolerance,
...
def surface_voronoi_point_segment_distance_squared(point, start, end):
    """Squared distance from a point to one finite 3D segment."""
    delta = end - start
...
def surface_voronoi_segments_are_local(
    first,
    second,
    face_width,
    tolerance=SURFACE_VORONOI_TOLERANCE,
):
    """True when two finite sites can compete inside the cropped band."""
    distance_squared = min(
        surface_voronoi_point_segment_distance_squared(
        ...
        surface_voronoi_point_segment_distance_squared(
            second["end"], first["start"], first["end"]
        ),
    )
...
def surface_voronoi_partition_face(
    face_points,
    sites,
    face_width,
    face_normal=None,
    ...
    """Partition one convex 3D support face into cropped edge-owned cells.

    ``sites`` are dictionaries containing ``start``, ``end`` and an optional
    stable ``id``.  The returned polygons remain on the original support face.
    Surface offset is a separate lifting operation because centerline points
    need the normals of both linked faces rather than only this face normal.
    """
    ...
    if face_normal is not None and normal.dot(face_normal) < 0.0:
        normal.negate()
    # Three points always define a plane.  Re-testing transformed triangles
    # against a recomputed normal can falsely reject valid FBX geometry at
    # small scales because the tolerance is below float mesh precision.
    if (
        len(face_points) > 3
        and not surface_voronoi_face_is_planar(
            face_points,
            normal,
            tolerance,
        )
    ):
        raise ValueError("Surface Voronoi requires a planar support face.")
    if not surface_voronoi_face_is_convex(face_points, normal, tolerance):
        raise ValueError("Surface Voronoi currently requires a convex support face.")

    all_prepared_sites = [
        surface_voronoi_site_data(face_points, normal, site)
        for site in sites or ()
    ]
    # Only sites whose finite, one-sided width neighborhood can touch this
    # carrier polygon may participate in ownership.  Allowing every boundary
    # line of a concave arch ngon to compete globally lets a remote line's
    # negative signed distance steal local corners and creates visible holes.
    active_sites = []
    for site in all_prepared_sites:
        signed_distances = [
            site["inward"].dot(point) - site["line_offset"]
            for point in face_points
        ]
        longitudinal = [
            (point - site["start"]).dot(site["tangent"])
            for point in face_points
        ]
    ...
    prepared_sites.sort(key=lambda site: (
        str(type(site.get("id")).__name__),
        str(site.get("id", "")),
        tuple(round(value, 12) for value in site["start"]),
        tuple(round(value, 12) for value in site["end"]),
    ))
    ...
    for site in prepared_sites:
        ...
        # Keep the cell between its source boundary line and the requested
        # inward width.  The lower clip also guards malformed sites whose line
        # lies infinitesimally outside the face because of transform noise.
        polygon = surface_voronoi_clip_halfspace(
            polygon,
            -site["inward"],
            -site["line_offset"],
            tolerance,
        ...
        polygon = surface_voronoi_clip_halfspace(
            polygon,
            site["inward"],
            site["line_offset"] + face_width,
            tolerance,
        )
        midpoint = (site["start"] + site["end"]) * 0.5
        start_join = surface_voronoi_endpoint_join(
            site,
            site["start"],
            all_prepared_sites,
            normal,
        ...
        end_join = surface_voronoi_endpoint_join(
            site,
            site["end"],
            all_prepared_sites,
            normal,
        ...
        polygon = surface_voronoi_clip_endpoint_divider(
            polygon,
            site["start"],
            start_join,
            midpoint,
        ...
        # Segment-interior ownership uses the source supporting lines.  Point
        # endpoint cells are added separately by the corner-processing stage.
        for other in prepared_sites:
            if other is site or len(polygon) < 3:
                continue
            if not surface_voronoi_segments_are_local(
                site,
                other,
                face_width,
                tolerance,
        ...
        cells.append({
            ...
            "polygon": polygon,
            "normal": normal.copy(),
            "inward": site["inward"].copy(),
            "line_offset": site["line_offset"],
            "area": area,
            "type": (
                "TRIANGLE" if len(polygon) == 3
...
def surface_voronoi_face_world_data(face, world_matrix, normal_matrix):
    """Cache the world polygon/plane used by surface-chart discovery."""
    points = [world_matrix @ vertex.co for vertex in face.verts]
    normal = surface_voronoi_polygon_normal(
        points,
    ...
    return {
        ...
        "points": points,
        "normal": normal,
        "centroid": surface_voronoi_polygon_centroid(points),
    }


def surface_voronoi_barycentric_map(point, flat_triangle, surface_triangle):
    """Map a point in a flattened triangle back onto its 3D carrier."""
    ...
    return (
        surface_triangle[0] * weight_a
        + surface_triangle[1] * weight_b
        + surface_triangle[2] * weight_c
    )


def surface_voronoi_partition_surface_face(
    face_data,
    sites,
    face_width,
    tolerance=SURFACE_VORONOI_TOLERANCE,
):
    """Partition a planar face or a virtual flattening of a non-planar face.

    Non-planar quads/ngons are tessellated only in a local parameter plane.
    Every clipped point is then barycentrically mapped back to its original 3D
    carrier triangle, so the generated decal follows the mesh rather than
    remaining on the temporary plane.
    """
    points = face_data["points"]
    normal = face_data["normal"]
    if (
        surface_voronoi_face_is_planar(points, normal, tolerance)
        and surface_voronoi_face_is_convex(points, normal, tolerance)
    ):
        cells = surface_voronoi_partition_face(
            points,
            sites,
            face_width,
            face_normal=normal,
        ...
        for cell in cells:
            cell["surface_polygon_raw"] = [
                point.copy() for point in cell["polygon"]
            ]
            cell["parameter_polygon"] = [
                point.copy() for point in cell["polygon"]
            ]
        return cells
    ...
    for index, point in enumerate(points):
        ...
        if candidate.length_squared > EPSILON:
            ...
            break
    if axis_u is None:
        raise ValueError("Surface Voronoi could not parameterize a support face.")
    ...
    for site in sites:
        ...
        if supplied_inward is not None:
            ...
            flat_site["inward"] = safe_normalized(
                projected,
                axis_v,
            )
    ...
    if not triangles:
        raise ValueError("Surface Voronoi could not triangulate a support face.")
    ...
    for triangle in triangles:
        if triangle and isinstance(triangle[0], int):
            ...
            flat_triangle = [
                flat_points[index].copy() for index in original_indices
            ]
        else:
            flat_triangle = [point.copy() for point in triangle]
            original_indices = [
                min(
                    range(len(flat_points)),
                    key=lambda index: (
                        flat_points[index] - point
                    ).length_squared,
                )
                for point in flat_triangle
            ]
        surface_triangle = [points[index].copy() for index in original_indices]
        triangle_normal = surface_voronoi_polygon_normal(
            surface_triangle,
            normal,
        )
        ...
        triangle_cells = surface_voronoi_partition_face(
            flat_triangle,
            flat_sites,
            face_width,
            face_normal=normal,
        ...
        for cell in triangle_cells:
            ...
            surface_polygon = [
                surface_voronoi_barycentric_map(
                    point,
                    flat_triangle,
                    surface_triangle,
                )
            ...
            cell["area"] = surface_voronoi_polygon_area(
                surface_polygon,
                triangle_normal,
            )
            cells.append(cell)
...
def surface_voronoi_clip_polygon_to_convex_carrier(
    polygon,
    carrier_points,
    carrier_normal,
    tolerance=SURFACE_VORONOI_TOLERANCE,
):
    """Intersect one planar cell with one convex surface carrier polygon."""
    ...
    if surface_voronoi_polygon_normal(
        carrier_points,
        normal,
    ).dot(normal) < 0.0:
    ...
    for index, start in enumerate(carrier_points):
        ...
        if edge_direction.length_squared <= tolerance * tolerance:
            continue
        # A consistently wound carrier keeps its interior to the left of
        # every directed edge. ``clip_halfspace`` retains <=, hence the
        # negated inward normal below.
...
def surface_voronoi_clip_polygon_to_surface_face(
    polygon,
    face_data,
    chart_normal,
    tolerance=SURFACE_VORONOI_TOLERANCE,
):
    """Clip one chart cell to a source face, triangulating only its domain."""
    ...
    if surface_voronoi_face_is_convex(face_points, normal, tolerance):
        clipped = surface_voronoi_clip_polygon_to_convex_carrier(
            polygon,
            face_points,
            normal,
            tolerance,
    ...
    # Concave source ngons are a domain description, not authored decal
    # topology. Clip against temporary carrier triangles and dissolve their
    # internal boundaries later by owner id.
    from mathutils.geometry import tessellate_polygon
    ...
    for triangle in triangles:
        triangle_points = (
            [face_points[index].copy() for index in triangle]
            if triangle and isinstance(triangle[0], int)
...
def surface_voronoi_faces_are_coplanar(
    reference,
    candidate,
    tolerance=SURFACE_VORONOI_TOLERANCE,
):
    """Return whether two connected face records share one oriented plane."""
    if reference["normal"].dot(candidate["normal"]) < 1.0 - 1.0e-6:
        return False
    origin = reference["points"][0]
    scale = max(
        max((point - origin).length for point in reference["points"]),
        max((point - origin).length for point in candidate["points"]),
        1.0,
    )
    return all(
        abs(reference["normal"].dot(point - origin))
        <= tolerance * 8.0 * scale
        for point in candidate["points"]
    )


def surface_voronoi_build_surface_charts(
    selected_edges,
    world_matrix,
    normal_matrix,
    tolerance=SURFACE_VORONOI_TOLERANCE,
):
    """Discover complete connected coplanar regions touched by the sites.

    A selected edge only seeds a chart.  The chart then crosses unselected
    internal edges, which is essential on triangulated or manually subdivided
    walls: Voronoi ownership belongs to the surface region, not to whichever
    source face happened to touch the selected segment.
    """
    seed_faces = {
        face
    ...
    def data(face):
        ...
        if record is None:
            record = surface_voronoi_face_world_data(
                face,
                world_matrix,
                normal_matrix,
            )
    ...
    for seed in sorted(seed_faces, key=lambda face: face.index):
        ...
        while pending:
            ...
            for edge in face.edges:
                for neighbor in edge.link_faces:
                    ...
                    if not surface_voronoi_faces_are_coplanar(
                        reference,
                        neighbor_data,
                        tolerance,
                    ):
        ...
        charts.append({
            "index": chart_index,
            "faces": sorted(chart_faces, key=lambda face: face.index),
            "normal": reference["normal"].copy(),
        })
    return charts, face_to_chart, world_data


def surface_voronoi_face_edge_inward(
    face,
    edge,
    world_matrix,
    face_normal,
    ...
    """Return the local face-interior direction from boundary loop winding.

    A face centroid is not a valid orientation test for concave arch/wall
    polygons: it may lie outside the local edge sector and flip the site.  The
    directed loop encodes the interior side at the edge unambiguously.
    """
...
def surface_voronoi_selected_vertex_normals(
    source_vertex,
    selected_edge_set,
    normal_matrix,
):
    """Collect unique support normals at a selected graph vertex."""
    normals = []
...
def surface_voronoi_lift_cell_point(
    point,
    cell,
    source_edge,
    selected_edge_set,
    ...
    """Lift a surface cell point while keeping shared centerlines welded."""
    ...
    longitudinal = (
        (point - site["start"]).dot(tangent) / max(tangent.length, EPSILON)
    )
    if (
        abs(signed_distance) > tolerance
    ...
    factor = (
        max(0.0, min(1.0, (point - start).dot(tangent) / length_squared))
        if length_squared > EPSILON
        else 0.0
    )
    ...
    if (point - start).length <= endpoint_tolerance:
        ...
        normals = surface_voronoi_selected_vertex_normals(
            source_vertex,
            selected_edge_set,
            normal_matrix,
        )
        ...
        normals = [
            transform_normal(normal_matrix, face.normal)
            for face in source_edge.link_faces
        ]

    source_point = start.lerp(end, factor)
    return offset_point_from_face_normals(
        source_point,
        normals,
        surface_offset,
    )
...
def surface_voronoi_sites_share_endpoint(first, second):
    """Return whether two sites meet at the same source-topology vertex."""
    first_vertices = {
        vertex for vertex in (
            first.get("start_vertex"), first.get("end_vertex")
        )
        if vertex is not None
    }
    second_vertices = {
        vertex for vertex in (
            second.get("start_vertex"), second.get("end_vertex")
        )
        if vertex is not None
    }
    ...
    return any(
        (first[first_key] - second[second_key]).length
...
def surface_voronoi_assign_curve_ids(chart_index, sites):
    """Tag connected boundary segments so one source curve never self-clamps."""
    sites = list(sites or ())
    ...
    for indices in vertex_sites.values():
        # A degree-2 vertex continues one curve. Junctions terminate paths;
        # merging all incident sites there would turn the whole wall boundary
        # graph into one self-exempt curve and disable local overlap clamping.
        if len(indices) == 2:
...
def surface_voronoi_conditional_band_ownership(
    polygon,
    competitor_band,
    ownership_normal,
    ownership_offset,
    ...
    """Apply one ownership bisector only inside a competitor's finite band.

    A signed supporting-line comparison is valid where both finite inward
    bands exist. Applying it globally on a concave chart lets a remote line
    with negative signed distance erase an unrelated cell. This routine
    preserves the part outside ``competitor_band`` and clips only the actual
    overlap by the pairwise bisector.
    """
    ...
    for index, start in enumerate(competitor):
        ...
        if edge_direction.length_squared <= tolerance * tolerance:
            continue
        inward = safe_normalized(normal.cross(edge_direction))
        outside = surface_voronoi_clip_halfspace(
            remaining,
            inward,
            inward.dot(start),
            tolerance,
        ...
        remaining = surface_voronoi_clip_halfspace(
            remaining,
            -inward,
            (-inward).dot(start),
            tolerance,
    ...
    owned_overlap = surface_voronoi_clip_halfspace(
        remaining,
        ownership_normal,
        ownership_offset,
        tolerance,
    ...
    return [
        ...
        if len(cleaned := surface_voronoi_clean_polygon(
            fragment,
            tolerance,
        )) >= 3
        and surface_voronoi_polygon_area(cleaned, normal)
        > tolerance * tolerance
    ]


def surface_voronoi_reference_chart_cells(
    chart,
    sites,
    selected_edge_set,
    world_matrix,
    ...
    """Build the reference finite-segment bands directly on one 3D chart.

    The source face tessellation is only a carrier and must never become
    authored topology.  Each site therefore produces its cropped band from
    its own segment and the shared offset-line joins at its endpoints.  Short
    segments whose inner joins cross collapse to a corner triangle.
    """
    normal = chart["normal"]
    prepared_sites = []
    for site in sites:
        prepared_sites.append(surface_voronoi_site_data(
            [
                site["start"],
                site["end"],
                site["start"] + site["inward"],
            ],
            normal,
            site,
        ))
    ...
    for site in prepared_sites:
        ...
        if surface_voronoi_polygon_normal(polygon, normal).dot(normal) < 0.0:
            polygon.reverse()
        if (
            surface_voronoi_polygon_area(polygon, normal)
            <= tolerance * tolerance
        ):
            continue
    ...
    for site in prepared_sites:
        ...
        # The fixed-width bands are only the crop stage.  Their final extents
        # come from segment ownership inside actual band overlaps. Comparing
        # infinite signed lines over the whole concave chart is invalid: a
        # remote boundary's negative half-plane can otherwise erase this cell.
        for other in prepared_sites if partition_ownership else ():
            ...
            if surface_voronoi_sites_share_endpoint(site, other):
                # Adjacent point cells are already resolved by their shared
                # offset-line miter (or collapsed corner triangle).
                continue
            if not surface_voronoi_segments_are_local(
                site,
                other,
            ...
            fragments = [
                ...
                for owned_fragment in surface_voronoi_conditional_band_ownership(
                    fragment,
                    competitor_band,
                    ownership_normal,
                    ownership_offset,
        ...
        for polygon in fragments:
            ...
            if area <= tolerance * tolerance:
                continue
            lifted = [
                surface_voronoi_lift_cell_point(
                    point,
                    cell_stub,
                    source_edge,
                    selected_edge_set,
            ...
            records.append({
                "id": site.get("id"),
                "polygon": lifted,
                "surface_polygon": [point.copy() for point in polygon],
                "surface_polygon_raw": [point.copy() for point in polygon],
                "parameter_polygon": [point.copy() for point in polygon],
                "normal": normal.copy(),
                "area": area,
                "type": (
                    "TRIANGLE" if len(polygon) == 3
                ...
                "source_side_face": source_face,
                "source_side_index": site["source_side_index"],
                "source_edge_index": source_edge.index,
                "source_face_index": source_face.index,
                "chart_index": chart["index"],
                "owner_id": (chart["index"], site["id"]),
            })
    return surface_voronoi_merge_chart_fragments(records, tolerance)


def surface_voronoi_point_in_polygon_2d(
    point,
    polygon,
    tolerance=SURFACE_VORONOI_TOLERANCE,
):
    """Even-odd containment with boundary inclusion for arbitrary 2D loops."""
    polygon = list(polygon or ())
    if len(polygon) < 3:
        return False
    inside = False
    for index, start in enumerate(polygon):
        ...
        if edge_length_squared > EPSILON:
            factor = max(
                0.0,
                min(1.0, relative.dot(edge) / edge_length_squared),
            )
...
def surface_voronoi_bisector_segment_in_overlap(
    first_polygon,
    second_polygon,
    ownership_normal,
    ownership_offset,
    ...
    """Return the supporting-line bisector segment inside two finite bands."""
    if ownership_normal.length_squared <= tolerance * tolerance:
        return None
    overlap = surface_voronoi_clip_polygon_to_convex_carrier(
        first_polygon,
        second_polygon,
        face_normal,
        tolerance,
...
def surface_voronoi_chart_arrangement_cells(
    chart,
    base_cells,
    world_data,
    selected_edge_set,
    ...
    surface_offset,
    tolerance=SURFACE_VORONOI_TOLERANCE,
    overlap_clearance=1.0,
):
    """Resolve one chart as a constrained, non-overlapping planar partition.

    Carrier boundaries, finite width bands, and local pairwise bisectors form
    one planar arrangement. Every resulting region is classified by true
    finite-segment distance, eliminating infinite-line theft on concave charts
    while guaranteeing that no generated cell can leave the source domain.
    """
    if not base_cells:
        return []
    ...
    def vertex_for(point):
        key = (
            int(round(point.x / vertex_tolerance)),
            int(round(point.y / vertex_tolerance)),
        )
        for offset_x in (-1, 0, 1):
            for offset_y in (-1, 0, 1):
                for index in vertex_buckets.get(
    ...
    for first_index, first in enumerate(base_cells[:-1]):
        ...
        for second in base_cells[first_index + 1:]:
            ...
            if (
                first_site.get("curve_id") is not None
                and first_site.get("curve_id")
                == second_site.get("curve_id")
            ):
                continue
            ...
            ownership_boundaries = (
                (
                    first_site["inward"]
                    - second_site["inward"] * overlap_clearance,
                    first_site["line_offset"]
                    - second_site["line_offset"] * overlap_clearance,
                ),
                (
                    first_site["inward"] * overlap_clearance
                    - second_site["inward"],
                    first_site["line_offset"] * overlap_clearance
                    - second_site["line_offset"],
                ),
            )
            for ownership_normal, ownership_offset in ownership_boundaries:
                bisector = surface_voronoi_bisector_segment_in_overlap(
                    first_polygon,
                    second["surface_polygon_raw"],
                    ownership_normal,
                    ownership_offset,
                    normal,
                    tolerance,
    ...
    for output_face in output_faces:
        if len(output_face) < 3:
            continue
        parameter_polygon = [
            output_vertices[index].copy() for index in output_face
        ]
        centroid = sum(parameter_polygon, Vector((0.0, 0.0))) / len(
            parameter_polygon
        )
        if not any(
            surface_voronoi_point_in_polygon_2d(
                centroid,
                carrier_polygon,
                tolerance * 8.0,
            )
        ...
        candidates = [
            cell
            for cell, band_polygon in zip(base_cells, band_polygons_2d)
            if surface_voronoi_point_in_polygon_2d(
                centroid,
                band_polygon,
                tolerance * 8.0,
            )
        ...
        if competing_distances:
            owner_distance = surface_voronoi_point_segment_distance_squared(
                surface_centroid,
                owner["site"]["start"],
                owner["site"]["end"],
            )
            if (
                ...
                * min(competing_distances)
                ...
                continue
        surface_polygon = [
            lift_parameter(point) for point in parameter_polygon
        ]
        if surface_voronoi_polygon_normal(
            surface_polygon,
            normal,
        ...
        records.append({
            **owner,
            ...
            "surface_polygon_raw": [point.copy() for point in surface_polygon],
            "parameter_polygon": [point.copy() for point in surface_polygon],
            "normal": normal.copy(),
            "area": area,
            "type": (
                "TRIANGLE" if len(surface_polygon) == 3
                else "QUAD" if len(surface_polygon) == 4
                else "IRREGULAR"
            ),
            "arrangement_cell": True,
        })
    return records


def build_surface_voronoi_cells(
    selected_edges,
    world_matrix,
    normal_matrix,
    face_width,
    ...
    """Build experimental 3D cells for selected manifold source edges.

    The result is intentionally data-only and does not mutate Blender meshes.
    Each record contains a world-space polygon, its source BMesh edge/face,
    semantic type, and ownership data.  This makes the kernel testable before
    it is connected to generation operators.
    """
    selected_edges = sorted(
        {
            edge for edge in selected_edges or ()
    ...
    charts, face_to_chart, world_data = (
        surface_voronoi_build_surface_charts(
            selected_edges,
            world_matrix,
            normal_matrix,
            tolerance,
    ...
    for chart in charts:
        ...
        for edge in selected_edges:
            ...
            midpoint = (start + end) * 0.5
            emitted_inwards = []
            for side_face in sorted(
                linked_chart_faces,
                key=lambda face: face.index,
            ):
                side_data = world_data[side_face]
                inward = surface_voronoi_face_edge_inward(
                    side_face,
                    edge,
                    world_matrix,
                    chart["normal"],
                    side_data["centroid"] - midpoint,
                )
                ...
                side_index = next(
                    index
                    for index, face in enumerate(edge.link_faces)
                    if face == side_face
                )
                ...
                sites.append({
                    ...
                    "end": end,
                    "inward": inward,
                    "source_side_index": side_index,
                    "source_edge": edge,
                    "source_side_face": side_face,
        ...
        # Build finite segment/endpoint bands first, then resolve all domain,
        # band and ownership boundaries in one constrained chart arrangement.
        # This avoids both infinite-line theft and per-competitor fragment
        # multiplication on concave surfaces.
        base_cells = surface_voronoi_reference_chart_cells(
            chart,
            sites,
            selected_edge_set,
            world_matrix,
            ...
            0.0,
            tolerance,
            partition_ownership=False,
        )
        records.extend(surface_voronoi_chart_arrangement_cells(
            chart,
            base_cells,
            world_data,
            selected_edge_set,
...
def surface_voronoi_vertex_key(
    point,
    tolerance=SURFACE_VORONOI_TOLERANCE,
):
    """Return a deterministic spatial key for weldable generated vertices."""
    quantum = max(float(tolerance), 1.0e-10)
    return tuple(int(round(component / quantum)) for component in point)


def surface_voronoi_merge_chart_fragments(
    cells,
    tolerance=SURFACE_VORONOI_TOLERANCE,
):
    """Dissolve support-face boundaries inside each planar owner cell.

    The Voronoi computation clips against every source face in a chart.  Those
    faces are merely a carrier for the surface and must not leak their
    arbitrary triangulation into the generated decal.  Shared fragment edges
    are cancelled and the remaining boundary loops become the authored cells.
    """
    groups = {}
    for cell in cells or ():
        groups.setdefault(
            cell.get(
                "owner_id",
                (cell.get("chart_index"), cell.get("id")),
            ),
            [],
        ).append(cell)
    ...
    for owner_id in sorted(groups, key=lambda item: str(item)):
                ...
                for offset_y in (-1, 0, 1):
                    for offset_z in (-1, 0, 1):
                        ...
                        for index in buckets.get(neighbor, ()):
                            if (
                                nodes[index][0] - surface_point
                            ).length <= weld_tolerance:
                                return index
            index = len(nodes)
            nodes.append((
                surface_point.copy(),
                lifted_point.copy(),
                parameter_point.copy(),
            ))
        ...
        for fragment in fragments:
            ...
            parameter_polygon = list(
                fragment.get("parameter_polygon", surface_polygon)
            )
            ...
            if surface_voronoi_polygon_normal(
                ...
                parameter_polygon.reverse()
            polygon_nodes = [
                node_for(surface_point, lifted_point, parameter_point)
                for surface_point, lifted_point, parameter_point in zip(
                    surface_polygon,
                    lifted_polygon,
                    parameter_polygon,
                )
        ...
        # Adjacent clipped fragments frequently meet with T-junctions: one
        # polygon owns a long edge while its neighbours own two or more
        # collinear sub-edges.  Cancelling only identical endpoint pairs leaks
        # those internal sample/carrier boundaries into the authored mesh.
        # Split every edge at all welded nodes lying on it before counting
        # uses, so all such boundaries dissolve exactly.
        edge_uses = {}
            ...
            for edge_index, start in enumerate(polygon_nodes):
                ...
                for split_index in range(len(split_nodes) - 1):
                    ...
                    edge_uses.setdefault(key, []).append(
                        (sub_start, sub_end, polygon_index)
                    )

        # An owner can occur in several disconnected islands after a competing
        # curve cuts through its finite band.  Treating all of those fragments
        # as one polygon group either prevents any dissolve or, worse, fills
        # between islands with long diagonals.  Build edge-connected fragment
        # components first, then dissolve each component independently.
        ...
        for component_indices in components.values():
            ...
            for uses in edge_uses.values():
                component_uses = [
                    use for use in uses if use[2] in component_set
                ]
            ...
            # A connected component with more than one loop contains a hole.
            # Blender faces cannot represent holes, so retain its safe source
            # fragments instead of inventing diagonals across the opening.
            ...
            area = surface_voronoi_polygon_area(
                surface_polygon,
                template["normal"],
            )
            if area <= tolerance * tolerance:
            ...
            merged.append({
                **template,
                "polygon": lifted_polygon,
                "surface_polygon": surface_polygon,
                "parameter_polygon": parameter_polygon,
                "area": area,
                "type": (
                    "TRIANGLE" if len(loop) == 3
                    else "QUAD" if len(loop) == 4
                    else "IRREGULAR"
                ),
            })
    return merged
...
def surface_voronoi_cell_uvs(cell, face_width, uv_scale=1.0):
    """Generate semantic strip UVs from owner-edge distance and direction."""
    site = cell["site"]
    ...
    if side_index is None:
        side_index = next(
            (
                index
                for index, face in enumerate(source_edge.link_faces)
                if face == source_face
            ),
            0,
        )
    ...
    uvs = []
    parameter_polygon = cell.get(
        "parameter_polygon",
        cell["surface_polygon"],
    )
    for point in parameter_polygon:
        longitudinal = (
            point - site["start"]
        ).dot(site["tangent"]) * safe_scale
        distance = (
            site["inward"].dot(point) - site["line_offset"]
        )
...
def surface_voronoi_clean_generated_loop(records):
    """Remove adjacent welded duplicates from one generated face loop."""
    cleaned = []
...
def surface_voronoi_split_self_touching_loop(records):
    """Split a loop whenever welding made one vertex occur twice."""
    ...
    for position, record in enumerate(records):
        ...
        return (
            surface_voronoi_split_self_touching_loop(first_loop)
            + surface_voronoi_split_self_touching_loop(second_loop)
        )
    return [records]


def surface_voronoi_generated_loop_is_safe_quad(
    records,
    vertices,
    target_normal,
    tolerance=SURFACE_VORONOI_TOLERANCE,
):
    """True for a non-bow-tie convex four-corner generated cell."""
    if len(records) != 4:
        return False
...
def surface_voronoi_generated_loop_is_safe_ngon(
    records,
    vertices,
    target_normal,
    tolerance=SURFACE_VORONOI_TOLERANCE,
):
    """True when one merged owner loop is planar and non-self-intersecting."""
    if len(records) < 5:
        return False
    ...
    scale = max(max((point - origin).length for point in points), 1.0)
    if any(
        abs(normal.dot(point - origin)) > tolerance * 16.0 * scale
        for point in points[1:]
    ):
        return False
    if surface_voronoi_float32_polygon_area(points) <= tolerance * tolerance:
    ...
    def segments_intersect(a, b, c, d):
        ...
        relative = c - a
        if abs(denominator) <= tolerance:
            # Collinear non-adjacent overlaps are unsafe for a Blender ngon.
...
def surface_voronoi_finalize_generated_loop(
    records,
    vertices,
    target_normal,
    tolerance=SURFACE_VORONOI_TOLERANCE,
    ...
    """Return safe triangle/convex-quad loops for Blender mesh creation."""
    records = surface_voronoi_clean_generated_loop(records)
    if len(records) < 3 or len({record[0] for record in records}) < 3:
        return []
    def has_area(candidate):
        # Blender stores mesh coordinates as float32.  Test the coordinates at
        # that precision so a mathematically tiny sliver cannot collapse into
        # a collinear face only after ``Mesh.from_pydata``.
    ...
    for triangle in triangles:
        if triangle and isinstance(triangle[0], int):
            ...
            positions = [
                min(
                    range(len(parameter_points)),
                    key=lambda index: (
                        parameter_points[index] - point
                    ).length_squared,
                )
                for point in triangle
            ]
        triangle_records = [records[position] for position in positions]
        triangle_records = surface_voronoi_clean_generated_loop(
            triangle_records
        )
        if (
            len(triangle_records) == 3
    ...
    def paired_quad(first, second):
        ...
        if not surface_voronoi_generated_loop_is_safe_quad(
            quad,
            vertices,
            target_normal,
            tolerance,
    ...
    # Tessellation is used only to discover a valid interior decomposition.
    # Pair adjacent triangles back into the best planar convex quads so a
    # five-corner crop becomes one quad plus one corner triangle rather than a
    # visible three-triangle fan.
    pair_candidates = []
    for first_index in range(len(triangle_loops) - 1):
        for second_index in range(first_index + 1, len(triangle_loops)):
            quad = paired_quad(
                triangle_loops[first_index],
                triangle_loops[second_index],
            )
            if quad is None:
                continue
            points = [vertices[record[0]] for record in quad]
            lengths = [
                (points[(index + 1) % 4] - point).length
                for index, point in enumerate(points)
            ]
            quality = min(lengths) / max(max(lengths), tolerance)
            pair_candidates.append(
                (-quality, first_index, second_index, quad)
            )
    used = set()
    finalized = []
    for _score, first_index, second_index, quad in sorted(
        pair_candidates,
    ...
    finalized.extend(
        triangle_loop
        for index, triangle_loop in enumerate(triangle_loops)
        if index not in used
    )
    return finalized


def build_surface_voronoi_graph_strip(
    selected_edges,
    world_matrix,
    normal_matrix,
    face_width,
    ...
    """Append welded experimental Voronoi cells to decal mesh buffers.

    This mirrors the data contract of ``build_selected_edge_graph_strip`` but
    remains opt-in. Ownership-cell boundaries on the same support face and
    shared offset centerlines across folded faces reuse one vertex index.
    """
    cells = build_surface_voronoi_cells(
        selected_edges,
        world_matrix,
        normal_matrix,
        face_width,
    ...
    weld_tolerance = max(
        float(tolerance) * 8.0,
        abs(float(face_width)) * 1.0e-6,
        1.0e-9,
    )

    # A surface-chart boundary is emitted once by every incident chart.  The
    # un-offset points agree, but independently offsetting them along each
    # chart normal creates duplicate vertices, open corner seams and faces
    # that merely touch instead of sharing an edge.  Collect those logical
    # surface vertices first and give every occurrence one canonical mitered
    # position computed from all incident chart normals.
    surface_weld_tolerance = max(
        float(tolerance) * 32.0,
        abs(float(face_width)) * 1.0e-6,
        1.0e-9,
    )
    surface_groups = []
        ...
        for offset_x in (-1, 0, 1):
            for offset_y in (-1, 0, 1):
                for offset_z in (-1, 0, 1):
                    ...
                    for group_index in surface_group_buckets.get(
                        neighbor_key,
                        (),
                    ):
                        group = surface_groups[group_index]
                        if (
                            group["surface_point"] - point
                        ).length <= surface_weld_tolerance:
                            return group
        if not create:
            return None
        group = {
    ...
    for cell in cells:
        for surface_point, lifted_point in zip(
            cell["surface_polygon"],
            cell["polygon"],
        ):
    ...
    for group in surface_groups:
        if surface_offset <= EPSILON:
            ...
            group["canonical_point"] = offset_point_from_face_normals(
                group["surface_point"],
                group["normals"],
                surface_offset,
            )
        else:
            # Preserve the established selected-edge endpoint miter when this
            # occurrence belongs to only one chart, while still collapsing
            # tiny numerical differences between fragments in that chart.
            group["canonical_point"] = sum(
                group["lifted_points"],
                Vector((0.0, 0.0, 0.0)),
            ) / len(group["lifted_points"])
    ...
    emitted_face_keys = {
        tuple(sorted(face)) for face in faces_out
    }
    ...
    for cell in cells:
        ...
        for surface_point, lifted_point, parameter_point in zip(
            ...
            vertex_index = find_or_append_vertex(
                canonical_lifted_point(surface_point, lifted_point)
            )
            indices.append(vertex_index)
            signed_distance = (
                site["inward"].dot(parameter_point) - site["line_offset"]
            )
            longitudinal = (
                parameter_point - site["start"]
            ).dot(site["tangent"])
            center_flags.append(
                abs(signed_distance) <= tolerance
                and -tolerance <= longitudinal <= site["length"] + tolerance
            )
        ...
        for simple_loop in simple_loops:
            for final_loop in surface_voronoi_finalize_generated_loop(
                simple_loop,
                vertices_out,
                cell["normal"],
                tolerance=tolerance,
                ...
                final_indices, final_uvs = oriented_face_with_uvs(
                    final_indices,
                    final_uvs,
                    vertices_out,
                    cell["normal"],
                ...
                for vertex_index, is_center in zip(
                    final_indices,
                    final_center_flags,
                ):
