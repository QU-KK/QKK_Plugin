...
def _intersection_point_in_triangle(point, triangle, tolerance):
    """Return whether a coplanar world-space point is inside a triangle."""
    a, b, c = triangle
    v0 = b - a
    v1 = c - a
    v2 = point - a
    ...
    return (
        ...
        and u + v <= 1.0 + barycentric_tolerance
    )


def _intersection_triangle_plane_points(
    triangle,
    other_triangle,
    plane_point,
    plane_normal,
...
def intersect_world_triangles(triangle_a, triangle_b, tolerance=1.0e-6):
    """Return the line segment shared by two non-coplanar triangles."""
    normal_a = (triangle_a[1] - triangle_a[0]).cross(
        triangle_a[2] - triangle_a[0]
    )
    normal_b = (triangle_b[1] - triangle_b[0]).cross(
    ...
    points = _intersection_triangle_plane_points(
        triangle_a,
        triangle_b,
        triangle_b[0],
        normal_b,
    ...
    points.extend(
        _intersection_triangle_plane_points(
            triangle_b,
            triangle_a,
            triangle_a[0],
            normal_a,
...
class _IntersectionSegment:
    ...
    def __init__(
        ...
        normal_a,
        normal_b,
        triangle_a_index=None,
        triangle_b_index=None,
    ):
...
def _evaluated_world_surface(obj, depsgraph, tolerance=1.0e-7):
    """Return evaluated world triangles, adjacency, and a nearest-point BVH."""
    ...
    try:
        ...
        triangles = [
            tuple(world_vertices[index].copy() for index in indices)
            for indices in triangle_vertex_indices
        ]
    finally:
    ...
    for triangle_index, indices in enumerate(triangle_vertex_indices):
        # Barycentric coordinate i is zero on the edge opposite vertex i.
        for opposite_index in range(3):
            edge = tuple(sorted((
                indices[(opposite_index + 1) % 3],
                indices[(opposite_index + 2) % 3],
            )))
            ...
            world_edge = (
                world_vertices[edge[0]].copy(),
                world_vertices[edge[1]].copy(),
            )
    ...
    bvh = BVHTree.FromPolygons(
        ...
        [tuple(range(index * 3, index * 3 + 3)) for index in range(len(triangles))],
        all_triangles=True,
        epsilon=tolerance,
...
def _evaluated_world_triangles(obj, depsgraph):
    """Compatibility wrapper used by older intersection tests/helpers."""
...
def _intersection_append_evaluated_bmesh(target_bmesh, obj, depsgraph, source_id):
    """Append one evaluated object in world space while retaining provenance."""
    evaluated = obj.evaluated_get(depsgraph)
    mesh = evaluated.to_mesh(
        preserve_all_data_layers=False,
        depsgraph=depsgraph,
    ...
    try:
        ...
        vertex_map = {
            vertex: target_bmesh.verts.new(vertex.co)
            for vertex in source_bmesh.verts
        }
        for face in source_bmesh.faces:
            try:
                new_face = target_bmesh.faces.new(tuple(
                    vertex_map[vertex]
...
def _exact_intersection_segments(
    obj_a,
    obj_b,
    depsgraph,
    surface_a,
    ...
    """Use Blender's Exact Intersect solver on a disposable combined mesh."""
    context = bpy.context
    ...
    try:
        combined = bmesh.new()
        try:
            _intersection_append_evaluated_bmesh(
                combined,
                obj_a,
                depsgraph,
                0,
        ...
        result = bpy.ops.mesh.intersect(
            ...
            separate_mode="NONE",
            threshold=max(float(tolerance), 1.0e-7),
            solver="EXACT",
        )
        ...
        for edge in edit_bmesh.edges:
            ...
            if not normals_a or not normals_b:
                continue
            normal_a = safe_normalized(sum(
                normals_a,
                Vector((0.0, 0.0, 0.0)),
            ))
            normal_b = safe_normalized(sum(
                normals_b,
                Vector((0.0, 0.0, 0.0)),
            ))
            ...
            segments.append(_IntersectionSegment(
                start,
                end,
                normal_a,
                normal_b,
        ...
        # Exact Intersect is context-sensitive.  Background generation and
        # unusual editor states retain the deterministic BVH fallback below.
        return []
    finally:
...
def mesh_intersection_data(obj_a, obj_b, depsgraph, tolerance=1.0e-6):
    """Extract segments together with their evaluated supporting surfaces."""
    ...
    exact_segments = _exact_intersection_segments(
        obj_a,
        obj_b,
        depsgraph,
        surface_a,
    ...
    bvh_a = BVHTree.FromPolygons(
        [vertex for triangle in triangles_a for vertex in triangle],
        [tuple(range(index * 3, index * 3 + 3)) for index in range(len(triangles_a))],
        all_triangles=True,
        epsilon=tolerance,
    )
    bvh_b = BVHTree.FromPolygons(
        [vertex for triangle in triangles_b for vertex in triangle],
        [tuple(range(index * 3, index * 3 + 3)) for index in range(len(triangles_b))],
        all_triangles=True,
        epsilon=tolerance,
    )
    ...
    for triangle_a_index, triangle_b_index in bvh_a.overlap(bvh_b):
        segment = intersect_world_triangles(
            triangles_a[triangle_a_index],
            triangles_b[triangle_b_index],
            tolerance,
        )
        if segment is None:
            continue
        ...
        candidate = _IntersectionSegment(
            start,
            end,
            normal_a,
            normal_b,
...
def mesh_intersection_segments(obj_a, obj_b, depsgraph, tolerance=1.0e-6):
    """Extract unique world-space triangle intersection segments."""
    segments, _surface_a, _surface_b = mesh_intersection_data(
        obj_a,
        obj_b,
        depsgraph,
        tolerance=tolerance,
...
def _intersection_direction_clusters(directions, minimum_dot=0.7):
    """Average smoothly varying directions while retaining genuine corners."""
    clusters = []
    for direction in directions:
        ...
        matching = next(
            (
                ...
                if safe_normalized(cluster["sum"]).dot(candidate) >= minimum_dot
            ),
            None,
        )
        if matching is None:
...
def _intersection_triangle_pair_score(points, first, second):
    normal_a = (
        points[first[1]] - points[first[0]]
    ).cross(
        points[first[2]] - points[first[0]]
    )
    normal_b = (
        points[second[1]] - points[second[0]]
    ).cross(
        points[second[2]] - points[second[0]]
    )
    if (
        normal_a.length_squared <= EPSILON
...
def _intersection_oriented_face_parts(
    indices,
    uvs,
    vertices,
    target_normal,
):
    """Keep a quad unless its alternate diagonal removes a local fold."""
    points = [vertices[index] for index in indices]
    default_score = _intersection_triangle_pair_score(
        points,
        (0, 1, 2),
        (0, 2, 3),
    )
    alternate_score = _intersection_triangle_pair_score(
        points,
        (0, 1, 3),
        (1, 2, 3),
    )
    if default_score >= -1.0e-6 or alternate_score <= default_score:
        return (oriented_face_with_uvs(
            indices,
            uvs,
            vertices,
            target_normal,
    ...
    for corners in ((0, 1, 3), (1, 2, 3)):
        parts.append(oriented_face_with_uvs(
            tuple(indices[index] for index in corners),
            tuple(uvs[index] for index in corners),
            vertices,
            target_normal,
        ))
    return tuple(parts)
...
def _intersection_triangle_barycentric(point, triangle):
    """Return barycentric coordinates for a point in a triangle's plane."""
    a, b, c = triangle
    edge_ab = b - a
    edge_ac = c - a
    relative = point - a
    ...
    if abs(denominator) <= EPSILON:
        return None
    coordinate_b = (
        dot_ac_ac * dot_relative_ab - dot_ab_ac * dot_relative_ac
    ) / denominator
    coordinate_c = (
        dot_ab_ab * dot_relative_ac - dot_ab_ac * dot_relative_ab
    ) / denominator
...
def _intersection_rotate_across_edge(direction, old_normal, new_normal, edge):
    """Parallel-transport a tangent direction across a triangle edge."""
...
def _intersection_walk_surface(
    surface,
    start,
    direction,
    distance,
    ...
    """Trace a fixed-distance tangent step over an evaluated triangle mesh.

    Each crossed triangle is unfolded through its shared edge by parallel
    transporting the direction.  The returned point therefore remains on the
    source mesh rather than being a free-space average of adjacent normals.
    """
    if (
        surface is None
    ...
    for _step in range(128):
        ...
        # Remove the tiny plane error inherited from triangle intersection
        # tolerances before evaluating barycentric edge crossings.
        ...
        crossings = []
        for opposite_index, (start_weight, end_weight) in enumerate(zip(
            current_barycentric,
            target_barycentric,
        )):
        ...
        tangent = _intersection_rotate_across_edge(
            tangent,
            normal,
            next_normal,
            edge,
...
def _intersection_project_to_surface(surface, point, surface_offset=0.0):
    """Project a candidate to one owned source surface, never the other one."""
    bvh = surface.get("bvh") if surface is not None else None
...
def _intersection_hard_join_edge_point(
    surface,
    triangle_a_index,
    triangle_b_index,
    point_a,
    ...
    """Meet two hard-face rail points on their actual shared mesh edge."""
...
def _intersection_project_miter_to_shared_edge(
    surface,
    triangle_a_index,
    triangle_b_index,
    candidate,
):
    """Constrain a shared hard-corner miter to both supporting faces."""
    if surface is None or triangle_a_index is None or triangle_b_index is None:
        return None
    edge = surface["shared_edges"].get((triangle_a_index, triangle_b_index))
    if edge is None:
...
def _intersection_surface_miter_is_valid(
    origin,
    candidate,
    sides,
    normals,
    ...
    maximum_plane_error_ratio=0.025,
):
    """Reject projected miters which reverse or distort either incident rail."""
    ...
    maximum_plane_error = max(
        tolerance * 8.0,
        face_width * maximum_plane_error_ratio,
    )
...
def build_intersection_decal_mesh(
    segments,
    face_width,
    surface_offset,
    uv_scale,
    ...
    """Build connected source-owned ribbons around an intersection graph."""
    vertices = []
    faces = []
    face_uvs = []
    center_vertices = set()
    ...
    if segment_points:
        span = max(
            max(point[axis] for point in segment_points)
    ...
    def graph_node_key(point):
        ...
        bucket = tuple(
            int(component // vertex_tolerance)
            for component in point
        )
    ...
    for source_segment in segments:
        ...
        prepared_segments.append({
            ...
            "side_a": side_a,
            "side_b": side_b,
            "triangle_a_index": getattr(
                source_segment,
                "triangle_a_index",
                None,
            ),
            "triangle_b_index": getattr(
                source_segment,
                "triangle_b_index",
                None,
            ),
    ...
    # Exact/triangle solvers can leave a microscopic break when a curve
    # endpoint lands numerically on a hard source corner.  Heal the closest
    # paired endpoints inside a deliberately tiny width-relative radius.
    healing_distance = max(
        vertex_tolerance * 12.0,
        face_width * 0.01,
        span * 2.0e-6,
    )
    open_node_keys = [
    ...
    for node_key in open_node_keys:
        ...
        for candidate_key in open_node_keys:
            ...
            if distance >= best_distance:
                continue
            # Exact intersections can represent the same hard corner with
            # slightly different coordinates on its incident faces.  At this
            # deliberately tiny, width-relative distance, proximity is more
            # reliable than requiring nearly opposite tangents or matching
            # face normals; both legitimately change at a corner.
        ...
        for candidate_segment_index, candidate_endpoint in candidate["incidents"]:
            prepared_segments[candidate_segment_index][
                f"{candidate_endpoint}_key"
            ] = node_key
    ...
    while unvisited_nodes:
        ...
        while pending_nodes:
            ...
            for segment_index, endpoint in graph_nodes[current_node]["incidents"]:
                ...
                neighbor = segment[
                    "end_key" if endpoint == "start" else "start_key"
                ]
        ...
        if component_nodes and all(
            len(graph_nodes[node_key]["incidents"]) == 2
            for node_key in component_nodes
        ):
            closed_component_seam_nodes.append(
                min(
                    component_nodes,
                    key=lambda node_key: tuple(
                        graph_nodes[node_key]["point_sum"]
                        / len(graph_nodes[node_key]["points"])
                    ),
                )
            )
    maximum_miter_distance = max(
        face_width,
        face_width * min(max(1.0, float(miter_limit)), 1.5),
    )
    ...
    for node_key, node in graph_nodes.items():
        ...
        all_normals = (
            _intersection_direction_clusters(normals_a)
        ...
        for slot in ("a", "b"):
            rail_group = [
                (segment_index, endpoint, slot)
                for segment_index, endpoint in node["incidents"]
            ]
            normals = [
                prepared_segments[segment_index][f"normal_{slot}"]
                for segment_index, _endpoint, slot in rail_group
            ]
            sides = [
                prepared_segments[segment_index][f"side_{slot}"]
                for segment_index, _endpoint, slot in rail_group
            ]
            ...
            # A shared rail corner needs one bounded direction, not a point
            # satisfying every width plane independently. The latter can jump
            # across a neighboring quad at a hard 3D turn and create the black
            # folded wedges seen on polyhedral intersections. The normalized
            # side sum is the ordinary miter bisector. Scaling by its weakest
            # incident projection preserves the requested width at normal
            # turns, while the compact cap bevel-limits acute turns.
            averaged_side = safe_normalized(
                sum(side_clusters, Vector((0.0, 0.0, 0.0))),
                side_clusters[0],
            )
            alignments = [
                averaged_side.dot(side)
                for side in sides
            ]
            weakest_alignment = min(alignments, default=1.0)
            requested_miter_distance = (
                face_width / weakest_alignment
                if weakest_alignment > 1.0e-4
                else float("inf")
            )
            hard_normal_transition = (
                owned_surface is not None
                and
                len(normals) == 2
            ...
            if hard_normal_transition:
                ...
                hard_edge_candidate = _intersection_project_miter_to_shared_edge(
                    owned_surface,
                    first_segment[f"triangle_{slot}_index"],
                    second_segment[f"triangle_{slot}_index"],
                    free_outer_point,
                )
                if (
                    hard_edge_candidate is not None
                    and _intersection_surface_miter_is_valid(
                        origin,
                        hard_edge_candidate,
                        sides,
                        normals,
            ...
            if (
                requested_miter_distance > maximum_miter_distance
                ...
                (
                    ...
                    capped_normal,
                    _capped_triangle,
                ) = _intersection_project_to_surface(
                    owned_surface,
                    free_outer_point,
                    0.0,
                )
                if _intersection_surface_miter_is_valid(
                    origin,
                    capped_candidate,
                    sides,
                    normals,
            ...
            if (
                ...
                # The surface-aware path keeps one outer vertex at every
                # ordinary corner.  Prefer the common hard edge when it is
                # available; otherwise use the bounded free-space miter only
                # as a query point and snap it back to this rail's source.
                # This restores the clean quad topology of the earlier solver
                # without allowing the corner vertex to float in space.
                if hard_edge_candidate is not None:
                    capped_surface_miter = hard_edge_candidate
                    capped_surface_normal = safe_normalized(sum(
                        normals,
                        Vector((0.0, 0.0, 0.0)),
                    ))
                else:
                    (
                        capped_surface_miter,
                        capped_surface_normal,
                        _capped_triangle,
                    ) = _intersection_project_to_surface(
                        owned_surface,
                        free_outer_point,
            ...
            if not use_surface_join:
                if hard_edge_miter is not None:
                    outer_point = offset_point_from_face_normals(
                        hard_edge_miter,
                        normals,
                        surface_offset,
                    )
                    ...
                    outer_point, _outer_normal, _triangle_index = (
                        _intersection_project_to_surface(
                            owned_surface,
                            free_outer_point,
                            surface_offset,
                ...
                for segment_index, _endpoint, group_slot in rail_group:
                    outer_by_node_incident_slot[
                        (node_key, segment_index, group_slot)
                    ] = outer_index
                continue

            # A hard source transition cannot be represented by one averaged
            # 3D vertex without leaving the source mesh.  Trace one exact-width
            # point on each supporting surface triangle, then meet the two
            # points on their real shared edge.  This produces two small
            # source-planar join triangles rather than one floating wedge.
            join_outer_indices = []
            join_normals = []
            ...
            for segment_index, _endpoint, group_slot in rail_group:
                ...
                if owned_surface is not None and triangle_index is not None:
                    surface_point, traced_normal, traced_triangle = (
                        _intersection_walk_surface(
                            owned_surface,
                            origin,
                            side,
                            face_width,
            ...
            if (
                len(join_outer_indices) == 2
                and (
                    vertices[join_outer_indices[0]]
                    - vertices[join_outer_indices[1]]
                ).length <= max(vertex_tolerance * 4.0, face_width * 1.0e-4)
            ):
                shared_outer_index = join_outer_indices[0]
                second_segment_index, _second_endpoint, second_slot = rail_group[1]
                outer_by_node_incident_slot[
                    (node_key, second_segment_index, second_slot)
                ] = shared_outer_index
            ...
            if len(join_outer_indices) == 2:
                edge_point = _intersection_hard_join_edge_point(
                    owned_surface,
                    join_triangle_indices[0],
                    join_triangle_indices[1],
                    join_surface_points[0],
                    join_surface_points[1],
                )
                if (
                    edge_point is not None
                    and (edge_point - origin).length > vertex_tolerance
                    and all(
                        (edge_point - point).length > vertex_tolerance
                        for point in join_surface_points
                    )
                ):
                    edge_outer_point = offset_point_from_face_normals(
                        edge_point,
                        join_normals,
                        surface_offset,
                    )
            ...
            minimum_join_area = max(
                vertex_tolerance * vertex_tolerance,
                face_width * face_width * 1.0e-6,
            )
            if len(join_outer_indices) == 3:
                center_index = center_by_node[node_key]
                first_area = (
                    vertices[join_outer_indices[0]] - vertices[center_index]
                ).cross(
                    vertices[join_outer_indices[1]] - vertices[center_index]
                ).length
                second_area = (
                    vertices[join_outer_indices[1]] - vertices[center_index]
                ).cross(
                    vertices[join_outer_indices[2]] - vertices[center_index]
                ).length
                if first_area <= minimum_join_area:
                    first_segment_index, _first_endpoint, first_slot = rail_group[0]
                    outer_by_node_incident_slot[
                        (node_key, first_segment_index, first_slot)
                    ] = join_outer_indices[1]
                    join_outer_indices.pop(0)
                if (
                    len(join_outer_indices) == 3
            ...
            if len(join_outer_indices) == 2:
                ...
                join_area = (
                    vertices[first_outer_index]
                    - vertices[center_by_node[node_key]]
                ).cross(
                    vertices[second_outer_index]
                    - vertices[center_by_node[node_key]]
                ).length
                if join_area <= minimum_join_area:
                    fallback_surface_point = hard_edge_candidate
                    fallback_normal = safe_normalized(sum(
                        join_normals,
                        Vector((0.0, 0.0, 0.0)),
                    ))
                    if fallback_surface_point is None and owned_surface is not None:
                        (
                            fallback_surface_point,
                            fallback_normal,
                            _fallback_triangle,
                        ) = _intersection_project_to_surface(
                            owned_surface,
                    ...
                    if fallback_surface_point is not None:
                        vertices[first_outer_index] = (
                            fallback_surface_point
                            + fallback_normal * surface_offset
                        )
                        second_segment_index, _second_endpoint, second_slot = (
                            rail_group[1]
                        )
                        outer_by_node_incident_slot[
                            (node_key, second_segment_index, second_slot)
            ...
            if len(join_outer_indices) >= 2:
                bevel_join_specs.append({
                    "center": center_by_node[node_key],
                    "outer_chain": tuple(join_outer_indices),
                    "normals": tuple(join_normals),
                    "slot": slot,
                })
    ...
    if seam_edges_out is not None:
        for node_key in closed_component_seam_nodes:
            ...
            for slot in ("a", "b"):
                seam_edges_out.append((
                    center_index,
                    outer_by_node_incident_slot[
                        (node_key, segment_index, slot)
                    ],
                ))
    ...
    for segment_index, segment in enumerate(prepared_segments):
        ...
        for side_index, slot in enumerate(("a", "b")):
            ...
            outer_start_index = outer_by_node_incident_slot[
                (segment["start_key"], segment_index, slot)
            ]
            outer_end_index = outer_by_node_incident_slot[
                (segment["end_key"], segment_index, slot)
            ]
            outer_v = 0.0 if side_index == 0 else 1.0
            face_parts = _intersection_oriented_face_parts(
                (
                    center_start_index,
                    center_end_index,
                    outer_end_index,
    ...
    for join_spec in bevel_join_specs:
        ...
        for chain_index, (first_outer, second_outer) in enumerate(zip(
            outer_chain,
            outer_chain[1:],
        )):
            triangle_normal = (
                vertices[first_outer] - vertices[center_index]
            ).cross(
                vertices[second_outer] - vertices[center_index]
            )
            ...
            if len(outer_chain) == 3 and len(normals) == 2:
                ...
                target_normal = safe_normalized(
                    sum(normals, Vector((0.0, 0.0, 0.0))),
                    triangle_normal,
                )
            u_start = (
                chain_index
            ...
            indices, uvs = oriented_face_with_uvs(
                (center_index, first_outer, second_outer),
                (
                    (0.0, 0.5),
                    (u_start, outer_v),
                    (u_end, outer_v),
    ...
    # Corner fallbacks can supersede an incident rail vertex with a shared
    # surface miter.  Compact those now-unused construction vertices so the
    # delivered decal has no loose or overlapping topology.
    ...
    if len(used_vertex_indices) != len(vertices):
        ...
        if seam_edges_out is not None:
            seam_edges_out[:] = [
                (compact_index[first], compact_index[second])
                for first, second in seam_edges_out
                if (
                    first in compact_index
...
def process_intersection_decal_uvs(
    context,
    source_obj,
    decal_obj,
    layer_settings,
):
    """Run the same UV and pin pipeline used by selected-edge generation."""
    ...
    use_auto_uv_pins = bool(
        scene_settings.auto_use_uv_pins
    ...
    if (
        (scene_settings.auto_unwrap_uvs or force_full_uv_for_pins)
        and not fast_geometry_only
    ):
        (
            quadrify_result,
            ...
            source_obj,
            decal_obj,
            scene_settings.use_integrated_quadrify,
            scene_settings.integrated_quadrify_average_shape,
            scene_settings.integrated_quadrify_even_shape,
            scene_settings.use_follow_active_quads,
            layer_settings.uv_scale,
            scene_settings.set_target_texel_density,
            scene_settings.target_texel_density,
            scene_settings.texture_resolution,
            context.scene.unit_settings.scale_length,
            scene_settings.generate_second_uv,
            scene_settings.average_uv_island_scale,
            scene_settings.align_uvs_horizontally,
            scene_settings.place_in_quarter_strips,
            scene_settings.randomize_quarter_strip,
            False,
            scene_settings.horizontal_randomize_amount,
            layer_settings.seed,
            scene_settings.uv_strip_padding,
        )
    ...
    decal_obj["edge_decal_last_uv_signature"] = decal_uv_settings_signature(
        layer_settings
    )
...
def _intersection_source_objects(layer_obj):
    source_a = find_object_by_name_or_full(
        layer_obj.get("edge_decal_intersection_source_a", "")
    )
    source_b = find_object_by_name_or_full(
        layer_obj.get("edge_decal_intersection_source_b", "")
    )
    return source_a, source_b


def _intersection_face_width(settings, source_obj, other_obj, reference_size):
    """Resolve one stable random width for a generated intersection curve."""
    ...
    minimum = max(
        float(getattr(settings, "minimum_face_width", MIN_FACE_WIDTH)),
        MIN_FACE_WIDTH,
    )
    maximum = max(
        float(getattr(settings, "maximum_face_width", MIN_FACE_WIDTH)),
        MIN_FACE_WIDTH,
    )
    if relative:
    ...
    signature = int.from_bytes(
        hashlib.blake2b(names.encode("utf-8"), digest_size=8).digest(),
        "big",
    )
    return randomized_face_width(
        minimum,
        maximum,
        int(getattr(settings, "seed", 0)),
        signature,
    )


def _write_intersection_decal_geometry(
    context,
    layer_obj,
    source_obj,
    other_obj,
    ...
    reference_size = max(
        source_mesh_max_dimension(source_obj),
        source_mesh_max_dimension(other_obj),
    )
    face_width = _intersection_face_width(
        settings,
        source_obj,
        other_obj,
        reference_size,
    ...
    segments, surface_a, surface_b = mesh_intersection_data(
        source_obj,
        other_obj,
    ...
    seam_edges = []
    vertices, faces, face_uvs, center_indices = build_intersection_decal_mesh(
        segments,
        face_width,
        max(0.0, float(settings.surface_offset)),
        settings.uv_scale,
        getattr(context.scene.edge_decal_settings, "miter_limit", 4.0),
        seam_edges_out=seam_edges,
        surface_a=surface_a,
        surface_b=surface_b,
    )
    if not faces:
...
def regenerate_intersection_decal(context, layer_obj, operator=None):
    ...
    if source_obj is None or other_obj is None:
        if operator is not None:
            operator.report({"ERROR"}, "An intersection source object is missing")
        return {"CANCELLED"}
    data = layer_obj.edge_decal_object_settings
    success, result = _write_intersection_decal_geometry(
        context,
        layer_obj,
        source_obj,
        other_obj,
    ...
    if not success:
        if operator is not None:
            operator.report({"WARNING"}, result)
        return {"CANCELLED"}
    ...
    process_intersection_decal_uvs(
        context,
        source_obj,
        layer_obj,
        data,
    ...
    if operator is not None:
        operator.report({"INFO"}, f"Updated intersection decal from {result} segment(s)")
    return {"FINISHED"}
...
class EDGEDECAL_OT_generate_intersections(Operator):
    bl_idname = "object.generate_edge_decals_at_intersections"
    bl_label = "Generate Intersections"
    bl_description = (
        "Generate a wrapped edge decal where exactly two selected mesh objects intersect"
    )
    ...
    def execute(self, context):
        ...
        if not segments:
            self.report({"WARNING"}, "The selected meshes do not intersect")
            return {"CANCELLED"}
        ...
        vertices, faces, face_uvs, center_indices = build_intersection_decal_mesh(
            ...
            max(0.0, float(settings.surface_offset)),
            settings.uv_scale,
            settings.miter_limit,
            seam_edges_out=seam_edges,
            surface_a=surface_a,
            surface_b=surface_b,
        )
        if not faces:
            self.report({"WARNING"}, "No usable intersection curve was found")
            return {"CANCELLED"}

        ensure_source_decal_layers_ready(source_obj, context)
        layer_obj = find_generation_shell_layer(
            source_obj,
            context=context,
        ...
        store_decal_settings(
            ...
            settings,
            {
                ...
                "relative_face_width": settings.relative_face_width,
        ...
        process_intersection_decal_uvs(
            ...
            source_obj,
            layer_obj,
            layer_obj.edge_decal_object_settings,
        )
        sync_source_layer_ui(source_obj, active_layer=layer_obj)
        finish_decal_generation(context, source_obj, layer_obj)
        self.report(
            {"INFO"},
            f"Generated intersection decal from {len(segments)} segment(s)",
        )
        return {"FINISHED"}


def _existing_boolean_decal_layer(
    source_obj,
    context=None,
):
    """Find the combined Boolean layer owned by this source."""
    matches = [
        decal_obj
        for decal_obj in iter_generated_decals(
...
def _remove_boolean_temporary_datablocks(objects, meshes, materials):
    """Remove private Boolean-evaluation datablocks without touching inputs."""
    for obj in objects:
...
def build_boolean_decal_mesh(
    context,
    source_obj,
    cutter_obj,
    settings,
    source_boolean_modifier=None,
):
    """Build a decal from Boolean seam edges on private mesh copies.

    The retained source Boolean is omitted while evaluating the source's other
    modifiers, then recreated only on the private evaluation copy. Temporary
    face materials distinguish original source faces from cutter-created faces,
    making affected seam edges deterministic without applying the live Boolean.
    """
    temporary_objects = []
    temporary_meshes = []
    temporary_materials = []
    ...
    try:
        if cutter_obj.mode == "EDIT":
            # Flush the current edit BMesh into the object datablock without
            # leaving Edit Mode so evaluated modifiers see the latest vertices.
        ...
        if source_boolean_modifier is not None:
            ...
            copied_boolean = source_pre_boolean.modifiers.get(
                source_boolean_modifier.name
            )
        ...
        source_mesh = bpy.data.meshes.new_from_object(
            source_evaluation_object.evaluated_get(depsgraph),
            preserve_all_data_layers=True,
            depsgraph=depsgraph,
        )
        cutter_mesh = bpy.data.meshes.new_from_object(
            cutter_obj.evaluated_get(depsgraph),
            preserve_all_data_layers=True,
            depsgraph=depsgraph,
        )
        temporary_meshes.extend((source_mesh, cutter_mesh))

        source_temp = bpy.data.objects.new(
            "__EdgeDecalBooleanSource", source_mesh
        )
        cutter_temp = bpy.data.objects.new(
            "__EdgeDecalBooleanCutter", cutter_mesh
        )
        ...
        temporary_boolean = source_temp.modifiers.new(
            name="Edge Decal Temporary Difference",
            type="BOOLEAN",
        )
        temporary_boolean.operation = (
            source_boolean_modifier.operation
            if source_boolean_modifier is not None
            else "DIFFERENCE"
        )
        temporary_boolean.solver = (
            source_boolean_modifier.solver
            if source_boolean_modifier is not None
            else "EXACT"
        )
        ...
        for property_name in (
            ...
            if (
                source_boolean_modifier is not None
                ...
                setattr(
                    temporary_boolean,
                    property_name,
                    getattr(source_boolean_modifier, property_name),
                )

        context.view_layer.update()
        result_depsgraph = context.evaluated_depsgraph_get()
        result_mesh = bpy.data.meshes.new_from_object(
            source_temp.evaluated_get(result_depsgraph),
            preserve_all_data_layers=True,
            depsgraph=result_depsgraph,
        )
        ...
        if bool(getattr(settings, "randomize_face_width", False)):
            random_width_minimum = max(
                float(getattr(settings, "minimum_face_width", MIN_FACE_WIDTH)),
                MIN_FACE_WIDTH,
            )
            random_width_maximum = max(
                float(getattr(settings, "maximum_face_width", MIN_FACE_WIDTH)),
                MIN_FACE_WIDTH,
            )
            if bool(getattr(settings, "relative_face_width", True)):
            ...
            random_width_minimum, random_width_maximum = (
                min(random_width_minimum, random_width_maximum),
                max(random_width_minimum, random_width_maximum),
            )
        scene_settings = context.scene.edge_decal_settings
        ...
        width_search_context = build_adaptive_width_search_context(
            ...
            selected_edge_set,
            world_matrix,
            clamp_boundaries=settings.auto_face_width,
            clamp_selected_overlaps=settings.clamp_edge_overlaps,
            search_radius=max(random_width_maximum * 4.0, 1.0e-4),
        )
        edge_groups = partition_selected_edge_graph_by_angle(
            affected_edges,
            world_matrix,
            (
                scene_settings.split_angle
                if scene_settings.use_edge_split
                else pi
            ),
        )
        ...
        created_face_count = build_partitioned_selected_edge_graph_strip(
            selected_edges=affected_edges,
            edge_groups=edge_groups,
            all_source_edges=all_source_edges,
            world_matrix=world_matrix,
            normal_matrix=normal_matrix,
            face_width=resolved_face_width,
            surface_offset=max(0.0, float(settings.surface_offset)),
            miter_limit=scene_settings.miter_limit,
            vertices_out=vertices,
            faces_out=faces,
            face_uvs_out=face_uvs,
            center_vertices_out=center_indices,
        ...
        return (
            ...
            faces,
            face_uvs,
            sorted(set(center_indices)),
            len(affected_edges),
        ), None
    finally:
        if result_bm is not None:
            result_bm.free()
        _remove_boolean_temporary_datablocks(
            temporary_objects,
            temporary_meshes,
            temporary_materials,
        )
...
def _enabled_source_boolean_modifiers(source_obj):
    """Return enabled, mesh Object-operand Booleans in source stack order."""
...
def _snap_zero_offset_boolean_vertices_to_surface(
    vertices,
    result_bm,
    world_matrix,
):
    """Constrain zero-offset Boolean decal geometry to its evaluated result."""
...
def build_combined_boolean_decal_mesh(
    context,
    source_obj,
    boolean_modifiers,
    settings,
):
    """Build one decal from all surviving seams in a Boolean modifier stack."""
    ...
    try:
        ...
        markers = [
            bpy.data.materials.new("__EdgeDecalBooleanOriginal")
        ] + [
            bpy.data.materials.new(
                f"__EdgeDecalBooleanAffected_{index:02d}"
            )
            for index in range(1, len(boolean_modifiers) + 1)
        ]
        ...
        modifier_by_name = {
            modifier.name: (index, modifier)
            for index, modifier in enumerate(boolean_modifiers, start=1)
        }
        for copied_modifier in source_temp.modifiers:
            ...
            if record is None:
                # Disabled and unsupported Collection operands are deliberately
                # outside this combined decal and must not alter its snapshot.
            ...
            cutter_temp = bpy.data.objects.new(
                f"__EdgeDecalBooleanCutter_{marker_index:02d}",
                cutter_mesh,
            )
            context.scene.collection.objects.link(cutter_temp)
            cutter_temp.matrix_world = cutter_obj.matrix_world.copy()
        ...
        affected_edges = [
            ...
            if (
                len(edge.link_faces) == 2
                and len({face.material_index for face in edge.link_faces}) > 1
                and any(face.material_index > 0 for face in edge.link_faces)
            )
        ]
        if not affected_edges:
        ...
        edge_groups = partition_selected_edge_graph_by_angle(
            affected_edges,
            world_matrix,
            scene_settings.split_angle if scene_settings.use_edge_split else pi,
        )
        face_width_by_edge = None
        if bool(getattr(settings, "randomize_face_width", False)):
            face_width_by_edge = randomized_face_widths_for_edge_groups(
...
def _store_combined_boolean_metadata(
    layer_obj,
    source_obj,
    boolean_modifiers,
    affected_edge_count,
    ...
    layer_obj["edge_decal_boolean_modifiers"] = json.dumps(
        [modifier.name for modifier in boolean_modifiers]
    )
    layer_obj["edge_decal_boolean_cutters"] = json.dumps(
        [modifier.object.name_full for modifier in boolean_modifiers]
    )
    ...
    for legacy_key in (
        "edge_decal_boolean_cutter",
        "edge_decal_boolean_modifier",
    ):
        if legacy_key in layer_obj:
            del layer_obj[legacy_key]


def _write_combined_boolean_decal_geometry(
    context,
    layer_obj,
    source_obj,
    boolean_modifiers,
    ...
    result, error = build_combined_boolean_decal_mesh(
        context,
        source_obj,
        boolean_modifiers,
        settings,
    ...
    if center_indices:
        center_group.add(center_indices, 1.0, "REPLACE")
    _store_combined_boolean_metadata(
        layer_obj,
        source_obj,
        boolean_modifiers,
        affected_edge_count,
...
def regenerate_boolean_decal(
    ...
    source_obj = find_object_by_name_or_full(
        layer_obj.get(
            ...
            layer_obj.get("edge_decal_source", ""),
        )
    )
    if source_obj is None:
        if operator is not None:
            operator.report({"ERROR"}, "The Boolean source object is missing")
        return {"CANCELLED"}
    ...
    if not boolean_modifiers:
        if operator is not None:
            operator.report(
                {"ERROR"},
                "The source has no enabled Object Boolean modifiers",
            )
        return {"CANCELLED"}

    data = layer_obj.edge_decal_object_settings
    success, result = _write_combined_boolean_decal_geometry(
        context,
        layer_obj,
        source_obj,
        boolean_modifiers,
    ...
    if operator is not None:
        operator.report(
            {"INFO"},
            f"Updated combined Boolean decal from {result} seam edge(s)",
        )
    return {"FINISHED"}


class EDGEDECAL_OT_generate_boolean(Operator):
    bl_idname = "object.generate_edge_decals_from_boolean"
    bl_label = "Generate From Booleans"
    bl_description = (
        "Generate one combined decal from all enabled Object Boolean "
        "modifiers already present on the selected source mesh"
    )
    bl_options = {"REGISTER", "UNDO"}
    ...
    def execute(self, context):
        ...
        if len(selected) != 1 or active not in selected:
            self.report(
                {"ERROR"},
                "Select exactly one source mesh",
            )
            return {"CANCELLED"}
        ...
        existing_layer = _existing_boolean_decal_layer(
            source_obj,
            context=context,
        )
        if existing_layer is not None:
            if existing_layer.get("edge_decal_locked", False):
                self.report(
                    {"WARNING"},
                    "The matching Boolean decal layer is locked",
                )
                return {"CANCELLED"}
            return regenerate_boolean_decal(
                context,
                existing_layer,
                operator=self,
            )
        ...
        self.report(
            {"INFO"},
            f"Generated combined Boolean decal from {affected_edge_count} seam edge(s)",
        )
        return {"FINISHED"}
