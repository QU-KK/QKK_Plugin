...
class EDGEDECAL_OT_remove_decal_sections(Operator):
    bl_idname = "object.edge_decal_remove_sections"
    bl_label = "Remove Decal Sections"
    bl_description = (
        "Enter an interactive mode with one red target per disconnected decal "
        "section; click a target to delete that section"
    )
    ...
    def _collect_components(self, obj):
        ...
        for polygon in mesh.polygons:
            for vertex_index in polygon.vertices:
                vertex_to_polygons.setdefault(vertex_index, []).append(
                    polygon.index
                )
    ...
    def _draw_overlay(self, context):
        ...
        for component_index, point_2d in self._projected_targets(context):
            ...
            self._draw_target(
                shader,
                point_2d,
                self._target_radius + (3.0 if hovered else 0.0),
    ...
    def _undo_last_remove(self, context, obj):
        if not self._undo_stack:
            self.report({"INFO"}, "Nothing to undo in Remove Decal Sections")
            return False
        backup = self._undo_stack.pop()
        if not self._restore_mesh_backup(backup):
            self.report({"WARNING"}, "Could not restore the removed decal section")
            return False
        self._components = self._collect_components(obj)
        self._hovered_index = -1
        if context.area:
            context.area.tag_redraw()
        self.report({"INFO"}, "Restored last removed decal section")
        return True

    def _finish(self, context):
        """Always remove the viewport overlay, even if cleanup fails."""
        ...
        if draw_handle is not None:
            try:
                ...
                # The area or handler may already have been destroyed.
                pass
    ...
    def invoke(self, context, event):
        ...
        if not self._components:
            self.report({"WARNING"}, "The selected decal has no removable sections.")
            return {"CANCELLED"}
        ...
        self.report(
            {"INFO"},
            "Click a red target to remove a decal section; Enter or Right Click finishes; Esc exits",
        )
        ...
        if event.type == "LEFTMOUSE" and event.value == "PRESS":
            ...
            if 0 <= self._hovered_index < len(self._components):
                ...
                if self._remove_component(obj, component):
                    ...
                    if not self._components:
                        self._finish(context)
                        self.report({"INFO"}, "All decal sections were removed.")
                        return {"FINISHED"}
...
class EDGEDECAL_OT_lock_current_decals(Operator):
    bl_idname = "object.edge_decal_lock_current"
    bl_label = "Lock Current Decals"
    bl_description = (
        "Lock all current decals generated from the selected source mesh so future generation keeps them untouched"
    )
    ...
    def execute(self, context):
        ...
        if locked_count == 0:
            self.report({"INFO"}, "All current decals are already locked")
            return {"FINISHED"}

        self.report({"INFO"}, f"Locked {locked_count} decal object(s)")
        return {"FINISHED"}
...
def _legacy_decal_layer_has_split_path_seams(layer_obj):
    """Infer pre-persistence Split Edge Paths from coincident island seams."""
    ...
    for coordinate in coordinates:
        key = tuple(
            int(round(float(component) * inverse_tolerance))
...
def sync_scene_settings_from_decal_layer(context, source_obj, layer_obj):
    """Mirror one decal layer's settings into the scene without changing selection."""
    ...
    try:
        if data is not None and getattr(data, "initialized", False):
            if not data.is_property_set("use_edge_split"):
                data.use_edge_split = _legacy_decal_layer_has_split_path_seams(
                    layer_obj
                )
                data.split_angle = float(
                    getattr(settings, "split_angle", radians(45.0))
                )
            ...
            if hasattr(data, "decal_material"):
                ...
                match_source_material = bool(
                    ...
                    or (
                        data.decal_material is not None
                        and data.decal_material.get("edge_decal_source_matched")
                    )
                )
                settings.match_source_material = match_source_material
                # Older files predate the explicit toggle. Their assigned
                # derived material is the durable evidence that matching was
                # enabled, so persist that state the first time the layer is
                # synchronized.
    ...
    EDGEDECAL_SCENE_LIVE_SYNC_CACHE[
        scene_live_sync_cache_key(source_obj, layer_obj)
    ] = scene_live_edit_signature(settings)
    return True


def activate_decal_layer(
    context,
    source_obj,
    layer_obj,
    select_source=True,
    ...
    """Make one decal layer active and mirror its settings into the scene."""
    if source_obj is None or layer_obj is None:
        return False

    if (
        layer_obj.data is not None
...
def repair_decal_layers_for_source(source_obj, context=None, activate=True):
    """Reconcile active-layer and UIList state after layer deletes or orphan pointers."""
    if (
        source_obj is None
...
def source_mesh_for_removed_decal(decal_name, registry_source_name=""):
    """Find the source mesh that owned a decal object removed from the scene."""
...
def schedule_source_layer_ui_sync(source_obj):
    """Refresh the UIList collection outside the panel draw callback."""
...
def sync_source_layer_ui(source_obj, active_layer=None):
    """Rebuild the UIList collection from decal objects on this source."""
...
def update_edge_decal_layer_index(self, context):
    """UIList selection changed — activate the chosen decal layer."""
    global EDGEDECAL_LAYER_UI_SYNCING
...
class EDGEDECAL_UL_layers(UIList):
    ...
    def draw_item(
        ...
        vis = row.operator(
            EDGEDECAL_OT_layer_toggle_visibility.bl_idname,
            text="",
            icon="HIDE_ON" if layer_obj.hide_viewport else "HIDE_OFF",
            emboss=False,
        )
        ...
        lock = row.operator(
            EDGEDECAL_OT_layer_toggle_lock.bl_idname,
            text="",
        ...
        name_row.label(
            text=f"{display_name}  ({edge_count})",
            icon="RENDERLAYERS",
        )
        workflow_label, workflow_icon = decal_layer_workflow_label(layer_obj)
        row.label(text=workflow_label, icon=workflow_icon)
        pin_summary = layer_uv_pin_summary_label(
            layer_obj,
        ...
        if pin_summary:
            row.label(text=pin_summary, icon="PIVOT_CURSOR")


class EDGEDECAL_OT_layer_toggle_uv_pin(Operator):
    bl_idname = "object.edge_decal_layer_toggle_uv_pin"
    bl_label = "Toggle Layer UV Pin"
    bl_options = {"REGISTER", "UNDO"}
...
class EDGEDECAL_OT_layer_select(Operator):
    bl_idname = "object.edge_decal_layer_select"
    bl_label = "Select Decal Layer"
    bl_options = {"INTERNAL"}
...
def create_empty_decal_layer(context, source_obj, layer_index=None):
    """Create and register a new empty decal layer object for one source mesh."""
    if source_obj is None or source_obj.type != "MESH":
        return None
    ...
    try:
        # Every new blank layer starts with independent split paths enabled,
        # regardless of the previously active layer's setting.
...
class EDGEDECAL_OT_layer_add(Operator):
    bl_idname = "object.edge_decal_layer_add"
    bl_label = "Add Decal Layer"
    bl_description = "Create a new empty active decal layer for this source mesh"
    bl_options = {"REGISTER", "UNDO"}

    def execute(self, context):
        source_obj = edge_decal_context_source(context)
        if source_obj is None or source_obj.type != "MESH":
            self.report({"ERROR"}, "Select a source mesh")
            return {"CANCELLED"}
...
class EDGEDECAL_OT_layer_delete(Operator):
    bl_idname = "object.edge_decal_layer_delete"
    bl_label = "Delete Decal Layer"
    bl_options = {"REGISTER", "UNDO"}
...
class EDGEDECAL_OT_layer_delete_active(Operator):
    bl_idname = "object.edge_decal_layer_delete_active"
    bl_label = "Delete Active Decal Layer"
    bl_description = "Delete the selected decal layer"
    bl_options = {"REGISTER", "UNDO"}
    ...
    def execute(self, context):
        ...
        bpy.ops.object.edge_decal_layer_delete(
            "EXEC_DEFAULT",
            layer_name=layer_obj.name_full,
        )
        return {"FINISHED"}


class EDGEDECAL_OT_layer_move(Operator):
    bl_idname = "object.edge_decal_layer_move"
    bl_label = "Move Decal Layer"
    bl_description = "Change the stack order of decal layers"
    bl_options = {"REGISTER", "UNDO"}

    direction: EnumProperty(
        name="Direction",
        items=(
            ("UP", "Up", "Move layer up in the stack"),
            ("DOWN", "Down", "Move layer down in the stack"),
        ),
        options={"HIDDEN"},
    )
...
def _decal_layer_mask_cutter_thickness(source_obj, target_layer, mask_layer):
    """Return a shallow, scale-aware depth spanning both decal surfaces."""
    source_extent = max(
        (abs(float(value)) for value in source_obj.dimensions),
        default=1.0,
    )
...
def _evaluated_decal_mask_volume(context, mask_layer, thickness):
    """Evaluate one decal without Bevel and solidify it into a cutter."""
    ...
    try:
        ...
        surface_mesh = bpy.data.meshes.new_from_object(
            evaluated,
    ...
    try:
        solidify = surface_obj.modifiers.new(
            "__EdgeDecalLayerMaskThickness",
            "SOLIDIFY",
        )
        ...
        volume_mesh = bpy.data.meshes.new_from_object(
            evaluated_volume,
        ...
        volume_obj = bpy.data.objects.new(
            "__EdgeDecalLayerMaskVolume",
            volume_mesh,
        )
...
def _extract_original_decal_surface(result_mesh, original_mesh, tolerance):
    """Discard temporary shell/rim faces while preserving Boolean-cut caps."""
    if result_mesh is None or original_mesh is None:
        return False

    original_bvh = BVHTree.FromPolygons(
        [vertex.co.copy() for vertex in original_mesh.vertices],
        [tuple(polygon.vertices) for polygon in original_mesh.polygons],
        all_triangles=False,
    )
    keep_indices = []
    for polygon in result_mesh.polygons:
        ...
        for vertex_index in polygon.vertices:
            nearest = original_bvh.find_nearest(
                result_mesh.vertices[vertex_index].co
            )
...
def _decal_mesh_polygon_components(mesh):
    """Return face-to-island ownership without merging coincident vertices."""
    vertex_faces = {}
    for polygon in mesh.polygons:
        for vertex_index in polygon.vertices:
...
def _copy_decal_component_mesh(original_mesh, polygon_indices, name):
    """Copy one disconnected face island with all mesh data layers intact."""
...
def _combine_decal_component_meshes(component_meshes, original_mesh):
    """Append independently cut islands without welding coincident seams."""
    combined_vertices = []
    combined_faces = []
    combined_material_indices = []
    combined_smooth_flags = []
    uv_layer_count = max(
        (len(mesh.uv_layers) for mesh in component_meshes),
        default=0,
    )
    uv_layer_names = []
    ...
    for component_mesh in component_meshes:
        ...
        for polygon in component_mesh.polygons:
            ...
            for layer_index in range(uv_layer_count):
                uv_layer = (
                    component_mesh.uv_layers[layer_index]
                    if layer_index < len(component_mesh.uv_layers)
                    else None
                )
                combined_uvs[layer_index].append([
                    (
    ...
    combined_mesh = bpy.data.meshes.new(
        f"{original_mesh.name}_LayerMaskCombined"
    )
    ...
    for component_mesh in component_meshes:
        for vertex in component_mesh.vertices:
            for assignment in vertex.groups:
                weights_by_group.setdefault(assignment.group, []).append((
                    vertex_offset + vertex.index,
...
def _restore_decal_vertex_group_weights(
    target_layer,
    group_names,
    weights_by_group,
):
    """Restore deform weights lost while component meshes are appended."""
    ...
    for group_index, assignments in weights_by_group.items():
        ...
        for vertex_index, weight in assignments:
            indices_by_weight.setdefault(round(weight, 6), []).append(
                vertex_index
            )
...
def _boolean_mask_decal_component(
    context,
    target_layer,
    component_mesh,
    cutter_obj,
    ...
    result_mesh = None
    try:
        target_shell = target_obj.modifiers.new(
            "__EdgeDecalLayerMaskTargetShell",
            "SOLIDIFY",
        )
        ...
        boolean = target_obj.modifiers.new(
            "__EdgeDecalLayerMaskDifference",
            "BOOLEAN",
        )
        ...
        if result_mesh.polygons and not _extract_original_decal_surface(
            result_mesh,
            component_mesh,
            surface_tolerance,
        ):
...
def apply_decal_layer_mask(
    context,
    source_obj,
    target_layer,
    mask_layer,
):
    """Subtract one upper decal footprint from one selected lower layer."""
    ...
    vertex_group_names = [
        group.name
        for group in sorted(
            target_layer.vertex_groups,
            key=lambda group: group.index,
        )
    ]
    component_by_polygon, component_count = _decal_mesh_polygon_components(
        original_mesh
    )
    polygons_by_component = {
        component_index: []
    ...
    try:
        thickness = _decal_layer_mask_cutter_thickness(
            source_obj,
            target_layer,
            mask_layer,
        )
        cutter_obj = _evaluated_decal_mask_volume(
            context,
            mask_layer,
            thickness * 4.0,
        )
        ...
        for component_index in range(component_count):
            component_mesh = _copy_decal_component_mesh(
                original_mesh,
                polygons_by_component[component_index],
                f"{original_mesh.name}_MaskIsland_{component_index:04d}",
            )
            component_meshes.append(component_mesh)
            result_mesh, error = _boolean_mask_decal_component(
                context,
                target_layer,
                component_mesh,
                cutter_obj,
        ...
        vertex_group_weights = _decal_component_vertex_group_weights(
            component_results
        )
        if len(component_results) == 1:
            final_result_mesh = component_results.pop()
        else:
            final_result_mesh = _combine_decal_component_meshes(
                component_results,
                original_mesh,
            )

        result_area = sum(
            polygon.area for polygon in final_result_mesh.polygons
        )
        ...
        _restore_decal_vertex_group_weights(
            target_layer,
            vertex_group_names,
            vertex_group_weights,
        )
...
class EDGEDECAL_OT_layer_apply_mask(Operator):
    bl_idname = "object.edge_decal_layer_apply_mask"
    bl_label = "Regenerate with Layer Mask"
    bl_description = (
        "Regenerate only the selected layer, then remove the footprint of its "
        "chosen upper mask layer"
    )
    bl_options = {"REGISTER", "UNDO"}
    ...
    def execute(self, context):
        ...
        if target_layer is None or mask_layer is None:
            self.report({"WARNING"}, "Choose a mask layer first")
            return {"CANCELLED"}
        if target_layer.get("edge_decal_locked", False):
            self.report({"WARNING"}, "Unlock the selected decal layer first")
            return {"CANCELLED"}
        if not _decal_layer_mask_is_above(source_obj, target_layer, mask_layer):
            self.report({"WARNING"}, "The mask layer must be above the selected layer")
            return {"CANCELLED"}
        ...
        success, message = apply_decal_layer_mask(
            context,
            source_obj,
            target_layer,
            mask_layer,
...
class EDGEDECAL_OT_layer_clear_mask(Operator):
    bl_idname = "object.edge_decal_layer_clear_mask"
    bl_label = "Clear Layer Mask"
    bl_description = "Remove the mask assignment and regenerate the full selected layer"
    bl_options = {"REGISTER", "UNDO"}
    ...
    def poll(cls, context):
        source_obj = edge_decal_context_source(context)
        target_layer = (
            active_decal_layer_for_source(source_obj, context=context)
            if source_obj is not None
            else None
        )
        data = getattr(target_layer, "edge_decal_object_settings", None)
    ...
    def execute(self, context):
        ...
        if not decal_has_regeneratable_source_data(target_layer):
            self.report(
                {"WARNING"},
                "This layer has no stored source data to restore from",
            )
            return {"CANCELLED"}
        ...
        if "FINISHED" not in result:
            ...
            return {"CANCELLED"}
        self.report({"INFO"}, "Cleared the layer mask and restored the full layer")
        return {"FINISHED"}


class EDGEDECAL_OT_layer_toggle_lock(Operator):
    bl_idname = "object.edge_decal_layer_toggle_lock"
    ...
    def execute(self, context):
        ...
        if layer_obj is None:
            return {"CANCELLED"}
        layer_obj["edge_decal_locked"] = not bool(
            layer_obj.get("edge_decal_locked", False)
        )
        return {"FINISHED"}


class EDGEDECAL_OT_layer_toggle_visibility(Operator):
    bl_idname = "object.edge_decal_layer_toggle_visibility"
...
def _draw_layer_uv_pin_toggles(box, context, layer_obj):
    ...
    header.label(text="UV Pins", icon="PIVOT_CURSOR")

    if not pins:
        pin_box.label(text="No UV pins defined yet", icon="INFO")
        pin_box.label(text="Add pins in the UV Editor panel")
        return

    assigned = layer_assigned_uv_pin_indices(layer_obj)
    if assigned:
        header.label(
            text=f"{len(assigned)} selected",
            icon="CHECKMARK",
        )
    else:
        header.label(text="Auto (all pins)", icon="LOOP_BACK")

    for index, pin in enumerate(pins):
        row = pin_box.row(align=True)
        enabled = layer_uv_pin_is_enabled(layer_obj, index)
        toggle = row.operator(
            EDGEDECAL_OT_layer_toggle_uv_pin.bl_idname,
            text="",
        ...
        row.label(
            text=(
                f"{uv_pin_display_name(pin, index)}   "
                f"U {pin.u:.3f}   V {pin.v:.3f}   W {pin.width:.3f}"
            )
        )
...
def _draw_decal_layer_details(box, context, active_layer):
    ...
    details.prop(
        active_layer.edge_decal_object_settings,
        "decal_template_material",
        text="Material",
    )
    if decal_layer_is_interactive_only(active_layer):
        details.label(
            text="Interactive-only layer: use Interactive Mode or add a new layer.",
            icon="INFO",
        )
...
def _draw_decal_layer_mask_controls(box, active_layer):
    ...
    header.label(text="Mask by Layer (Experimental)", icon="MOD_MASK")
    if active_layer is None:
        mask_box.label(text="Select the layer to regenerate", icon="INFO")
        return

    data = active_layer.edge_decal_object_settings
    mask_box.prop(data, "layer_mask", text="Mask Layer")

    actions = mask_box.row(align=True)
    actions.scale_y = 1.2
    apply_row = actions.row(align=True)
    apply_row.enabled = (
        data.layer_mask is not None
        and not active_layer.get("edge_decal_locked", False)
    )
    apply_row.operator(
        EDGEDECAL_OT_layer_apply_mask.bl_idname,
        text="Regenerate with Mask",
        icon="MOD_BOOLEAN",
    )
    if data.layer_mask is not None:
        actions.operator(
            EDGEDECAL_OT_layer_clear_mask.bl_idname,
            text="",
            icon="LOOP_BACK",
        )
...
def draw_masking_category(layout, context, settings, active_layer):
    ...
    texture_header.label(text="Layer Texture Mask", icon="IMAGE_DATA")

    if active_layer is None:
        texture_box.label(text="Select a decal layer", icon="INFO")
    else:
        mask_data = active_layer.edge_decal_object_settings
        mask_image = getattr(mask_data, "texture_mask", None)
        if mask_image is None:
            texture_box.label(text="Black blocks; white generates", icon="INFO")
            add_mask = texture_box.column()
            add_mask.scale_y = 1.25
            add_mask.operator(
                EDGEDECAL_OT_texture_mask_add.bl_idname,
                text="Add Black Mask",
                icon="ADD",
            )
        else:
            texture_header.prop(
                ...
                icon=(
                    "CHECKBOX_HLT"
                    if mask_data.use_texture_mask
                    else "CHECKBOX_DEHLT"
                ),
            )
            texture_box.template_ID(
                mask_data,
                "texture_mask",
                open="image.open",
            )
            ...
            controls.prop(mask_data, "texture_mask_threshold", slider=True)
            paint = controls.row(align=True)
            paint.scale_y = 1.25
            painting_mask = texture_mask_paint_is_active(
                context,
                active_layer,
            )
            paint.operator(
                EDGEDECAL_OT_texture_mask_paint.bl_idname,
                text="Exit Paint Mask" if painting_mask else "Paint Mask",
                icon="OBJECT_DATA" if painting_mask else "BRUSH_DATA",
                depress=painting_mask,
            )
            paint.operator(
                EDGEDECAL_OT_texture_mask_reset.bl_idname,
                text="Reset Black",
                icon="LOOP_BACK",
            )
            texture_box.operator(
                EDGEDECAL_OT_texture_mask_remove.bl_idname,
                text="Remove Mask",
                icon="UNLINKED",
            )

    crevice_box = body.box()
    crevice_box.label(text="Crevices Masking", icon="MOD_MASK")
    crevice_box.prop(settings, "crevice_removal", text="Amount", slider=True)
    crevice = crevice_box.column(align=True)
    crevice.enabled = settings.crevice_removal > 0.0
    crevice.prop(settings, "crevice_detection_mode", text="Detection")
    ao = crevice.column(align=True)
    ao.enabled = settings.crevice_detection_mode == "AO"
    ao.prop(settings, "crevice_ao_distance")
    ao.prop(settings, "crevice_ao_samples")


def _draw_decal_layers_legacy(box, context, layers, active_layer):
    if not layers:
        empty = box.column(align=True)
        empty.label(text="No decal layers", icon="INFO")
        empty.operator(
            EDGEDECAL_OT_layer_add.bl_idname,
            text="Create First Layer",
            icon="ADD",
        )
        return
    ...
    for layer_obj in layers:
        ...
        lock = row.operator(
            ...
            icon=(
                "LOCKED"
                if layer_obj.get("edge_decal_locked", False)
                else "UNLOCKED"
            ),
            emboss=False,
        )
        ...
        select = row.operator(
            EDGEDECAL_OT_layer_select.bl_idname,
            text=str(
                layer_obj.get(
                    "edge_decal_layer_name",
                    layer_obj.name,
        ...
        delete = row.operator(
            EDGEDECAL_OT_layer_delete.bl_idname,
            text="",
            icon="X",
            emboss=False,
        )
...
def draw_decal_layers(layout, context, source_obj, layers=None, active_layer=None):
    ...
    header.label(text="Decal Layers", icon="RENDERLAYERS")
    header.operator(
        EDGEDECAL_OT_layer_add.bl_idname,
        text="",
        icon="ADD",
    ...
    list_col.template_list(
        EDGEDECAL_UL_layers.bl_idname,
        "edgedecal_decal_layers",
        source_obj,
        "edge_decal_layers_ui",
        source_obj,
        "edge_decal_layer_index",
        rows=min(8, max(3, len(ui))),
    )

    controls = list_row.column(align=True)
    move_up = controls.operator(
        EDGEDECAL_OT_layer_move.bl_idname,
        text="",
        icon="TRIA_UP",
    )
    move_up.direction = "UP"
    move_down = controls.operator(
        EDGEDECAL_OT_layer_move.bl_idname,
        text="",
        icon="TRIA_DOWN",
    )
    ...
    controls.operator(
        EDGEDECAL_OT_layer_delete_active.bl_idname,
        text="",
        icon="REMOVE",
    )
...
class EDGEDECAL_PT_panel(Panel):
    bl_label = "Edge Decal Generator"
    bl_idname = "EDGEDECAL_PT_panel"
    ...
    def draw(self, context):
        ...
        # Compact branded header and current context.
        header = layout.box()
        title = header.row(align=True)
        title.scale_y = 1.15
        title.label(text="Edge Decal Generator", icon="MOD_BEVEL")

        draw_edge_decal_presets(layout, context)

        if source_obj is None:
            header.label(text="Select a mesh to begin", icon="INFO")
            return

        if source_mesh_needs_layer_repair(source_obj):
            schedule_decal_layer_repair(source_obj)

        # Resolve the layer list and active layer once per redraw and reuse the
        # results throughout the panel instead of re-scanning the collection.
        layers = sorted_decal_layers_for_source(source_obj)
        active_layer = active_decal_layer_for_source(source_obj, context=context)
        if (
        ...
        status.label(text=f"Source: {source_obj.name}", icon="OBJECT_DATA")
        if active_layer is not None:
            status.label(
                text=str(
                    active_layer.get(
        ...
        if not layers:
            starter = layout.box()
            starter.label(
                text="Create a decal layer before generating",
                icon="INFO",
            )
        ...
        if (
            ...
            row = actions.row(align=True)
            update = row.operator(
                EDGEDECAL_OT_regenerate.bl_idname,
                text="Update Layer",
                icon="FILE_REFRESH",
            )
            update.preview = False
            apply_uvs = row.row(align=True)
            apply_uvs.enabled = not active_layer.get(
                "edge_decal_locked",
                False,
            )
            apply_uvs.operator(
                EDGEDECAL_OT_apply_uvs.bl_idname,
                text="Apply UVs",
                icon="UV",
            )

        # Quick controls: the settings changed most often.
        quick = layout.box()
        quick_header = quick.row(align=True)
        quick_header.label(text="Quick Controls", icon="PREFERENCES")
        if active_layer is not None:
            quick_header.prop(
                active_layer.edge_decal_object_settings,
                "live_update",
        ...
        # Standard Blender numeric field: drag freely, hold Shift for fine
        # adjustment, and type any value without a slider-range ceiling.
        width.prop(settings, "face_width")
        body.prop(
            settings,
        ...
        random_width_bounds.prop(settings, "minimum_face_width")
        random_width_bounds.prop(settings, "maximum_face_width")
        body.prop(settings, "surface_offset")
        body.prop(settings, "decal_amount", slider=True)
        body.prop(settings, "seed")
        if context.mode == "OBJECT":
            auto_angle = body.row(align=True)
            auto_angle.scale_y = 1.1
            auto_angle.prop(settings, "auto_edge_angle")
            body.prop(settings, "auto_follow_edge_loops")
        body.prop(settings, "use_edge_split")
        quick_split_angle = body.row(align=True)
        quick_split_angle.enabled = settings.use_edge_split
        quick_split_angle.prop(settings, "split_angle")
        ...
        # Geometry is a full category; Quick Controls remains limited to the
        # settings adjusted most often during generation.
        draw_shape_settings(layout, settings)

        draw_masking_category(
            layout,
            context,
            settings,
            active_layer,
        ...
        # Collapsible material category.
        material = layout.box()
        if draw_edge_decal_foldout(
            material,
            settings,
            "show_material_category",
            "Material",
            icon="MATERIAL",
        ):
            ...
            if settings.use_material:
                ...
                pending_name = (
                    pending_fn(context.scene, settings)
                    if pending_fn is not None
                    else ""
                )
                if pending_name:
                    material_body.label(
                        text=f"Loads on first generation: {pending_name}",
                        icon="INFO",
                    )
                material_body.prop(
                    settings,
                ...
                if settings.match_source_material:
                    ...
                    projection.prop(
                        settings,
                        "generate_second_uv",
                        text="Material UV Projection",
                        toggle=True,
                    )
                    update_material = material_body.row(align=True)
                    update_material.enabled = active_layer is not None
                    update_material.operator(
                        EDGEDECAL_OT_update_material.bl_idname,
                        text="Update Material",
                        icon="FILE_REFRESH",
                    )
                    if active_layer is not None:
                        source_material = getattr(
                            active_layer.edge_decal_object_settings,
                            "source_material",
                            None,
                        )
                        if source_material is not None:
                            material_body.label(
                                text=f"Source: {source_material.name}",
                                icon="LINKED",
                            )

        # Collapsible UV category.
        uv_card = layout.box()
        if draw_edge_decal_foldout(
            uv_card,
            settings,
            ...
            auto_pins.prop(settings, "auto_use_uv_pins", text="Auto Pins")
            ...
            uv_actions.prop(settings, "uv_scale", text="Scale", slider=True)
            draw_uv_settings(uv_card, settings)

        # Less frequently changed categories remain easy to find without a
        # second master disclosure layer.
        draw_finish_settings(layout, settings)
        draw_normals_settings(layout, settings)

        draw_edge_decal_unreal_export(
            layout,
            context,
            source_obj,
            layers,
        ...
        if draw_edge_decal_foldout(
            ...
            help_box.label(
                text="Click add/edit | Hold R + click remove"
            )
            help_box.label(
                text="Hold R + double-click connected | Alt chain | Shift path"
            )
            help_box.label(
                text="Ctrl slice | T trim"
            )
