...
SCENE_LIVE_EDIT_PROPERTIES=("face_width","randomize_face_width","minimum_face_width","maximum_face_width","surface_offset","uv_scale","seed","decal_amount","maximum_decal_length","use_edge_split","split_angle","taper_sliced_ends","slice_taper_length","auto_trim_corner_ends","corner_end_trim_multiplier","crevice_removal","crevice_detection_mode","crevice_ao_distance","crevice_ao_samples","remove_short_edges","minimum_edge_length","randomize_horizontal_offset","horizontal_randomize_amount","auto_face_width","auto_width_samples","auto_width_clearance","clamp_edge_overlaps","overlap_clearance","normal_mode","normal_keep_sharp","normal_weight","normal_threshold", "use_face_loop_slide","add_weld_modifier","weld_distance","add_center_displace_modifier","center_displace_strength","add_shrinkwrap_modifier","add_subdivision_modifier","add_decimate_modifier", "add_bevel_modifier","bevel_edge_center","center_bevel_width","center_bevel_segments","center_bevel_profile","bevel_harden_normals", "bevel_angle","match_source_material",)
...
def scene_live_edit_signature(settings):
	return tuple(_stable_scene_setting_value(getattr(settings,name,None))
		for name in SCENE_LIVE_EDIT_PROPERTIES) + (_stable_scene_setting_value(getattr(settings, "decal_material",None)),bool(getattr(settings, "use_material",True)),)
	...
	try:
		...
		if hasattr(data, "decal_template_material"):
			data.decal_template_material=(root_decal_template_material(scene_settings.decal_material)
				if getattr(scene_settings, "use_material",True) else None)
	finally:
...
def edge_decal_scene_live_sync_timer():
	...
	try:
		...
		if (decal_obj is None
			...
			or not getattr(decal_obj.edge_decal_object_settings,"initialized",False,)
			or not getattr(decal_obj.edge_decal_object_settings,"live_update",False,)
		...
		EDGEDECAL_SCENE_LIVE_SYNC_CACHE[cache_key]=signature
		copy_scene_live_settings_to_decal(scene_settings,decal_obj)
		queue_decal_live_update(decal_obj,fast_geometry_only=bool(getattr(scene_settings, "fast_geometry_only",False)),)
...
def _source_bevel_signature(source_obj):
	...
	if values is None:
		return None
	return (round(values["width"],8),values["segments"],round(values["profile"],6),round(values["angle_limit"],8),values["limit_method"],values["affect"],values["offset_type"],values["harden_normals"],values["loop_slide"],values["use_clamp_overlap"],values["miter_outer"],values["miter_inner"],)
def decal_uses_face_generation(decal_obj):
	...
	return bool(decal_obj is not None
		and (decal_obj.get("edge_decal_mode")== "FACE_SURFACES"
			or str(getattr(data, "selection_mode", "") if data is not None else "")== "SELECTED_FACES" ))
...
def sync_generated_decal_bevels(source_obj,scene_settings=None):
	...
	if values is None:
		for decal_obj in iter_generated_decals(source_obj=source_obj):
			...
			if (scene_settings is not None
				...
				ensure_decal_bevel_modifier(decal_obj,source_obj,scene_settings,)
	...
	for decal_obj in iter_generated_decals(source_obj=source_obj):
		if decal_uses_face_generation(decal_obj):
			disable_face_generated_decal_bevel(decal_obj)
			continue
		layer_settings=getattr(decal_obj,"edge_decal_object_settings",None,)
		...
		if (scene_settings is None
			...
			if layer_settings is not None:
				ensure_decal_bevel_modifier(decal_obj,source_obj,layer_settings,)
...
def edge_decal_invalidate_prepared_source_cache_handler(_scene,depsgraph):
	...
	for update in depsgraph.updates:
		...
		if isinstance(updated_id,bpy.types.Mesh):
			mesh=updated_id
		elif (isinstance(updated_id,bpy.types.Object)
...
def register_decal_in_registry(decal_obj,source_obj=None):
	...
	try:
		...
		return False
	old_source_pointer=EDGEDECAL_SOURCE_POINTER_BY_DECAL_POINTER.get(decal_pointer)
	...
	bucket=EDGEDECAL_DECALS_BY_SOURCE.setdefault(source_pointer,{"source": source_obj, "decals": {}},)
...
def registered_decal_layers_for_source(source_obj):
	...
	if bucket is None:
		return []
	return [decal_obj
		for decal_obj in bucket["decals"].values() if decal_layer_is_valid(decal_obj,source_obj,assume_in_object_data=True,)]
...
def _collect_decal_registry():
	return {decal_obj.name_full: source_obj.name_full for decal_obj,source_obj in _scan_decal_registry_entries()}
def rebuild_decal_registry():
	...
	for scene in bpy.data.scenes:
		EDGEDECAL_OBJECT_COUNT_SIGNATURES[int(scene.as_pointer())]=(data_object_count,len(scene.objects),)
...
def _prune_decal_registry():
	...
	for source_pointer,bucket in tuple(EDGEDECAL_DECALS_BY_SOURCE.items()):
		...
		for decal_name,decal_obj in tuple(bucket["decals"].items()):
			...
			try:
				EDGEDECAL_SOURCE_POINTER_BY_DECAL_POINTER.pop(int(decal_obj.as_pointer()),None,)
			except ReferenceError:
				pass
...
def edge_decal_repair_layers_handler(_scene,depsgraph):
	...
	if (EDGEDECAL_LAYER_REPAIR_HANDLER_RUNNING
	...
	try:
		...
		if not EDGEDECAL_DECAL_REGISTRY_READY:
			...
			for decal_name in set(previous) - current_names:
				source_obj=source_mesh_for_removed_decal(decal_name,previous.get(decal_name, ""),)
				if source_obj is not None:
					try:
		...
		for update in getattr(depsgraph, "updates",()):
			...
			try:
				...
				continue
			if (updated_pointer in EDGEDECAL_SOURCE_POINTER_BY_DECAL_POINTER
		...
		EDGEDECAL_LAYER_REPAIR_HANDLER_RUNNING=False
classes=(EDGEDECAL_AP_preferences,EDGEDECAL_PG_settings,EDGEDECAL_PG_object_settings,EDGEDECAL_PG_preset_ui,EDGEDECAL_OT_preset_save,EDGEDECAL_OT_preset_apply,EDGEDECAL_OT_preset_delete,EDGEDECAL_OT_regenerate,EDGEDECAL_OT_update_material,EDGEDECAL_OT_apply_uvs,EDGEDECAL_PG_uv_slice_pin,EDGEDECAL_PG_uv_pin,EDGEDECAL_OT_generate,EDGEDECAL_OT_generate_selected_faces,EDGEDECAL_OT_generate_contextual,EDGEDECAL_OT_generate_automatic,EDGEDECAL_OT_generate_intersections,EDGEDECAL_OT_generate_boolean,EDGEDECAL_OT_uv_pin_toggle_edit_mode,EDGEDECAL_OT_uv_slice_pin_set_shortcut,EDGEDECAL_OT_uv_pin_add_shortcut,EDGEDECAL_OT_uv_pin_remove_shortcut,EDGEDECAL_OT_uv_pin_move_shortcut,EDGEDECAL_OT_uv_pin_tool,EDGEDECAL_OT_uv_pin_add,EDGEDECAL_OT_uv_pin_delete,EDGEDECAL_OT_uv_pin_remove_index,EDGEDECAL_OT_uv_pin_clear,EDGEDECAL_OT_uv_pin_create_equal,EDGEDECAL_OT_layer_toggle_uv_pin,EDGEDECAL_OT_uv_slice_pin_add,EDGEDECAL_OT_uv_slice_pin_remove,EDGEDECAL_UL_uv_pins,EDGEDECAL_OT_apply_uv_pins,EDGEDECAL_OT_remove_decal_sections,EDGEDECAL_OT_interactive_generate,EDGEDECAL_OT_texture_mask_add,EDGEDECAL_OT_texture_mask_remove,EDGEDECAL_OT_texture_mask_reset,EDGEDECAL_OT_texture_mask_paint,EDGEDECAL_PG_unreal_export_settings,EDGEDECAL_OT_send_to_unreal,EDGEDECAL_OT_export_combined_mesh,EDGEDECAL_OT_export_unreal_bundle,EDGEDECAL_PG_layer_item,EDGEDECAL_UL_layers,EDGEDECAL_OT_layer_select,EDGEDECAL_OT_layer_add,EDGEDECAL_OT_layer_delete,EDGEDECAL_OT_layer_delete_active,EDGEDECAL_OT_layer_move,EDGEDECAL_OT_layer_apply_mask,EDGEDECAL_OT_layer_clear_mask,EDGEDECAL_OT_layer_toggle_lock,EDGEDECAL_OT_layer_toggle_visibility,EDGEDECAL_PT_panel,EDGEDECAL_PT_uv_pins,)
...
def edge_decal_rebuild_registry_handler(*_args):
	...
	if not bpy.app.timers.is_registered(_sync_source_layer_ui_after_register):
		bpy.app.timers.register(_sync_source_layer_ui_after_register,first_interval=0.0,)
def register():
	invalidate_edge_decal_bundled_asset_cache()
	invalidate_edge_decal_preset_cache()
	RCL(classes)
	SAF(setattr,bpy.types.Scene,'edge_decal_settings',bpy.props.PointerProperty(type=EDGEDECAL_PG_settings))
	SAF(setattr,bpy.types.Scene,'edge_decal_preset_ui',bpy.props.PointerProperty(type=EDGEDECAL_PG_preset_ui))
	SAF(setattr,bpy.types.Scene,'edge_decal_unreal_export',bpy.props.PointerProperty(type=EDGEDECAL_PG_unreal_export_settings))
	SAF(setattr,bpy.types.Object,'edge_decal_object_settings',bpy.props.PointerProperty(type=EDGEDECAL_PG_object_settings))
	SAF(setattr,bpy.types.Object,'edge_decal_layers_ui',bpy.props.CollectionProperty(type=EDGEDECAL_PG_layer_item,))
	SAF(setattr,bpy.types.Object,'edge_decal_layer_index',bpy.props.IntProperty(name='Active Decal Layer',description='贴花层列表中的选定行',default=0,min=0,update=update_edge_decal_layer_index,))
	if not bpy.app.timers.is_registered(_sync_source_layer_ui_after_register):
		bpy.app.timers.register(_sync_source_layer_ui_after_register,first_interval=0.0,)
	SAF(setattr,bpy.types.Scene,'edge_decal_uv_pins',bpy.props.CollectionProperty(type=EDGEDECAL_PG_uv_pin))
	SAF(setattr,bpy.types.Scene,'edge_decal_uv_pin_index',bpy.props.IntProperty(name='Selected UV Pin',default=-1,min=-1,))
	SAF(setattr,bpy.types.Scene,'edge_decal_uv_pin_material',bpy.props.PointerProperty(name='UV Pin Material',type=bpy.types.Material,description='编辑该材质的保持UV固定集',update=update_uv_pin_material_selection,))
	SAF(setattr,bpy.types.Scene,'edge_decal_show_uv_pins',bpy.props.BoolProperty(name='Show UV Pins',default=True,description='在UV编辑器中显示全部贴花固定',update=update_uv_pin_overlay_setting,))
	SAF(setattr,bpy.types.Scene,'edge_decal_pin_edit_active',bpy.props.BoolProperty(name='Editing UV Pins',default=False,options={'HIDDEN'},))
	SAF(setattr,bpy.types.Scene,'edge_decal_selected_pin_color',bpy.props.FloatVectorProperty(name='Selected Pin Color',subtype='COLOR',size=3,default=(1.0,0.35,0.05),min=0.0,max=1.0,update=update_uv_pin_overlay_setting,))
	SAF(setattr,bpy.types.Scene,'edge_decal_unselected_pin_color',bpy.props.FloatVectorProperty(name='Unselected Pin Color',subtype='COLOR',size=3,default=(0.15,0.7,1.0),min=0.0,max=1.0,update=update_uv_pin_overlay_setting,))
	SAF(setattr,bpy.types.Scene,'edge_decal_selected_pin_size',bpy.props.FloatProperty(name='Selected Pin Size',default=10.0,min=3.0,max=40.0,soft_min=5.0,soft_max=20.0,update=update_uv_pin_overlay_setting,))
	SAF(setattr,bpy.types.Scene,'edge_decal_unselected_pin_size',bpy.props.FloatProperty(name='Unselected Pin Size',default=7.0,min=3.0,max=40.0,soft_min=4.0,soft_max=18.0,update=update_uv_pin_overlay_setting,))
	global EDGEDECAL_UV_PIN_DRAW_HANDLE
	if EDGEDECAL_UV_PIN_DRAW_HANDLE is None:
		EDGEDECAL_UV_PIN_DRAW_HANDLE=(bpy.types.SpaceImageEditor.draw_handler_add(draw_uv_pin_overlay,(), "WINDOW","POST_PIXEL",))
	...
	if keyconfig is not None:
		...
		add_item=keymap.keymap_items.new(EDGEDECAL_OT_uv_pin_add_shortcut.bl_idname,type="LEFTMOUSE",value="PRESS",shift=True,)
		EDGEDECAL_ADDON_KEYMAPS.append((keymap,add_item))
		remove_item=keymap.keymap_items.new(EDGEDECAL_OT_uv_pin_remove_shortcut.bl_idname,type="RIGHTMOUSE",value="PRESS",shift=True,)
		EDGEDECAL_ADDON_KEYMAPS.append((keymap,remove_item))
		move_item=keymap.keymap_items.new(EDGEDECAL_OT_uv_pin_move_shortcut.bl_idname,type="LEFTMOUSE",value="PRESS",)
		EDGEDECAL_ADDON_KEYMAPS.append((keymap,move_item))
		exit_edit_item=keymap.keymap_items.new(EDGEDECAL_OT_uv_pin_toggle_edit_mode.bl_idname,type="ESC",value="PRESS",)
	...
	if (edge_decal_invalidate_prepared_source_cache_handler
		not in bpy.app.handlers.depsgraph_update_post):
		bpy.app.handlers.depsgraph_update_post.append(edge_decal_invalidate_prepared_source_cache_handler)
	if (edge_decal_clear_prepared_source_cache_handler
		not in bpy.app.handlers.load_post):
		bpy.app.handlers.load_post.append(edge_decal_clear_prepared_source_cache_handler)
	...
	if not bpy.app.timers.is_registered(edge_decal_scene_live_sync_timer):
		bpy.app.timers.register(edge_decal_scene_live_sync_timer,first_interval=EDGEDECAL_SCENE_LIVE_SYNC_INTERVAL,persistent=True,)
...
def unregister():
	...
	if (edge_decal_invalidate_prepared_source_cache_handler
		...
		bpy.app.handlers.depsgraph_update_post.remove(edge_decal_invalidate_prepared_source_cache_handler)
	if (edge_decal_clear_prepared_source_cache_handler
		in bpy.app.handlers.load_post):
		bpy.app.handlers.load_post.remove(edge_decal_clear_prepared_source_cache_handler)
	...
	if edge_decal_initialize_presets_handler in bpy.app.handlers.load_post:
		bpy.app.handlers.load_post.remove(edge_decal_initialize_presets_handler)
	clear_prepared_source_cache(reset_stats=True)
	clear_decal_registry()
	...
	if EDGEDECAL_UV_PIN_DRAW_HANDLE is not None:
		bpy.types.SpaceImageEditor.draw_handler_remove(EDGEDECAL_UV_PIN_DRAW_HANDLE,"WINDOW",)
		EDGEDECAL_UV_PIN_DRAW_HANDLE=None
	for property_name in ("edge_decal_unselected_pin_size","edge_decal_selected_pin_size","edge_decal_unselected_pin_color","edge_decal_selected_pin_color","edge_decal_pin_edit_active","edge_decal_show_uv_pins",):
		if hasattr(bpy.types.Scene,property_name):
			delattr(bpy.types.Scene,property_name)
	if hasattr(bpy.types.Scene, "edge_decal_uv_pin_index"):
		DAS("edge_decal_uv_pin_index","Scene")
	if hasattr(bpy.types.Scene, "edge_decal_uv_pin_material"):
		DAS("edge_decal_uv_pin_material","Scene")
	if hasattr(bpy.types.Scene, "edge_decal_uv_pins"):
		DAS("edge_decal_uv_pins","Scene")
	if hasattr(bpy.types.Scene, "edge_decal_settings"):
		DAS("edge_decal_settings","Scene")
	if hasattr(bpy.types.Scene, "edge_decal_preset_ui"):
		DAS("edge_decal_preset_ui","Scene")
	if hasattr(bpy.types.Scene, "edge_decal_unreal_export"):
		DAS("edge_decal_unreal_export","Scene")
	if hasattr(bpy.types.Object, "edge_decal_object_settings"):
		DAS("edge_decal_object_settings","Object")
	if hasattr(bpy.types.Object, "edge_decal_layer_index"):
		DAS("edge_decal_layer_index","Object")
	if hasattr(bpy.types.Object, "edge_decal_layers_ui"):
		DAS("edge_decal_layers_ui","Object")
	EDGEDECAL_LIVE_UPDATE_QUEUE.clear()
	UCL(classes)
