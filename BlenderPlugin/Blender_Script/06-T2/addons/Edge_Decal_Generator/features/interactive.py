...
"""
def resolve_interactive_preview_width(face_width,relative_face_width,source_reference_size,):
...
class EDGEDECAL_OT_interactive_generate(Operator):
	bl_idname= "mesh.edge_decal_interactive_generate";bl_label= g("Interactive Generate");bl_description=("悬停源网格边并单击以生成贴花，而无需手动选择它们");bl_options={"REGISTER", "UNDO"}
	_draw_handle=None
	...
	def _restore_working_mode(self,context):
		source_obj=self._activate_source(context,edit_mode=self._started_in_edit_mode,)
		if source_obj is None:
			return None
		...
		try:
			for edge in bm.edges:
				if edge.hide or len(edge.link_faces) !=2:
					continue
				yield (edge.index,edge.verts[0].co.copy(),edge.verts[1].co.copy(),)
		finally:
			if owns_bm:
				bm.free()
		...
		try:
			...
			for index in pending_valid:
				for vert in bm.edges[index].verts:
					vertex_pending_count[vert.index]=(vertex_pending_count.get(vert.index,0) + 1)
			endpoint_candidates=[]
			for index in pending_valid:
				for vert in bm.edges[index].verts:
					if vertex_pending_count.get(vert.index,0)==1:
						endpoint_candidates.append((index,world_matrix @ vert.co))
			...
			target_mid=((world_matrix @ target_verts[0].co)
				+ (world_matrix @ target_verts[1].co)) * 0.5
			best=None
	...
	def _edge_path_between(self,source_obj,start_edge_index,end_edge_index):
		...
		try:
			...
			valid_edges=[edge for edge in bm.edges if not edge.hide and len(edge.link_faces)==2]
			if not valid_edges:
				return []
			average_length=sum(((world_matrix @ edge.verts[1].co) -
				 (world_matrix @ edge.verts[0].co)).length
				for edge in valid_edges) / max(1,len(valid_edges))
			...
			while queue:
				...
				current_direction=safe_normalized((world_matrix @ exit_vertex.co)
					- (world_matrix @ entry_vertex.co),Vector((1.0,0.0,0.0)),)
				for neighbor in exit_vertex.link_edges:
					...
					next_direction=safe_normalized((world_matrix @ other_vertex.co)
						- (world_matrix @ exit_vertex.co),current_direction,)
					dot_value=max(-1.0,min(1.0,current_direction.dot(next_direction)))
					turn_angle=acos(dot_value)
					neighbor_length=((world_matrix @ other_vertex.co)
						- (world_matrix @ exit_vertex.co)).length
		...
		try:
			for edge_index in edge_indices:
				if 0 <=edge_index < len(bm.edges):
					...
					segments.extend((world_matrix @ edge.verts[0].co,world_matrix @ edge.verts[1].co,))
		finally:
			if owns_bm:
				bm.free()
	...
	def _refresh_pending_overlay(self,context):
		...
		if source_obj is None or not self._pending_edge_indices:
			...
			return
		full_edges=[edge_index for edge_index in self._pending_edge_indices if edge_index !=self._pending_partial_edge_index]
		points=list(self._edge_world_segments(source_obj,full_edges))
		if (self._pending_partial_edge_index >=0
					...
					for candidate_index in reversed(self._pending_edge_indices):
						...
						if 0 <=candidate_index < len(bm.edges):
							...
							if any(vertex in candidate.verts for vertex in edge.verts):
								...
								break
					shared=(next((vertex for vertex in edge.verts if previous_edge and vertex in previous_edge.verts),None))
			...
			points.extend(self._edge_world_segments(source_obj,[self._pending_partial_edge_index] if self._pending_partial_edge_index >=0 else [],))
	...
	def _find_hovered_edge(self,context):
		...
		for edge_index,local_a,local_b in self._iter_hover_edges(source_obj):
			...
			point_a=view3d_utils.location_3d_to_region_2d(region,region_3d,world_a)
			point_b=view3d_utils.location_3d_to_region_2d(region,region_3d,world_b)
			if point_a is None or point_b is None:
				continue
			distance,screen_factor=self._screen_distance_and_factor_to_segment(mouse,point_a,point_b,)
			if distance >=best_distance:
				continue
			factor=self._world_factor_from_screen_factor(region_3d,world_a,world_b,screen_factor,)
			hit_point=world_a.lerp(world_b,factor)
			hit_screen=point_a.lerp(point_b,screen_factor)
			if not self._source_point_is_view_visible(context,source_obj,
				hit_point,hit_screen,):
				continue
			best_distance=distance
			best_index=edge_index
		if best_index < 0:
			return -1,[],None
		edge_indices=(self._topological_edge_loop(source_obj,best_index) if self._alt_down else [best_index])
	...
	def _ctrl_partial_preview(self,context,edge_index):
		...
		try:
			...
			region=context.region
			region_3d=context.space_data.region_3d
			screen_points=[view3d_utils.location_3d_to_region_2d(region,region_3d,point) for point in (world_a,world_mid,world_b)]
			if any(point is None for point in screen_points):
				return None,None,None,-1
			mouse=Vector(self._mouse_region)
			active_index=min(range(3),
	...
	def _update_hover(self,context):
		(self._hovered_edge_index,self._hovered_edge_indices,self._hovered_edge_points,)=self._find_hovered_edge(context)
		...
		if (self._ctrl_down
			...
			interval,points,control_points,active_point=self._ctrl_partial_preview(context,self._hovered_edge_index,)
	...
	def _compute_shift_slice_interval(self,context,target_edges,path):
		...
		try:
			...
			previous_edge=(bm.edges[path[path_position - 1]]
				if path_position > 0 and 0 <=path[path_position - 1] < len(bm.edges) else None)
			shared=next((v for v in target_edge.verts if previous_edge and v in previous_edge.verts),None,)
			if shared is None:
				return None
			for chain_verts,chain_edges,closed in chains:
				...
				if shared==start_vertex:
					...
					return None
				self._shift_slice_interval=(max(0.0,min(1.0,interval[0])),max(0.0,min(1.0,interval[1])),)
	...
	def _update_shift_preview(self,context):
		...
		if source_obj is None:
			return
		full_path=self._edge_path_between(source_obj,self._effective_path_anchor(source_obj),self._hovered_edge_index,)
		if not full_path:
			return
		...
		self._shift_span_edges=max(0.05,min(float(len(new_edges)),float(self._shift_span_edges)),)
		...
		partial_target=self._shift_partial_target_segment(source_obj,included_path,terminal_edge_index,terminal_fraction,)
	...
	def _build_width_preview_ribbon(self,context,segment_points,width):
		...
		for index in range(0,len(segment_points) - 1,2):
			...
			triangles.extend((left_a,left_b,right_b,left_a,right_b,right_a,))
			outlines.extend((left_a,left_b,right_a,right_b,))
		return triangles,outlines
	def _draw_width_preview(self,context,shader,segment_points,width,fill_color,line_color):
		triangles,outlines=self._build_width_preview_ribbon(context,segment_points,width,)
		if triangles:
	...
	def _screen_distance_to_segment(self,point,a,b):
		distance,_factor=self._screen_distance_and_factor_to_segment(point,a,b,)
		return distance
	def _screen_distance_and_factor_to_segment(self,point,a,b):
		...
		factor=max(0.0,min(1.0,(point - a).dot(segment) / length_sq),)
		closest=a + segment * factor
		return (point - closest).length,factor
	def _world_factor_from_screen_factor(self,region_3d,world_a,world_b,screen_factor,):
		...
		denominator=((1.0 - screen_factor) * clip_b.w + screen_factor * clip_a.w)
		if abs(denominator) <=EPSILON:
			return screen_factor
		world_factor=screen_factor * clip_a.w / denominator
		return max(0.0,min(1.0,float(world_factor)))
	def _source_point_is_view_visible(self,context,source_obj,world_point,screen_point,):
		region=context.region
		...
		ray_origin=view3d_utils.region_2d_to_origin_3d(region,region_3d,screen_point,)
		ray_direction=view3d_utils.region_2d_to_vector_3d(region,region_3d,screen_point,)
		...
		hit,local_hit,_normal,_face_index=eval_obj.ray_cast(local_origin,local_direction,distance=local_distance + 0.01,)
		if not hit:
			return True
	...
	def _interactive_help_shortcuts(self):
		...
		return [("LMB", "Add / edit edge"),("Shift + LMB", "Shortest path / join partial"),("Shift + Wheel", "Shortest path length"),("Ctrl + LMB", "Partial edge (standalone)"),("Ctrl + Wheel", "Partial length"),("Alt", "Topology edge loop"),("E",f"Endpoint taper ({taper_state})"),("F",f"Auto-merge neighbor ({merge_state})"),("R + LMB", "Remove edge"),("R + Double LMB", "Remove connected edges"),("W + Wheel", "Adjust width"),("Ctrl + Z", "Undo"),("Del / X", "Clear pending path"),("H", "Toggle this help"),("RMB / Esc", "Finish"),]
	...
	def _help_overlay_box_x(self,context,box_width,margin):
		sidebar_width=max((area_region.width for area_region in context.area.regions if area_region.type== "UI"),default=0,)
		right_clearance=max(360.0,float(sidebar_width) + 48.0)
		return max(float(margin),float(context.region.width)
			- float(margin)
			- float(box_width) - right_clearance,)
	def _draw_interactive_mode_badge(self,context):
		if (not EDGEDECAL_INTERACTIVE_RUNNING
		...
		content_gap=18
		text_gap=32
		badge_width=(pad_x * 2 + dot_size + content_gap + label_width + text_gap + help_width + text_gap + exit_width)
		badge_x=max(margin,(float(region.width) - badge_width) * 0.5)
		badge_y=max(margin,float(region.height) - top_clearance - badge_height,)
		shader=gpu.shader.from_builtin("UNIFORM_COLOR")
		gpu.state.blend_set("ALPHA")
		vertices=((badge_x,badge_y),(badge_x + badge_width,badge_y),(badge_x + badge_width,badge_y + badge_height),(badge_x,badge_y + badge_height),)
		indices=((0,1,2),(0,2,3))
		shader.bind()
		shader.uniform_float("color",(0.055,0.07,0.08,0.92))
		batch_for_shader(shader, "TRIS",{"pos": vertices},indices=indices).draw(shader)
		shader.uniform_float("color",(0.30,0.68,0.95,0.90))
		gpu.state.line_width_set(2.0)
		border=((badge_x,badge_y),(badge_x + badge_width,badge_y),(badge_x + badge_width,badge_y + badge_height),(badge_x,badge_y + badge_height),(badge_x,badge_y),)
		batch_for_shader(shader, "LINE_STRIP",{"pos": border}).draw(shader)
		gpu.state.line_width_set(1.0)
		dot_x=badge_x + pad_x
		dot_y=badge_y + (badge_height - dot_size) * 0.5
		dot_vertices=((dot_x,dot_y),(dot_x + dot_size,dot_y),(dot_x + dot_size,dot_y + dot_size),(dot_x,dot_y + dot_size),)
		shader.uniform_float("color",(0.25,0.72,1.0,1.0))
		batch_for_shader(shader,"TRIS",
			{"pos": dot_vertices},indices=indices,).draw(shader)
		label_x=dot_x + dot_size + content_gap
		blf.size(font_id,26)
		blf.position(font_id,label_x,badge_y + (badge_height - label_height) * 0.5,0,)
		blf.color(font_id,0.45,0.85,1.0,1.0)
		blf.draw(font_id,g(label))
		blf.size(font_id,22)
		help_x=label_x + label_width + text_gap
		blf.position(font_id,help_x,badge_y + (badge_height - help_height) * 0.5,0,)
		blf.color(font_id,0.45,0.85,1.0,1.0)
		blf.draw(font_id,g(help_hint))
		blf.position(font_id,help_x + help_width + text_gap,badge_y + (badge_height - exit_height) * 0.5,0,)
		blf.color(font_id,0.72,0.76,0.80,1.0)
		blf.draw(font_id,g(exit_hint))
	...
	def _draw_help_overlay(self,context):
		...
		key_width=max((blf.dimensions(font_id,key_text)[0] for key_text,_ in shortcuts),default=0.0,)
		desc_width=max((blf.dimensions(font_id,desc_text)[0] for _,desc_text in shortcuts),default=0.0,)
		...
		shader=gpu.shader.from_builtin("UNIFORM_COLOR")
		gpu.state.blend_set("ALPHA")
		vertices=((box_x,box_y),(box_x + box_width,box_y),(box_x + box_width,box_y + box_height),(box_x,box_y + box_height),)
		indices=((0,1,2),(0,2,3))
		shader.bind()
		shader.uniform_float("color",(0.08,0.08,0.10,0.88))
		batch_for_shader(shader, "TRIS",{"pos": vertices},indices=indices).draw(shader)
		shader.uniform_float("color",(0.30,0.68,0.95,0.45))
		border=((box_x,box_y),(box_x + box_width,box_y),(box_x + box_width,box_y + box_height),(box_x,box_y + box_height),(box_x,box_y),)
		batch_for_shader(shader, "LINE_STRIP",{"pos": border}).draw(shader)
	...
	def _draw_overlay(self,context):
		...
		preview_width=resolve_interactive_preview_width(self._interactive_face_width,bool(settings.relative_face_width),self._interactive_width_reference_size,)
		if self._pending_edge_points and self._shift_down:
			self._draw_width_preview(context,shader,self._pending_edge_points,preview_width,(1.0,0.15,0.75,0.08),(1.0,0.4,0.9,0.30),)
			gpu.state.line_width_set(4.0)
			shader.uniform_float("color",(1.0,0.15,0.75,0.95))
			batch_for_shader(shader,"LINES",
				{"pos": self._pending_edge_points},).draw(shader)
		...
		if self._shift_preview_edge_points and not self._suppress_hover_until_mousemove:
			self._draw_width_preview(context,shader,self._shift_preview_edge_points,preview_width,(0.25,1.0,0.2,0.10),(0.45,1.0,0.35,0.45),)
			gpu.state.line_width_set(7.0)
			shader.uniform_float("color",(0.25,1.0,0.2,1.0))
			batch_for_shader(shader,"LINES",
				{"pos": self._shift_preview_edge_points},).draw(shader)
			gpu.state.point_size_set(10.0)
			shader.uniform_float("color",(0.65,1.0,0.35,1.0))
			batch_for_shader(shader,"POINTS",
				{"pos": self._shift_preview_edge_points},).draw(shader)
		if self._ctrl_down and self._ctrl_control_points:
			inactive_points=[point for index,point in enumerate(self._ctrl_control_points) if index !=self._ctrl_active_point]
			active_points=([self._ctrl_control_points[self._ctrl_active_point]]
				if 0 <=self._ctrl_active_point < len(self._ctrl_control_points) else [])
			if inactive_points:
		...
		if self._hovered_edge_points and not self._suppress_hover_until_mousemove:
			hover_fill=((0.75,0.25,1.0,0.10)
				if self._ctrl_down and self._ctrl_slice_interval is not None
				else (0.1,0.85,1.0,0.10)
				if self._alt_down
				else (1.0,0.55,0.05,0.10))
			hover_line=((0.85,0.45,1.0,0.45)
				if self._ctrl_down and self._ctrl_slice_interval is not None
				else (0.25,0.9,1.0,0.45)
				if self._alt_down
				else (1.0,0.75,0.2,0.45))
			self._draw_width_preview(context,shader,self._hovered_edge_points,preview_width,hover_fill,hover_line,)
			...
			batch_for_shader(shader,"LINES",
				{"pos": self._hovered_edge_points},).draw(shader)
	...
	def _draw_remove_overlay(self,context):
		...
		for component_index,point_2d in self._project_remove_targets(context):
			...
			self._draw_remove_target(shader,point_2d,self._remove_target_radius + (3.0 if hovered else 0.0),(1.0,0.75,0.05,1.0) if hovered else (1.0,0.02,0.02,0.95),5.0 if hovered else 4.0,)
		gpu.state.line_width_set(1.0)
		gpu.state.blend_set("NONE")
	def _remove_stroke_source_indices(self,object_name,edge_indices):
		...
		remaining=[index for index in parsed_source_indices(data) if index not in removed]
	...
	def _remove_master_vertices(self,object_name,vertex_indices):
		...
		try:
			...
			bm.verts.ensure_lookup_table()
			vertices=[bm.verts[index] for index in vertex_indices if 0 <=index < len(bm.verts)]
			if not vertices:
				return False
	...
	def _load_interactive_strokes(self,object_name):
		...
		for entry in data:
			...
			if not (isinstance(edge_local_interval,(list,tuple))
				and len(edge_local_interval)==2):
				edge_local_interval=None
			try:
				edge_local_edge_index=int(entry.get("edge_local_slice_edge_index",-1))
			...
			if (edge_local_interval is None
				...
				if slice_space != "chain":
					...
					edge_local_interval=self._chain_interval_to_edge_local(source_obj,edges[0],interval,)
			if (interval is not None
				and slice_space != "chain"
				and source_obj is not None
				and len(edges)==1):
				converted=self._edge_local_slice_to_chain_interval(source_obj,edges[0],interval,)
			...
			try:
				taper_vertices=sorted(set(int(index) for index in entry.get("taper_vertices",[])))
			except Exception:
				taper_vertices=[]
			try:
				merge_open_vertices=sorted(set(int(index) for index in entry.get("merge_open_vertices",[])))
			except Exception:
				merge_open_vertices=[]
			try:
				merge_tip_vertices=sorted(set(int(index) for index in entry.get("merge_tip_vertices",[])))
			except Exception:
				merge_tip_vertices=[]
			stroke_record={"id": str(entry.get("id", "")),"edges": edges,"vertices": vertices,"force_connected": bool(entry.get("force_connected",False)),"slice_interval": interval,"slice_space": slice_space if interval is not None else None,"edge_local_slice_interval": (list(edge_local_interval) if edge_local_interval is not None else None),"edge_local_slice_edge_index": edge_local_edge_index,"face_width": stroke_face_width,"taper_vertices": taper_vertices, "merge_open_vertices": merge_open_vertices, "merge_tip_vertices": merge_tip_vertices,}
			if source_obj is not None and not merge_open_vertices and not merge_tip_vertices:
				open_corners,tip_corners=self._rebuild_merge_corners_for_stroke(source_obj,stroke_record,)
	...
	def _save_interactive_strokes(self,object_name,strokes):
		...
		for index,stroke in enumerate(strokes or []):
			serialized.append({"id": str(stroke.get("id",index)),"edges": sorted(set(int(value) for value in stroke.get("edges",[]))),"vertices": sorted(set(int(value) for value in stroke.get("vertices",[]))),"force_connected": bool(stroke.get("force_connected",False)),"slice_interval": stroke.get("slice_interval"),"slice_space": (stroke.get("slice_space")
					if stroke.get("slice_interval") is not None
					else None),"edge_local_slice_interval": stroke.get("edge_local_slice_interval"),"edge_local_slice_edge_index": int(stroke.get("edge_local_slice_edge_index",-1)),"face_width": float(stroke.get("face_width",0.0) or 0.0),"taper_vertices": sorted(set(int(value) for value in stroke.get("taper_vertices",[]) or [])),"merge_open_vertices": sorted(set(int(value) for value in stroke.get("merge_open_vertices",[]) or [])),"merge_tip_vertices": sorted(set(int(value) for value in stroke.get("merge_tip_vertices",[]) or [])),})
		obj["edge_decal_interactive_strokes"]=json.dumps(serialized)
	def _register_interactive_stroke(self,object_name,edge_indices,vertex_indices,force_connected=False,slice_interval=None,face_width=None,taper_vertices=None,edge_local_slice_interval=None,edge_local_slice_edge_index=None,preserve_stroke_ids=None,merge_open_vertices=None,merge_tip_vertices=None,):
		obj=bpy.data.objects.get(object_name) if object_name else None
		if obj is None:
			return
		source_obj=self._source_object(bpy.context)
		if (merge_open_vertices is None
			and merge_tip_vertices is None
			and source_obj is not None):
			if (edge_local_slice_interval is not None
				and len(edge_indices or [])==1):
				merge_open_vertices,merge_tip_vertices=(self._partial_merge_corners_from_edge(source_obj,int(edge_indices[0]),edge_local_slice_interval,))
			elif slice_interval is None:
				merge_open_vertices,merge_tip_vertices=(self._full_stroke_merge_corners(source_obj,edge_indices,taper_vertices,))
			else:
				merge_open_vertices,merge_tip_vertices=(self._rebuild_merge_corners_for_stroke(source_obj,
						{"edges": edge_indices or [],"slice_interval": list(slice_interval), "slice_space": "chain", "taper_vertices": taper_vertices or [],},))
		strokes=self._load_interactive_strokes(object_name)
		new_edge_set={int(index) for index in edge_indices or []}
		preserved_ids={str(value) for value in (preserve_stroke_ids or [])}
		if new_edge_set:
			strokes=[stroke
				for stroke in strokes
				if not ({int(index) for index in stroke.get("edges",[])} & new_edge_set and str(stroke.get("id", "")) not in preserved_ids and not self._partial_strokes_can_share_source_edge(stroke,new_edge_set,edge_local_slice_interval,source_obj=source_obj,) )]
		next_id=max([int(stroke.get("id",0) or 0) for stroke in strokes] + [0]) + 1
		strokes.append({"id": str(next_id),"edges": sorted(set(int(index) for index in edge_indices or [])),"vertices": sorted(set(int(index) for index in vertex_indices or [])),"force_connected": bool(force_connected),"slice_interval": list(slice_interval) if slice_interval is not None else None,"slice_space": "chain" if slice_interval is not None else None,"edge_local_slice_interval": (list(edge_local_slice_interval)
				if edge_local_slice_interval is not None
				else None),"edge_local_slice_edge_index": (int(edge_local_slice_edge_index)
				if edge_local_slice_edge_index is not None
				else (int(edge_indices[0])
					if edge_local_slice_interval is not None
					and len(edge_indices or [])==1 else -1)),"face_width": float(self._interactive_face_width
				if face_width is None
				else face_width),"taper_vertices": sorted(set(int(index) for index in (taper_vertices or []))),"merge_open_vertices": sorted(set(int(index) for index in (merge_open_vertices or []))),"merge_tip_vertices": sorted(set(int(index) for index in (merge_tip_vertices or []))),})
	...
	def _remove_interactive_stroke_record(self,object_name,edge_indices=None,vertex_indices=None,stroke_id=None,):
		strokes=self._load_interactive_strokes(object_name)
		remaining=[]
	...
	def _ensure_interactive_master(self,context):
		...
		if (decal_layer_is_valid(master,source_obj)
	...
	def _other_layer_label(self,decal_obj):
		...
		return str(decal_obj.get( "edge_decal_layer_name",decal_obj.name,))
	...
	def _stroke_is_partial_slice(self,stroke):
		if stroke.get("edge_local_slice_interval") is not None:
			return True
		return (self._normalized_slice_interval(stroke.get("slice_interval")) is not None)
	...
	def _open_endpoint_vertices_for_edges(self,source_obj,edge_indices):
		...
		bm,owns_bm=self._open_bmesh_for_read(source_obj)
		try:
			edges=[bm.edges[int(index)] for index in edge_indices if 0 <=int(index) < len(bm.edges)]
			if not edges:
				return set()
			...
			if owns_bm:
				bm.free()
	def _vertex_candidates_at_chain_fraction(self,chain_verts,chain_edges,world_matrix,fraction,):
		world_points=[world_matrix @ vert.co for vert in chain_verts]
		edge_lengths=[(world_points[index + 1] - world_points[index]).length for index in range(len(chain_edges))]
		...
		for edge_index,edge_length in enumerate(edge_lengths):
			if distance_remaining <=edge_length + EPSILON:
				...
				return {chain_verts[edge_index].index,chain_verts[edge_index + 1].index,}
			distance_remaining -=edge_length
		return {chain_verts[-1].index}
	def _slice_boundary_vertices(self,chain_verts,chain_edges,world_matrix,fraction,toward_start,):
		world_points=[world_matrix @ vert.co for vert in chain_verts]
		edge_lengths=[(world_points[index + 1] - world_points[index]).length for index in range(len(chain_edges))]
		total_length=sum(edge_lengths)
		if total_length <=EPSILON:
	...
	def _stroke_connectable_vertices(self,source_obj,stroke):
		...
		if not stroke_edges:
			return set()
		slice_interval=self._normalized_slice_interval(stroke.get("slice_interval"))
		bm,owns_bm=self._open_bmesh_for_read(source_obj)
		try:
			edges=[bm.edges[index] for index in stroke_edges if 0 <=index < len(bm.edges)]
			if not edges:
				return set()
			world_matrix=source_obj.matrix_world
			...
			for chain_verts,chain_edges,closed in chains:
				...
				if slice_interval is not None:
					...
					if start_trimmed:
						connectable.update(self._slice_boundary_vertices(chain_verts,chain_edges,world_matrix,start_fraction,toward_start=True,))
					else:
						connectable.add(chain_verts[0].index)
					if end_trimmed:
						connectable.update(self._slice_boundary_vertices(chain_verts,chain_edges,world_matrix,end_fraction,toward_start=False,))
					else:
			...
			if owns_bm:
				bm.free()
	def _stroke_connectable_vertices_near(self,source_obj,stroke,new_edge_indices,):
		connectable=self._stroke_connectable_vertices(source_obj,stroke)
		if not connectable:
			return set()
		new_endpoints=self._open_endpoint_vertices_for_edges(source_obj,new_edge_indices,)
		...
		try:
			...
			for vertex_index in new_endpoints:
				if 0 <=vertex_index < len(bm.verts):
					target_points.append(world_matrix @ bm.verts[vertex_index].co)
			...
			for vertex_index in connectable:
				...
				distance_sq=min((point - target_point).length_squared for target_point in target_points)
	...
	def _sync_active_layer_from_scene(self,context):
		...
		if active_name !=self._interactive_layer_name:
			...
			sync_scene_settings_from_decal_layer(context,source_obj,active_layer,)
			self._interactive_face_width=max(MIN_FACE_WIDTH,float(context.scene.edge_decal_settings.face_width),)
		elif not self._width_drag_active:
			scene_width=max(MIN_FACE_WIDTH,float(context.scene.edge_decal_settings.face_width),)
	...
	def _set_interactive_face_width(self,context,width):
		...
		if source_obj is not None:
			EDGEDECAL_SCENE_LIVE_SYNC_CACHE[scene_live_sync_cache_key(source_obj,active_decal_layer_for_source(source_obj,context=context,),)]=scene_live_edit_signature(context.scene.edge_decal_settings)
	def _endpoints_can_connect(self,source_obj,connectable,new_endpoints,stroke_edges=(),):
	...
	def _endpoint_vertex_sets_touch(self,source_obj,endpoints_a,endpoints_b):
		...
		try:
			...
			if owns_bm:
				bm.free()
	def _nearest_stroke_endpoint_edge(self,source_obj,stroke_edges,target_edge_index,connectable_vertices=None,):
		if not stroke_edges:
			return -1
		stroke_set={int(index) for index in stroke_edges}
		endpoints=set(connectable_vertices or ())
		if not endpoints:
			endpoints=self._open_endpoint_vertices_for_edges(source_obj,stroke_edges,)
		if not endpoints:
		...
		try:
			...
			world_matrix=source_obj.matrix_world
			target_mid=((world_matrix @ target_edge.verts[0].co)
				+ (world_matrix @ target_edge.verts[1].co)) * 0.5
			best_edge=-1
			best_distance_sq=float("inf")
			for vertex_index in endpoints:
				...
				for edge in bm.verts[vertex_index].link_edges:
					if edge.index not in stroke_set:
						continue
					edge_mid=((world_matrix @ edge.verts[0].co)
						+ (world_matrix @ edge.verts[1].co)) * 0.5
	...
	def _shift_partial_merge_allowed(self,context,pending_edges,connection_edges):
		...
		for _decal_obj,stroke in self._iter_interactive_stroke_records(context):
			...
			if not (set(stroke_edges) & connect_set):
				continue
			shared_pending=self._shared_source_vertices(source_obj,list(pending_set),stroke_edges,)
			if not shared_pending:
				continue
			open_vertices,tip_vertices=self._stroke_merge_corners(source_obj,stroke,)
			if shared_pending & tip_vertices:
				return False,("Shift merge: connect at the partial open end, " "not the taper tip")
			if not (shared_pending & open_vertices):
				return False,("Shift merge: partial open end does not meet the pending run")
		return True, ""
	def _plan_auto_connect_to_pending_neighbor(self,context,new_edge_indices,edge_local_slice_interval=None,):
		...
		if hovered_edge in pending_edges:
			return None
		if not self._edges_share_vertex(source_obj,hovered_edge,
			pending_edges,):
			return None
		...
		if master is not None:
			...
			for _decal_obj,stroke in self._iter_neighbor_strokes_for_merge(context,new_edge_indices,):
				stroke_edges=[int(index) for index in stroke.get("edges",[])]
				if not (set(stroke_edges) & pending_set):
					continue
				if not self._merge_connection_allowed(source_obj,[hovered_edge],
					stroke,edge_local_slice_interval=edge_local_slice_interval,):
					return None
				pending_stroke_ids.append(str(stroke.get("id", "")))
				break
		anchor_edge=self._nearest_stroke_endpoint_edge(source_obj,pending_edges,hovered_edge,)
		if anchor_edge < 0:
		...
		for edge_index in pending_edges + list(new_edge_indices):
			...
			if edge_index not in seen:
				...
				seen.add(edge_index)
		return {"target_edges": ordered,"path": list(new_edge_indices), "anchor_edge": anchor_edge,"stroke_edges": pending_edges, "stroke_ids": pending_stroke_ids,}
	def _plan_auto_connect_to_touching_stroke(self,context,new_edge_indices,edge_local_slice_interval=None,):
		...
		for _decal_obj,stroke in self._iter_neighbor_strokes_for_merge(context,new_edge_indices,):
			...
			if new_edges <=set(stroke_edges):
				continue
			new_endpoints=self._open_endpoint_vertices_for_edges(source_obj,new_edges,)
			if not new_endpoints:
				continue
			slice_interval=stroke.get("slice_interval")
			has_slice=(isinstance(slice_interval,(list,tuple))
				and len(slice_interval)==2)
			shares_vertex=self._edges_share_vertex(source_obj,hovered_edge,stroke_edges,)
			if edge_local_slice_interval is not None and not shares_vertex:
				continue
			if not shares_vertex:
				stroke_endpoints=self._stroke_connectable_vertices(source_obj,stroke,)
				if not self._endpoints_can_connect(source_obj,stroke_endpoints,
					new_endpoints,stroke_edges,):
					continue
			stroke_open,_stroke_tips=self._stroke_merge_corners(source_obj,stroke,)
			if shares_vertex:
				shared_vertices=self._shared_source_vertices(source_obj,list(new_edge_indices),stroke_edges,)
				anchor_vertices=stroke_open & shared_vertices
				if not anchor_vertices:
					anchor_vertices=stroke_open or None
			else:
				anchor_vertices=(self._stroke_connectable_vertices_near(source_obj,stroke,list(new_edge_indices),) if has_slice else None)
			anchor_edge=self._nearest_stroke_endpoint_edge(source_obj,stroke_edges,hovered_edge,connectable_vertices=anchor_vertices,)
			if anchor_edge < 0:
				continue
			if shares_vertex:
				if not self._merge_connection_allowed(source_obj,[hovered_edge],
			...
			if shares_vertex:
				...
				for edge_index in stroke_edges + list(new_edge_indices):
					...
					if edge_index not in seen:
						ordered.append(edge_index)
						seen.add(edge_index)
				return {"target_edges": ordered,"path": list(new_edge_indices), "anchor_edge": anchor_edge,"stroke_edges": stroke_edges, "stroke_ids": [str(stroke.get("id", ""))],}
			path=self._edge_path_between(source_obj,anchor_edge,hovered_edge,)
			if not path:
				continue
			...
			for edge_index in stroke_edges + path + list(new_edge_indices):
				...
				if edge_index not in seen:
					ordered.append(edge_index)
					seen.add(edge_index)
			return {"target_edges": ordered,"path": path, "anchor_edge": anchor_edge,"stroke_edges": stroke_edges, "stroke_ids": [str(stroke.get("id", ""))],}
		return None
	...
	def _normalized_slice_interval(self,slice_interval):
		if not (isinstance(slice_interval,(list,tuple))
			and len(slice_interval)==2):
			return None
	...
	def _partial_strokes_can_share_source_edge(self,existing_stroke,new_edge_indices,new_edge_local_interval,source_obj=None,):
		new_edges={int(index) for index in (new_edge_indices or [])}
		existing_edges={int(index) for index in existing_stroke.get("edges",[])}
		if (len(new_edges) !=1
			or not (existing_edges & new_edges)
			or new_edge_local_interval is None):
			return False
		target_edge_index=next(iter(new_edges))
		existing_interval=self._stroke_edge_local_coverage(source_obj,existing_stroke,target_edge_index,)
		if existing_interval is None:
			return False
		return not self._slice_intervals_overlap(existing_interval,new_edge_local_interval,)
	def _stroke_edge_local_coverage(self,source_obj,stroke,edge_index):
		...
		if target_edge_index not in stroke_edges:
			return None
		explicit_interval=self._normalized_slice_interval(stroke.get("edge_local_slice_interval"))
		try:
			explicit_edge_index=int(stroke.get("edge_local_slice_edge_index",-1))
		except (TypeError,ValueError):
			explicit_edge_index=-1
		if explicit_interval is not None and (explicit_edge_index==target_edge_index
			or len(stroke_edges)==1):
			return explicit_interval
		chain_interval=self._normalized_slice_interval(stroke.get("slice_interval"))
		...
		bm,owns_bm=self._open_bmesh_for_read(source_obj)
		try:
			selected=[bm.edges[index] for index in sorted(stroke_edges) if 0 <=index < len(bm.edges)]
			chains=extract_edge_chains(selected,source_obj.matrix_world)
			chain=next((item
					for item in chains
					if any(edge.index==target_edge_index for edge in item[1])),None,)
			if chain is None:
				return explicit_interval
			...
			if closed or not chain_edges:
				return explicit_interval
			world_matrix=source_obj.matrix_world
			lengths=[(world_matrix @ chain_verts[index + 1].co
					- world_matrix @ chain_verts[index].co).length for index in range(len(chain_edges))]
			total_length=sum(lengths)
			if total_length <=EPSILON:
				return explicit_interval
			edge_position=next(index
				for index,edge in enumerate(chain_edges) if edge.index==target_edge_index)
			distance_before=sum(lengths[:edge_position])
			edge_length=lengths[edge_position]
			if edge_length <=EPSILON:
				return 0.0,0.0
			coverage_start=max(chain_interval[0] * total_length,distance_before,)
			coverage_end=min(chain_interval[1] * total_length,distance_before + edge_length,)
			...
			if (chain_verts[edge_position].index==source_edge.verts[0].index):
				return local_start,local_end
			return 1.0 - local_end,1.0 - local_start
		finally:
			if owns_bm:
				bm.free()
	def _full_edge_groups_form_junction(self,source_obj,clicked_edge_indices,stroke_edge_indices,shared_vertex_indices,):
		clicked={int(index) for index in (clicked_edge_indices or [])}
		...
		try:
			for vertex_index in shared:
				if not (0 <=vertex_index < len(bm.verts)):
					continue
				incident={edge.index for edge in bm.verts[vertex_index].link_edges}
				clicked_at_vertex=incident & clicked
				existing_at_vertex=incident & existing
				if (clicked_at_vertex
			...
			if owns_bm:
				bm.free()
	def _edge_local_slice_to_chain_interval(self,source_obj,edge_index,local_interval,):
		...
		try:
			...
			for chain_verts,_chain_edges,closed in chains:
				...
				if chain_start==v1 and chain_end==v0:
					return (max(0.0,min(1.0,1.0 - end_fraction)),max(0.0,min(1.0,1.0 - start_fraction)),)
			return normalized
		finally:
			if owns_bm:
				bm.free()
	def _normalize_slice_interval_for_edges(self,source_obj,edge_indices,slice_interval,*,interval_space="chain",):
		...
		if len(unique_edges) !=1 or source_obj is None:
			return normalized
		return self._edge_local_slice_to_chain_interval(source_obj,unique_edges[0],normalized,)
	def _chain_interval_to_edge_local(self,source_obj,edge_index,chain_interval,):
		...
		try:
			...
			if owns_bm:
				bm.free()
	def _partial_merge_corners_from_edge(self,source_obj,edge_index,edge_local_interval,):
		...
		try:
			...
			if owns_bm:
				bm.free()
	def _full_stroke_merge_corners(self,source_obj,edge_indices,taper_vertices=None,):
		edge_indices=[int(index) for index in (edge_indices or [])]
		if not edge_indices or source_obj is None:
			return set(),set()
		bm,owns_bm=self._open_bmesh_for_read(source_obj)
		try:
			edges=[bm.edges[index] for index in edge_indices if 0 <=index < len(bm.edges)]
			if not edges:
				return set(),set()
			chains=extract_edge_chains(edges,source_obj.matrix_world)
			open_vertices=set()
			...
			tip_vertices={int(index) for index in (taper_vertices or [])}
			open_vertices -=tip_vertices
			return open_vertices,tip_vertices
		finally:
			if owns_bm:
				bm.free()
	def _rebuild_merge_corners_for_stroke(self,source_obj,stroke,edge_local_slice_interval=None,):
		stroke_edges=[int(index) for index in stroke.get("edges",[])]
		...
		if edge_local is not None and len(stroke_edges)==1:
			return self._partial_merge_corners_from_edge(source_obj,stroke_edges[0],edge_local,)
		chain_interval=self._normalized_slice_interval(stroke.get("slice_interval"))
		if chain_interval is not None:
			...
			try:
				...
				if owns_bm:
					bm.free()
		return self._full_stroke_merge_corners(source_obj,stroke_edges,stroke.get("taper_vertices",[]),)
	def _stroke_merge_corners(self,source_obj,stroke):
		open_vertices,tip_vertices=self._rebuild_merge_corners_for_stroke(source_obj,stroke,)
		return set(open_vertices),set(tip_vertices)
	def _new_click_merge_corners(self,source_obj,edge_indices,edge_local_slice_interval=None,):
		edge_indices=[int(index) for index in (edge_indices or [])]
		if not edge_indices or source_obj is None:
			return set(),set()
		if edge_local_slice_interval is not None and len(edge_indices)==1:
			return self._partial_merge_corners_from_edge(source_obj,edge_indices[0],edge_local_slice_interval,)
	...
	def _stroke_taper_tip_vertices(self,source_obj,stroke):
		_open_vertices,tip_vertices=self._stroke_slice_corner_vertices(source_obj,stroke,)
		return tip_vertices
	def _stroke_merge_open_vertices(self,source_obj,stroke):
		open_vertices,_tip_vertices=self._stroke_slice_corner_vertices(source_obj,stroke,)
	...
	def _merge_connection_allowed(self,source_obj,clicked_edge_indices,stroke,edge_local_slice_interval=None,):
		stroke_edges=[int(index) for index in stroke.get("edges",[])]
		...
		if not self._edge_groups_touch(source_obj,clicked_edges,stroke_edges):
			return False
		shared=self._shared_source_vertices(source_obj,clicked_edges,stroke_edges,)
		if not shared:
			return False
		exist_open,exist_tips=self._stroke_merge_corners(source_obj,stroke)
		new_open,new_tips=self._new_click_merge_corners(source_obj,clicked_edges,edge_local_slice_interval=edge_local_slice_interval,)
		if edge_local_slice_interval is not None:
		...
		if shared & new_tips:
			return False
		if not (shared & exist_open) and not (shared & exist_tips and not existing_is_partial):
			full_edge_junction=(edge_local_slice_interval is None
				and not existing_is_partial
				and self._full_edge_groups_form_junction(source_obj,clicked_edges,stroke_edges,shared,))
			if not full_edge_junction:
				return False
		if edge_local_slice_interval is not None and not (shared & new_open):
			return False
		return True
	def _partial_merge_chain_interval(self,context,target_edges,partial_strokes,):
		source_obj=self._source_object(context)
		target_indices=sorted({int(index) for index in (target_edges or [])})
		if source_obj is None or not target_indices:
			return None
		if not any(self._stroke_is_partial_slice(stroke)
			for stroke in (partial_strokes or [])):
			return None
		bm,owns_bm=self._open_bmesh_for_read(source_obj)
		try:
			selected=[bm.edges[index] for index in target_indices if 0 <=index < len(bm.edges)]
			target_chains=extract_edge_chains(selected,source_obj.matrix_world,)
			...
			if target_closed or not target_chain_edges:
				return None
			world_matrix=source_obj.matrix_world
			target_lengths=[(world_matrix @ target_verts[index + 1].co
					- world_matrix @ target_verts[index].co).length for index in range(len(target_chain_edges))]
			target_total=sum(target_lengths)
			if target_total <=EPSILON:
				return None
			target_positions={edge.index: position for position,edge in enumerate(target_chain_edges)}
			target_before=[]
			running=0.0
			...
			def map_stroke_fraction(stroke_verts,stroke_edges,fraction):
				stroke_lengths=[(world_matrix @ stroke_verts[index + 1].co
						- world_matrix @ stroke_verts[index].co).length for index in range(len(stroke_edges))]
				...
				if stroke_start==target_start and stroke_end==target_end:
					...
					return None
				return (target_before[target_position]
					+ target_lengths[target_position] * target_factor) / target_total
			coverage=[]
			covered_edges=set()
			for stroke in partial_strokes or []:
				stroke_indices=sorted({int(index)
				...
				if stroke_closed:
					return None
				interval=self._normalized_slice_interval(stroke.get("slice_interval"))
				edge_local=self._normalized_slice_interval(stroke.get("edge_local_slice_interval"))
				if edge_local is not None and len(stroke_chain_edges)==1:
					...
					if (start_vertex==edge.verts[0].index
						and end_vertex==edge.verts[1].index):
						interval=edge_local
					else:
						interval=(1.0 - edge_local[1],1.0 - edge_local[0],)
				if interval is None:
					interval=(0.0,1.0)
				coverage_start=map_stroke_fraction(stroke_verts,stroke_chain_edges,interval[0],)
				coverage_end=map_stroke_fraction(stroke_verts,stroke_chain_edges,interval[1],)
				if coverage_start is None or coverage_end is None:
					return None
				coverage.append((min(coverage_start,coverage_end),max(coverage_start,coverage_end),))
			for edge in target_chain_edges:
				if edge.index in covered_edges:
					continue
				position=target_positions[edge.index]
				coverage.append((target_before[position] / target_total,(target_before[position] + target_lengths[position]) / target_total,))
			if not coverage:
				return None
			...
			if owns_bm:
				bm.free()
	def _compute_taper_tip_vertices(self,context,target_edges,slice_interval=None,merge_taper_start=False,merge_taper_end=False,force_all=False,):
		source_obj=self._source_object(context)
		...
		bm,owns_bm=self._open_bmesh_for_read(source_obj)
		try:
			selected=[bm.edges[int(index)] for index in target_edges if 0 <=int(index) < len(bm.edges)]
			if not selected:
				return set()
			world_matrix=source_obj.matrix_world
	...
	def _active_layer_source_edge_indices(self,context):
		...
		if layer_obj is None:
			return set()
		indices=set(parsed_source_indices(layer_obj.edge_decal_object_settings))
		for stroke in self._load_interactive_strokes(layer_obj.name_full):
			indices.update(int(index)
				for index in stroke.get("edges",[]))
		return indices
	def _connected_active_layer_edge_component(self,context,seed_edge_indices,):
		source_obj=self._source_object(context)
		...
		bm,owns_bm=self._open_bmesh_for_read(source_obj)
		try:
			valid_edges={index: bm.edges[index] for index in candidates if 0 <=index < len(bm.edges)}
			vertex_edges={}
			...
			if owns_bm:
				bm.free()
	def _clear_active_layer_for_atomic_rebuild(self,context,target_edge_indices,):
		layer_obj=self._interactive_layer_object(context)
		source_obj=self._source_object(context)
		...
		return True
	def _plan_click_merge_group(self,context,hovered_edge_index,clicked_edge_indices=None,edge_local_slice_interval=None,):
		...
		strokes=[stroke for _decal_obj,stroke in self._iter_interactive_stroke_records(context,)]
		if not strokes:
			return None
		mergeable_strokes=[]
		for stroke in strokes:
			...
			if overlaps_click:
				continue
			if self._merge_connection_allowed(source_obj,clicked_edges,
				stroke,edge_local_slice_interval=edge_local_slice_interval,):
				mergeable_strokes.append(stroke)
		...
		return {"target_edges": component,"path": list(clicked_edges),"anchor_edge": hovered_edge,"stroke_edges": sorted(existing_edges), "stroke_ids": [str(stroke.get("id", "")) for stroke in mergeable_strokes], "atomic_component_rebuild": False,}
	def _collect_strokes_for_edges(self,context,edge_indices):
		...
		return collected
	def _compute_merge_endpoint_tapers(self,context,target_edges,absorbed_strokes,):
		...
		try:
			...
			for chain_verts,_chain_edges,closed in chains:
				...
				for stroke in absorbed_strokes or []:
					_open_corners,tip_corners=(self._stroke_slice_corner_vertices( source_obj,stroke,))
			...
			if owns_bm:
				bm.free()
	def _absorb_geometry_for_target_edges(self,context,edge_indices,extra_vertex_indices=None,geometry_candidate_edges=None,skip_promote=False,):
		...
		if source_obj is None:
			return target_set
		restrict_vertices={int(index) for index in (extra_vertex_indices or [])}
		layer_obj=self._interactive_layer_object(context)
		if layer_obj is None:
			return target_set
		decal_obj=layer_obj
		...
		if geometry_candidate_edges is not None:
			eligible_edge_indices=sorted({int(index) for index in geometry_candidate_edges})
		candidate_edge_indices=sorted(target_set)
		if data is not None and data.initialized:
			candidate_edge_indices=sorted(set(parsed_source_indices(data)) | target_set)
		self._remove_decal_geometry_for_source_edges(context,decal_obj,list(target_set),candidate_edge_indices=candidate_edge_indices,restrict_vertex_indices=restrict_vertices or None,source_obj=source_obj,eligible_edge_indices=eligible_edge_indices,)
		...
		if not skip_promote:
			for edge_index in sorted(target_set):
				if not self._edge_has_generated_stroke(edge_index):
					continue
				_decal_obj,interactive_stroke=(self._find_interactive_stroke_by_edge(edge_index))
				if interactive_stroke is not None:
					continue
				self._promote_automatic_edge_to_interactive(context,edge_index,)
		return target_set
	def _iter_source_decals_for_edges(self,source_obj):
		...
		interactive_edges={int(index) for stroke in strokes for index in stroke.get("edges",[])}
		data=getattr(layer_obj, "edge_decal_object_settings",None)
		layer_indices=(set(parsed_source_indices(data)) if data is not None else set())
	...
	def _remove_specific_interactive_strokes(self,context,decal_obj,strokes,):
		if decal_obj is None or not strokes:
			return False
		stroke_ids={str(stroke.get("id", "")) for stroke in strokes}
		edge_indices=sorted({int(index)
			for stroke in strokes
			for index in stroke.get("edges",[])})
		vertex_indices=sorted({int(index)
			for stroke in strokes
		...
		candidate_edge_indices=sorted(set(parsed_source_indices(data))
			| {int(index) for stroke in layer_strokes for index in stroke.get("edges",[])}
			| set(edge_indices))
		removed=self._remove_decal_geometry_for_source_edges(context,decal_obj,edge_indices,candidate_edge_indices=candidate_edge_indices,restrict_vertex_indices=vertex_indices,)
		if not removed:
			self._remove_master_vertices(object_name,vertex_indices)
		remaining_strokes=[stroke for stroke in self._load_interactive_strokes(object_name) if str(stroke.get("id", "")) not in stroke_ids]
		self._save_interactive_strokes(object_name,remaining_strokes)
		remaining_interactive_edges={int(index) for stroke in remaining_strokes for index in stroke.get("edges",[])}
		base_edges=set()
		base_raw=str(decal_obj.get("edge_decal_base_source_indices", ""))
		if base_raw:
			try:
				base_edges={int(token) for token in base_raw.split(",") if token.strip()}
			except (TypeError,ValueError):
				base_edges=set()
	...
	def _edge_has_automatic_generated_stroke(self,edge_index):
		...
		if target_index not in layer_indices:
			return False
		interactive_edges={int(index) for stroke in self._load_interactive_strokes(layer_obj.name_full) for index in stroke.get("edges",[])}
	...
	def _automatic_decal_for_edge(self,context,edge_index):
		...
		if base_raw:
			try:
				base_indices={int(token) for token in base_raw.split(",") if token.strip()}
			except Exception:
				base_indices=all_indices - interactive_edges
		else:
			base_indices=all_indices - interactive_edges
		if target_index in base_indices or (target_index in all_indices
	...
	def _nearest_decal_component_vertices(self,context,decal_obj,edge_index):
		...
		try:
			...
			return []
		source_a=source_obj.matrix_world @ source_obj.data.vertices[source_edge.vertices[0]].co
		source_b=source_obj.matrix_world @ source_obj.data.vertices[source_edge.vertices[1]].co
		...
		for component in components:
			...
			score=min(point_segment_distance_sq( decal_obj.matrix_world @ mesh.vertices[index].co) for index in sample_indices)
			if score < best_score:
				best_score=score
		...
		tolerance=max(0.02,face_width * 5.0,surface_offset * 12.0,segment_length * 0.35,)
	...
	def _promote_automatic_edge_to_interactive(self,context,edge_index):
		...
		target_index=int(edge_index)
		data=decal_obj.edge_decal_object_settings
		interactive_edges={int(index) for stroke in self._load_interactive_strokes(decal_obj.name_full) for index in stroke.get("edges",[])}
		base_raw=str(decal_obj.get("edge_decal_base_source_indices", ""))
		if base_raw:
			try:
				base_indices={int(token) for token in base_raw.split(",") if token.strip()}
		...
		removed=self._remove_decal_geometry_for_source_edges(context,decal_obj,[target_index],candidate_edge_indices=candidate_edge_indices,source_obj=source_obj,)
		if not removed:
			component_vertices=self._nearest_decal_component_vertices(context,decal_obj,edge_index,)
			if component_vertices:
				removed=self._remove_master_vertices(decal_obj.name_full,component_vertices,)
		if not removed:
			return decal_obj,False
		stored_indices=[index for index in parsed_source_indices(data) if index !=target_index]
		set_stored_source_indices(decal_obj,stored_indices)
		base_indices.discard(target_index)
		decal_obj["edge_decal_base_source_indices"]= ",".join(str(index) for index in sorted(base_indices))
		return decal_obj,True
	def _remove_decal_geometry_for_source_edges(self,context,decal_obj,target_edge_indices,candidate_edge_indices=None,restrict_vertex_indices=None,source_obj=None,eligible_edge_indices=None,):
		if source_obj is None:
			source_obj=self._source_object(context)
		...
		if mesh is None or not mesh.polygons:
			return False
		try:
			target_edge_set={int(index) for index in (target_edge_indices or [])}
			if candidate_edge_indices is None:
				candidate_edge_indices=sorted(target_edge_set)
			else:
				candidate_edge_indices=sorted({int(index) for index in candidate_edge_indices})
			eligible_edge_set=None
			if eligible_edge_indices is not None:
				eligible_edge_set={int(index) for index in eligible_edge_indices}
		except (TypeError,ValueError):
			return False
		...
		for polygon in mesh.polygons:
			...
			if restricted and not polygon_vertices.intersection(restricted):
				continue
			center_local=sum((mesh.vertices[index].co for index in polygon.vertices),Vector((0.0,0.0,0.0)),) / max(len(polygon.vertices),1)
			center_world=decal_obj.matrix_world @ center_local
			nearest_index=min(source_segments,
				key=lambda index: distance_sq_to_segment(center_world,*source_segments[index]),)
			if nearest_index not in target_edge_set:
				continue
			if (eligible_edge_set is not None
		...
		for polygon in mesh.polygons:
			...
			if uv_layer is not None:
				kept_face_uvs.append([uv_layer.data[loop_index].uv.copy() for loop_index in polygon.loop_indices])
			else:
		...
		for vertex in mesh.vertices:
			if vertex.index not in old_to_new:
				continue
			for membership in vertex.groups:
				group_memberships.setdefault(membership.group,[]).append((old_to_new[vertex.index],membership.weight))
		...
		for polygon,material_index,smooth in zip(replacement.polygons,kept_material_indices,kept_smooth):
			polygon.material_index=material_index
			polygon.use_smooth=smooth
		new_uv=replacement.uv_layers.new(name=uv_layer.name if uv_layer is not None else "UVMap")
		...
		for stroke in strokes:
			stroke["vertices"]=sorted(old_to_new[index]
				for index in stroke.get("vertices",[]) if index in old_to_new)
		self._save_interactive_strokes(decal_obj.name_full,strokes)
		if old_mesh.users==0:
			bpy.data.meshes.remove(old_mesh)
		return True
	def _remove_source_edge_geometry_in_place(self,decal_obj,source_obj,target_edge_index,candidate_edge_indices,restrict_vertex_indices=None,):
		return self._remove_decal_geometry_for_source_edges(bpy.context,decal_obj,[int(target_edge_index)],candidate_edge_indices=candidate_edge_indices,restrict_vertex_indices=restrict_vertex_indices,source_obj=source_obj,)
	def _rebuild_full_stroke_after_edge_removal(self,context,layer_obj,stroke,target_edge_index,):
		...
		if target_edge_index not in seen or len(original_edges) <=1:
			return None
		remaining_edges=[edge_index for edge_index in original_edges if edge_index !=target_edge_index]
		if not remaining_edges:
			return None
		merge_taper_start,merge_taper_end=(self._compute_merge_endpoint_tapers(context,remaining_edges,[stroke],))
		if not self._remove_specific_interactive_strokes(context,layer_obj,
			[stroke],):
			return False
		...
		try:
			success,_reason,generated_name=self._generate_from_edge_indices(context,remaining_edges,force_connected=True,slice_interval=None,merge_taper_start=merge_taper_start,merge_taper_end=merge_taper_end,)
		finally:
			self._interactive_face_width=previous_width
		if not success:
			return False
		merged_name,new_stroke_vertices=self._merge_new_decal_into_master(generated_name,vertex_count_before=vertex_count_before,)
		if not merged_name:
			return False
		new_taper_vertices=self._compute_taper_tip_vertices(context,remaining_edges,merge_taper_start=merge_taper_start,merge_taper_end=merge_taper_end,)
		self._register_interactive_stroke(merged_name,remaining_edges,new_stroke_vertices,force_connected=True,slice_interval=None,face_width=(stroke_width if stroke_width > 0.0 else previous_width),taper_vertices=new_taper_vertices,)
		return True
	def _connected_generated_edge_indices(self,source_obj,layer_obj,edge_index,bridge_removed_seed=False,):
		...
		if mesh is None or data is None:
			return set()
		generated={int(index) for index in parsed_source_indices(data) if 0 <=int(index) < len(mesh.edges)}
		generated.update(int(index)
			for stroke in self._load_interactive_strokes(layer_obj.name_full)
			for index in stroke.get("edges",[])
		...
		return connected
	def _remove_connected_edges_from_active_layer(self,context,edge_index,bridge_removed_seed=False,record_history=True,):
		source_obj=self._source_object(context)
		layer_obj=bpy.data.objects.get(self._interactive_layer_name)
		if (source_obj is None
			...
			or layer_obj.get("edge_decal_source") !=source_obj.name_full):
			return set()
		target_indices=self._connected_generated_edge_indices(source_obj,layer_obj,edge_index,bridge_removed_seed=bridge_removed_seed,)
		if not target_indices:
			return set()
		return self._remove_generated_edge_indices_from_active_layer(context,target_indices,record_history=record_history,)
	def _remove_generated_edge_indices_from_active_layer(self,context,target_indices,record_history=True,):
		source_obj=self._source_object(context)
		layer_obj=bpy.data.objects.get(self._interactive_layer_name)
		if (source_obj is None
			or layer_obj is None
		...
		data=layer_obj.edge_decal_object_settings
		all_indices=set(parsed_source_indices(data))
		all_indices.update(int(index)
			for stroke in self._load_interactive_strokes(layer_obj.name_full)
			for index in stroke.get("edges",[]))
		target_indices={int(index) for index in (target_indices or [])}.intersection(all_indices)
		if not target_indices:
			return set()
		...
		backup=(self._make_object_backup(layer_obj.name_full) if record_history else None)
		removed=self._remove_decal_geometry_for_source_edges(context,layer_obj,sorted(target_indices),candidate_edge_indices=sorted(all_indices),source_obj=source_obj,)
		if not removed:
		...
		interactive_edges={int(index) for stroke in remaining_strokes for index in stroke.get("edges",[])}
		base_raw=str(layer_obj.get("edge_decal_base_source_indices", ""))
		if base_raw:
			try:
				base_indices={int(token) for token in base_raw.split(",") if token.strip()}
		...
		layer_obj["edge_decal_base_source_indices"]= ",".join(str(index) for index in sorted(base_indices))
		...
		if record_history:
			self._action_history.append({"created_name": layer_obj.name_full,"deleted_backup": backup,"previous_pending": previous_pending,"previous_anchor": previous_anchor, "previous_last_name": layer_obj.name_full, "previous_last_stroke_vertices": previous_last_vertices,})
		if self._remove_mode:
	...
	def _remove_click_matches_last(self,context,event):
		if (self._last_remove_click_edge_index < 0
			or not self._action_history
			or self._action_history[-1] is not self._last_remove_action):
			return False
		double_click_delay=max(0.25,float(context.preferences.inputs.mouse_double_click_time) / 1000.0 + 0.05,)
		previous_x,previous_y=self._last_remove_click_mouse_region
		mouse_distance_sq=((event.mouse_region_x - previous_x) ** 2
			+ (event.mouse_region_y - previous_y) ** 2)
		return bool(time.monotonic() - self._last_remove_click_time <=double_click_delay and mouse_distance_sq <=144.0)
	def _expand_last_edge_removal(self,context):
		...
		removed_indices=self._remove_generated_edge_indices_from_active_layer(context,remainder,record_history=False,)
		removed_count=1 + len(removed_indices)
		self._clear_last_remove_click()
		GR(self,"INFO",f"Removed {removed_count} connected edge(s)",)
	...
	def _commit_connected_edge_removal(self,context,target_index):
		connected_indices=self._remove_connected_edges_from_active_layer(context,target_index,bridge_removed_seed=False,record_history=True,)
		if not connected_indices:
			GR(self,"WARNING","That edge is not part of the selected decal layer",)
			return False
		GR(self,"INFO",f"Removed {len(connected_indices)} connected edge(s)",)
		self._suppress_hover_until_mousemove=True
		self._hovered_edge_index=-1
		self._hovered_edge_indices=[]
		self._hovered_edge_points=None
	...
	def _remove_edge_from_active_layer(self,context,edge_index):
		...
		previous_last_vertices=list(self._last_stroke_vertex_indices or [])
		strokes=self._load_interactive_strokes(layer_obj.name_full)
		target_stroke_position=next((position
				for position,stroke in enumerate(strokes)
				if target_index in stroke.get("edges",[])),-1,)
		target_stroke=(strokes[target_stroke_position] if target_stroke_position >=0 else None)
		rebuilt_stroke=self._rebuild_full_stroke_after_edge_removal(context,layer_obj,target_stroke,target_index,)
		...
		if not self._remove_source_edge_geometry_in_place(layer_obj,source_obj,target_index,candidates,
		...
		remaining_interactive_edges={int(index) for stroke in cleaned_strokes for index in stroke.get("edges",[])}
		base_raw=str(layer_obj.get("edge_decal_base_source_indices", ""))
		if base_raw:
			try:
				base_indices={int(token) for token in base_raw.split(",") if token.strip()}
		...
		remaining_indices=[index for index in all_indices if index !=target_index or target_index in retained_edges]
		...
		self._action_history.append({"created_name": layer_obj.name_full,"deleted_backup": backup,"previous_pending": previous_pending,"previous_anchor": previous_anchor, "previous_last_name": layer_obj.name_full, "previous_last_stroke_vertices": previous_last_vertices,})
		return True
	def _remove_interactive_stroke_by_edge(self,context,edge_index,record_history=True,):
		...
		if decal_obj is None or stroke is None:
			...
			if target_index not in all_indices:
				return False
			interactive_edges={int(index) for entry in self._load_interactive_strokes(decal_obj.name_full) for index in entry.get("edges",[])}
			...
			decal_obj["edge_decal_base_source_indices"]= ",".join(str(index) for index in sorted(set(base_indices)))
			...
			if "FINISHED" not in result:
				...
				return False
			self._action_history.append({"created_name": decal_obj.name_full,"deleted_backup": backup,"previous_pending": list(self._pending_edge_indices or []),"previous_anchor": self._path_anchor_edge_index,"previous_last_name": self._last_generated_object_name,"previous_last_stroke_vertices": list(self._last_stroke_vertex_indices or []),})
		...
		removed=self._remove_decal_geometry_for_source_edges(context,decal_obj,stroke_edges,candidate_edge_indices=stroke_edges,restrict_vertex_indices=stroke.get("vertices",[]),)
		if not removed:
			self._remove_master_vertices(object_name,stroke.get("vertices",[]),)
		self._remove_interactive_stroke_record(object_name,stroke_id=stroke.get("id", ""),)
		...
		if self._last_generated_object_name==object_name:
			self._last_stroke_vertex_indices=[]
		if record_history:
			self._action_history.append({"created_name": modified_name,"deleted_backup": backup,"previous_pending": list(self._pending_edge_indices or []),"previous_anchor": self._path_anchor_edge_index, "previous_last_name": self._last_generated_object_name, "previous_last_stroke_vertices": previous_last_vertices,})
		else:
			self._discard_object_backup(backup)
	...
	def _merge_new_decal_into_master(self,generated_name,vertex_count_before=None):
		def finalize_master_state(master_obj):
			...
			if source_obj is not None and master_obj is not None:
				...
				apply_decal_normal_settings(master_obj,settings.normal_mode,settings.normal_keep_sharp,settings.normal_weight,settings.normal_threshold,)
				ensure_decal_match_source_bevel(master_obj,source_obj)
				ensure_decal_finish_modifiers(master_obj,source_obj,settings,)
		...
		if master is None:
			source_obj=self._source_object(bpy.context)
			if source_obj is not None:
				master=active_decal_layer_for_source(source_obj,include_locked=False,)
		if master is None:
			return "",[]
		vertex_count_after=len(master.data.vertices)
		start_vertex=(int(vertex_count_before) if vertex_count_before is not None else vertex_count_after)
	...
	def _collect_remove_components(self,context):
		...
		for decal_obj in iter_generated_decals(source_obj=source_obj):
			...
			while unvisited:
				...
				components.append({"object_name": decal_obj.name_full,"vertices": vertex_indices, "center": center,})
		return components
	def _project_remove_targets(self,context):
		...
		for index,component in enumerate(self._remove_components or []):
			point_2d=view3d_utils.location_3d_to_region_2d(context.region,context.space_data.region_3d,component["center"],)
	...
	def _remove_component(self,context,component):
		...
		try:
			bm.from_mesh(obj.data)
			bm.verts.ensure_lookup_table()
			vertices=[bm.verts[index]
				for index in component.get("vertices",[]) if 0 <=index < len(bm.verts)]
			if not vertices:
				self._discard_object_backup(backup)
				return False
			bmesh.ops.delete(bm,geom=vertices,context="VERTS")
		...
		self._action_history.append({"created_name": modified_name,"deleted_backup": backup,"previous_pending": list(self._pending_edge_indices or []),"previous_anchor": self._path_anchor_edge_index,"previous_last_name": self._last_generated_object_name,"previous_last_stroke_vertices": list(self._last_stroke_vertex_indices or []),})
		self._last_stroke_vertex_indices=[]
		self._remove_components=self._collect_remove_components(context)
		self._hovered_remove_index=-1
		return True
	...
	def _finish(self,context):
		...
		if self._remove_draw_handle is not None:
			try:
				bpy.types.SpaceView3D.draw_handler_remove(self._remove_draw_handle,"WINDOW",)
			except Exception:
				pass
			self._remove_draw_handle=None
		if self._help_draw_handle is not None:
			try:
				bpy.types.SpaceView3D.draw_handler_remove(self._help_draw_handle,"WINDOW",)
			except Exception:
				pass
			self._help_draw_handle=None
		if self._interactive_event_timer is not None:
			try:
				context.window_manager.event_timer_remove(self._interactive_event_timer)
			except Exception:
				pass
		...
		if context.area:
			context.area.tag_redraw()
	def _execute_generator(self,context,safe_fallback=False,force_connected=False,generate_selected_graph=False,slice_interval=None,face_width=None,merge_taper_start=False,merge_taper_end=False,):
		...
		try:
			return bpy.ops.mesh.generate_edge_decal_strips("EXEC_DEFAULT",face_width=(max(MIN_FACE_WIDTH,float(face_width))
					if face_width is not None
					else settings.face_width),randomize_face_width=settings.randomize_face_width,minimum_face_width=settings.minimum_face_width,maximum_face_width=settings.maximum_face_width,crevice_removal=(0.0
					if safe_fallback or force_connected
					else settings.crevice_removal),crevice_detection_mode=settings.crevice_detection_mode,crevice_ao_distance=settings.crevice_ao_distance,crevice_ao_samples=settings.crevice_ao_samples,remove_short_edges=(False
					if safe_fallback or force_connected
					else settings.remove_short_edges),minimum_edge_length=settings.minimum_edge_length,minimum_length_per_edge=False,decal_amount=(1.0
					if (safe_fallback or force_connected or slice_interval is not None)
					else settings.decal_amount),edge_slice=settings.edge_slice,interactive_slice_start=(slice_interval[0] if slice_interval else -1.0),interactive_slice_end=(slice_interval[1] if slice_interval else -1.0),interactive_detect_endpoint_taper=True,interactive_force_endpoint_taper=self._endpoint_taper_enabled,interactive_merge_taper_start=bool(merge_taper_start),interactive_merge_taper_end=bool(merge_taper_end),interactive_skip_limited_dissolve=True,generate_selected_edge_graph=bool(generate_selected_graph),maximum_decal_length=(0.0
					if safe_fallback or force_connected
					else settings.maximum_decal_length),taper_sliced_ends=(True
					if slice_interval is not None
					else (False if force_connected else settings.taper_sliced_ends)),slice_taper_length=settings.slice_taper_length,auto_trim_corner_ends=settings.auto_trim_corner_ends,corner_end_trim_multiplier=settings.corner_end_trim_multiplier,randomize_horizontal_offset=settings.randomize_horizontal_offset,horizontal_randomize_amount=settings.horizontal_randomize_amount,seed=settings.seed,uv_scale=settings.uv_scale,auto_face_width=settings.auto_face_width,auto_width_samples=settings.auto_width_samples,auto_width_clearance=settings.auto_width_clearance,clamp_edge_overlaps=settings.clamp_edge_overlaps,overlap_clearance=settings.overlap_clearance,use_face_loop_slide=settings.use_face_loop_slide,fast_geometry_only=settings.fast_geometry_only,add_weld_modifier=settings.add_weld_modifier,add_bevel_modifier=settings.add_bevel_modifier,surface_offset=settings.surface_offset,)
		finally:
	...
	def _new_generated_object_name(self,source_obj,existing_names):
		candidates=[obj for obj in iter_generated_decals(source_obj=source_obj) if obj.name_full not in existing_names]
		if not candidates:
			return ""
	...
	def _make_object_backup(self,object_name):
		...
		return {"object": backup,"original_name": obj.name_full,"mesh_name": obj.data.name if obj.data is not None else "","collection_names": [collection.name for collection in obj.users_collection], "matrix_world": obj.matrix_world.copy(),"parent_name": obj.parent.name_full if obj.parent is not None else "","matrix_parent_inverse": obj.matrix_parent_inverse.copy(),"hide_viewport": obj.hide_viewport, "hide_render": obj.hide_render, "hide_set": obj.hide_get(),}
	...
	def _restore_object_backup(self,backup_data):
		...
		backup.matrix_parent_inverse=backup_data.get("matrix_parent_inverse",Matrix.Identity(4),)
	...
	def _undo_last_action(self,context):
		if not self._action_history:
			GR(self,"INFO", "Nothing to undo in Interactive Generate")
			return False
		...
		if backup:
			...
			restored_name= ""
		self._pending_edge_indices=list(action.get("previous_pending",[]))
		self._path_anchor_edge_index=action.get("previous_anchor",-1,)
		self._last_generated_object_name=(restored_name
			or action.get("previous_last_name", ""))
		self._interactive_layer_name=self._last_generated_object_name
		self._last_stroke_vertex_indices=list(action.get("previous_last_stroke_vertices",[]))
		source_obj=self._source_object(context)
		...
		if context.area:
			context.area.tag_redraw()
		GR(self,"INFO", "Undid last interactive generate/remove action")
		return True
	def _generate_from_edge_indices(self,context,edge_indices,force_connected=False,slice_interval=None,merge_taper_start=False,merge_taper_end=False,):
		global EDGEDECAL_STANDALONE_GENERATION
		generate_selected_graph=self._interactive_selection_uses_graph(edge_indices,force_connected=force_connected,slice_interval=slice_interval,)
		...
		try:
			try:
				result=self._execute_generator(context,safe_fallback=False,force_connected=force_connected,generate_selected_graph=generate_selected_graph,slice_interval=slice_interval,face_width=self._interactive_face_width,merge_taper_start=merge_taper_start,merge_taper_end=merge_taper_end,)
			except RuntimeError as error:
				result={"CANCELLED"}
			...
			if "FINISHED" not in result:
				...
				if source_obj is not None and self._select_edge_indices(source_obj,edge_indices) > 0:
					try:
						result=self._execute_generator(context,safe_fallback=True,force_connected=force_connected,generate_selected_graph=generate_selected_graph,slice_interval=slice_interval,face_width=self._interactive_face_width,merge_taper_start=merge_taper_start,merge_taper_end=merge_taper_end,)
					except RuntimeError as error:
						first_error=str(error)
						result={"CANCELLED"}
			generated_name= ""
			if "FINISHED" in result:
				source_obj=self._source_object(context)
				if source_obj is not None:
					generated_name=self._new_generated_object_name(source_obj,existing_names,)
		finally:
	...
	@staticmethod
	def _interactive_selection_uses_graph(edge_indices,force_connected=False,slice_interval=None,):
		return full_edge_selection_uses_graph(edge_indices,force_connected=force_connected,slice_interval=slice_interval,)
	def _connected_target_edges(self,context):
		...
		hovered_edges=list(self._hovered_edge_indices or [self._hovered_edge_index])
		if not self._pending_edge_indices:
		...
		if not path:
			...
			if anchor_edge >=0:
				path=self._edge_path_between(source_obj,anchor_edge,self._hovered_edge_index,)
		if path:
			...
			for _decal_obj,stroke in self._iter_interactive_stroke_records(context):
				...
				if not stroke_edges:
					continue
				if not self._edge_groups_touch(source_obj,stroke_edges,
					path_touch_edges,):
					continue
	...
	def invoke(self,context,event):
		...
		if source_obj is None:
			return {"CANCELLED"}
		ensure_source_decal_layers_ready(source_obj,context)
		sync_decal_bevel_from_source(source_obj,context.scene.edge_decal_settings,)
		global EDGEDECAL_INTERACTIVE_RUNNING
		EDGEDECAL_INTERACTIVE_RUNNING=True
		existing_master=active_decal_layer_for_source(source_obj,include_locked=False,)
		...
		existing_master_name=(existing_master.name_full if existing_master is not None else "")
		sync_fn=globals().get("sync_source_layer_ui")
		if sync_fn is not None:
		...
		self._interactive_face_width=max(MIN_FACE_WIDTH,float(context.scene.edge_decal_settings.face_width),)
		self._interactive_width_reference_size=source_mesh_max_dimension(source_obj)
		...
		self._draw_handle=bpy.types.SpaceView3D.draw_handler_add(self._draw_overlay,(context,), "WINDOW","POST_VIEW",)
		self._remove_draw_handle=None
		self._help_draw_handle=bpy.types.SpaceView3D.draw_handler_add(self._draw_help_overlay,(context,), "WINDOW","POST_PIXEL",)
		self._interactive_event_timer=context.window_manager.event_timer_add(0.03,window=context.window,)
		context.window_manager.modal_handler_add(self)
		if context.area:
			context.area.tag_redraw()
		GR(self,"INFO", "Interactive: click add/edit • R+click remove • R+double-click connected • H shortcuts",)
	...
	def modal(self,context,event):
		...
		if event.type != "TIMER" and mouse_x is not None and mouse_y is not None:
			over_viewport_window=any(region.type== "WINDOW" and region.x <=mouse_x < region.x + region.width and region.y <=mouse_y < region.y + region.height for region in context.area.regions)
		...
		if (not self._remove_mode
			...
			self._width_drag_start_value=max(MIN_FACE_WIDTH,float(self._interactive_face_width),)
			self._width_drag_current_value=self._width_drag_start_value
			GR(self,"INFO",f"Width: {self._width_drag_current_value:.4f} — use mouse wheel",)
			return {"RUNNING_MODAL"}
		if self._width_drag_active:
			if (event.type in {"WHEELUPMOUSE", "WHEELDOWNMOUSE"}
				and event.value== "PRESS"):
				multiplier=1.10 if event.type== "WHEELUPMOUSE" else (1.0 / 1.10)
				width=max(MIN_FACE_WIDTH,float(self._interactive_face_width) * multiplier,)
				self._set_interactive_face_width(context,width)
				self._width_drag_current_value=width
				self._update_hover(context)
				self._update_shift_preview(context)
				context.area.tag_redraw()
				GR(self,"INFO",f"Width: {width:.4f}")
				return {"RUNNING_MODAL"}
			...
			if event.type== "W" and event.value== "RELEASE":
				self._width_drag_active=False
				self._set_interactive_face_width(context,self._width_drag_current_value,)
				GR(self,"INFO",f"Width set to {self._width_drag_current_value:.4f}",)
				return {"RUNNING_MODAL"}
			if event.type== "ESC" and event.value== "PRESS":
				self._set_interactive_face_width(context,self._width_drag_start_value,)
				self._width_drag_current_value=self._width_drag_start_value
				self._width_drag_active=False
				self._update_hover(context)
				self._update_shift_preview(context)
				context.area.tag_redraw()
				GR(self,"INFO", "Width adjustment cancelled")
				return {"RUNNING_MODAL"}
			return {"RUNNING_MODAL"}
		self._sync_active_layer_from_scene(context)
		if (event.type== "E"
			...
			GR(self,"INFO",f"Endpoint taper: {state}")
			if context.area:
				context.area.tag_redraw()
			return {"RUNNING_MODAL"}
		if (event.type== "H"
		...
		if (event.type== "F"
			...
			GR(self,"INFO",f"Neighbor auto-merge: {state}")
			if context.area:
				context.area.tag_redraw()
			return {"RUNNING_MODAL"}
		...
		if (not self._remove_mode
			...
			self._update_shift_preview(context)
			context.area.tag_redraw()
			GR(self,"INFO",f"Ctrl segment length: {self._ctrl_fraction * 100:.0f}%",)
			return {"RUNNING_MODAL"}
		if (not self._remove_mode
		...
		if event.type== "R" and not event.ctrl and not event.alt:
			if event.value== "PRESS":
				if not self._r_remove_down:
					...
					self._mouse_region=(event.mouse_region_x,event.mouse_region_y,)
					self._update_hover(context)
					GR(self,"INFO", "Remove mode: click an edge,or double-click its connected component",)
					context.area.tag_redraw()
		...
		if (event.type== "LEFTMOUSE"
			...
			if target_index < 0:
				GR(self,"WARNING", "No generated edge under cursor")
				return {"RUNNING_MODAL"}
		...
		if (event.type== "LEFTMOUSE"
			...
			source_obj=self._source_object(context)
			layer_obj=bpy.data.objects.get(self._interactive_layer_name)
			connected_indices=self._connected_generated_edge_indices(source_obj,layer_obj,target_index,)
			...
			self._last_remove_click_mouse_region=(event.mouse_region_x,event.mouse_region_y,)
			self._last_remove_connected_indices=set(connected_indices)
			self._last_remove_action=(self._action_history[-1]
				if len(self._action_history or []) > history_size else None)
			GR(self,"INFO", "Removed edge")
			self._suppress_hover_until_mousemove=True
			self._hovered_edge_index=-1
			self._hovered_edge_indices=[]
			self._hovered_edge_points=None
		...
		if event.type== "LEFTMOUSE" and event.value== "PRESS":
			...
			now=time.monotonic()
			if (hovered_edge_index==self._last_click_edge_index
			...
			if self._edge_belongs_to_other_layer(context,hovered_edge_index):
				...
				if owner is not None and source_obj is not None and activate_fn is not None:
					activate_fn(context,source_obj,owner,select_source=False,select_layer=True,)
					...
					self._shift_connection_path=[]
					self._last_stroke_vertex_indices=[]
					GR(self,"INFO",f"Switched to {self._other_layer_label(owner)}",)
				else:
					GR(self,"WARNING","Could not switch to that edge's decal layer",)
					return {"RUNNING_MODAL"}
			...
			if ctrl_partial_click:
				...
				if self._ctrl_slice_interval is not None:
					generator_slice_interval=(self._normalize_slice_interval_for_edges(click_source,[hovered_edge_index],self._ctrl_slice_interval,interval_space="edge_local",) if click_source is not None else self._ctrl_slice_interval)
				else:
					GR(self,"WARNING","Ctrl partial preview unavailable — hover the edge again",)
					return {"RUNNING_MODAL"}
			(existing_interactive_obj,existing_strokes_on_edge,)=self._find_interactive_strokes_by_edge(hovered_edge_index)
			existing_interactive_stroke=(existing_strokes_on_edge[0] if existing_strokes_on_edge else None)
			existing_partial_edge_coverage=any((coverage:=self._stroke_edge_local_coverage(click_source,stroke,hovered_edge_index,)) is not None
				and (coverage[0] > EPSILON or coverage[1] < 1.0 - EPSILON) for stroke in existing_strokes_on_edge)
			existing_automatic_obj=None
			mutation_target=(existing_interactive_obj
				or bpy.data.objects.get(self._last_generated_object_name))
			old_generated_name=(mutation_target.name_full if mutation_target is not None else self._last_generated_object_name)
			previous_pending=list(self._pending_edge_indices or [])
			previous_anchor=self._path_anchor_edge_index
			previous_last_stroke_vertices=list(self._last_stroke_vertex_indices or [])
			deleted_backup=(self._make_object_backup(old_generated_name) if old_generated_name else None)
			conflicting_ctrl_strokes=[]
			if ctrl_partial_click:
				new_edge_set={hovered_edge_index}
				conflicting_ctrl_strokes=[stroke for stroke in existing_strokes_on_edge if not self._partial_strokes_can_share_source_edge(stroke,new_edge_set,click_edge_local_slice,source_obj=click_source,) 	]
			replacing_ctrl_partial=bool(ctrl_partial_click
				and (conflicting_ctrl_strokes
					or self._edge_has_automatic_generated_stroke(hovered_edge_index) ))
			if replacing_ctrl_partial:
				targeted_strokes=bool(conflicting_ctrl_strokes) and all({int(index)
						for index in stroke.get("edges",[])}=={hovered_edge_index} for stroke in conflicting_ctrl_strokes)
				if targeted_strokes:
					self._remove_specific_interactive_strokes(context,existing_interactive_obj,conflicting_ctrl_strokes,)
				else:
					replace_stroke_vertices=sorted({int(index)
						for stroke in conflicting_ctrl_strokes
						for index in stroke.get("vertices",[])})
					self._absorb_geometry_for_target_edges(context,[hovered_edge_index],extra_vertex_indices=replace_stroke_vertices,geometry_candidate_edges=[hovered_edge_index],skip_promote=True,)
			...
			if auto_neighbor_merge:
				clicked_edge_indices=list(self._hovered_edge_indices or [self._hovered_edge_index])
				click_merge_plan=self._plan_click_merge_group(context,self._hovered_edge_index,clicked_edge_indices=clicked_edge_indices,edge_local_slice_interval=click_edge_local_slice,)
			retaper_plan=None
			if (self._endpoint_taper_enabled
				and not event.shift
				and not event.ctrl
				...
				and int(self._hovered_edge_index) in {int(index) for index in existing_interactive_stroke.get("edges",[])}):
				stroke_edges=[int(index)
					for index in existing_interactive_stroke.get("edges",[])]
				which_end=self._chain_end_for_edge(context,stroke_edges,self._hovered_edge_index,)
				if which_end is not None:
					retaper_plan={"target_edges": stroke_edges,"taper_start": which_end== "start", "taper_end": which_end== "end",}
			...
			if (auto_neighbor_merge
				...
				if source_obj is not None:
					...
					for _decal_obj,stroke in self._iter_neighbor_strokes_for_merge(context,[hovered],):
						stroke_edges=[int(index)
							for index in stroke.get("edges",[])]
						if not self._edges_share_vertex(source_obj,hovered,
							stroke_edges,):
							continue
						if self._merge_connection_allowed(source_obj,[hovered],
							stroke,edge_local_slice_interval=click_edge_local_slice,):
							break
						merge_blocked_info=("Adjacent decal: merge only at the open/full end \n (not the taper tip). Use the other Ctrl control " "point or click the open end side.")
						break
			replacing_existing_edge=bool(not event.shift
				and not event.ctrl
				and click_merge_plan is None
				and retaper_plan is None
				and ((existing_interactive_stroke is not None
						and (len(existing_interactive_stroke.get("edges",[])) <=1 or existing_partial_edge_coverage))
					or (existing_interactive_stroke is None
						and self._edge_has_automatic_generated_stroke(self._hovered_edge_index) )))
			if (not event.shift
				and not event.ctrl
				and click_merge_plan is None
				and retaper_plan is None
			...
			if replacing_existing_edge and existing_interactive_stroke is None:
				existing_automatic_obj=self._automatic_decal_for_edge(context,self._hovered_edge_index,)
			if replacing_existing_edge:
				if existing_interactive_stroke is not None:
					replace_vertices=sorted({int(index)
						for stroke in existing_strokes_on_edge
						for index in stroke.get("vertices",[])})
					self._absorb_geometry_for_target_edges(context,[hovered_edge_index],extra_vertex_indices=replace_vertices,geometry_candidate_edges=[hovered_edge_index],skip_promote=True,)
					existing_interactive_obj=None
					existing_interactive_stroke=None
				else:
					promoted_obj,promoted=(self._promote_automatic_edge_to_interactive( context,self._hovered_edge_index,))
					if not promoted:
			...
			if (auto_neighbor_merge
				...
				if auto_connect_plan is None:
					auto_connect_plan=(self._plan_auto_connect_to_touching_stroke(context,list(self._hovered_edge_indices or []),) 	)
				if auto_connect_plan is None:
					auto_connect_plan=self._plan_auto_connect_to_pending_neighbor(context,list(self._hovered_edge_indices or []),edge_local_slice_interval=click_edge_local_slice,)
			force_connected=bool(event.shift or auto_connect_plan is not None or retaper_plan is not None)
			...
			if force_connected:
				if event.shift:
					...
					if not target_edges:
						self._discard_object_backup(deleted_backup)
						GR(self,"WARNING",reason)
						return {"RUNNING_MODAL"}
					new_connection_edges=[int(index) for index in target_edges if index not in set(previous_pending)]
					partial_ok,partial_reason=(self._shift_partial_merge_allowed(context,previous_pending,new_connection_edges,))
					if not partial_ok:
						self._discard_object_backup(deleted_backup)
						GR(self,"WARNING",partial_reason)
						return {"RUNNING_MODAL"}
					...
					reason= ""
				target_strokes=self._collect_strokes_for_edges(context,target_edges,)
				planned_stroke_ids={str(value) for value in (auto_connect_plan.get("stroke_ids",[]) if auto_connect_plan is not None else []) 	}
				if planned_stroke_ids:
					absorbed_strokes=[stroke for stroke in target_strokes if str(stroke.get("id", "")) in planned_stroke_ids]
					preserve_stroke_ids={str(stroke.get("id", "")) for stroke in target_strokes if str(stroke.get("id", "")) not in planned_stroke_ids}
				else:
				...
				if ctrl_partial_click and click_edge_local_slice is not None:
					pre_absorb_strokes.append({"edges": [int(self._hovered_edge_index)],"edge_local_slice_interval": list(click_edge_local_slice),"edge_local_slice_edge_index": int(self._hovered_edge_index),"slice_interval": None, "taper_vertices": [],})
				atomic_component_rebuild=bool(auto_connect_plan is not None
					and auto_connect_plan.get("atomic_component_rebuild",False)
					and not any(self._stroke_is_partial_slice(stroke) for stroke in pre_absorb_strokes))
				cleared_atomically=False
				if atomic_component_rebuild:
					cleared_atomically=(self._clear_active_layer_for_atomic_rebuild( context,target_edges,))
				removed_specific_strokes=False
				if planned_stroke_ids and absorbed_strokes:
					removed_specific_strokes=(self._remove_specific_interactive_strokes(context,self._interactive_layer_object(context),absorbed_strokes,))
				if removed_specific_strokes:
					target_set={int(index) for index in target_edges}
				elif cleared_atomically:
					target_set={int(index) for index in target_edges}
				else:
					target_set=self._absorb_geometry_for_target_edges(context,target_edges,)
			...
			if event.shift:
				slice_interval=self._compute_shift_slice_interval(context,target_edges,self._shift_connection_path,)
				if any(self._stroke_is_partial_slice(stroke)
					for stroke in pre_absorb_strokes):
					slice_interval=None
			elif auto_connect_plan is not None:
				slice_interval=self._partial_merge_chain_interval(context,target_edges,pre_absorb_strokes,)
			else:
				slice_interval=(generator_slice_interval
					if (event.ctrl
						and len(target_edges)==1) else None)
			if force_connected and slice_interval is None:
				merge_taper_start,merge_taper_end=(self._compute_merge_endpoint_tapers(context,target_edges,pre_absorb_strokes,))
			...
			vertex_count_before=(len(master_before.data.vertices)
				if (master_before is not None and master_before.data is not None) else 0)
			success,reason,generated_name=self._generate_from_edge_indices(context,target_edges,force_connected=force_connected,slice_interval=slice_interval,merge_taper_start=merge_taper_start,merge_taper_end=merge_taper_end,)
			new_taper_vertices=self._compute_taper_tip_vertices(context,target_edges,slice_interval=slice_interval,merge_taper_start=merge_taper_start,merge_taper_end=merge_taper_end,force_all=(self._endpoint_taper_enabled and slice_interval is None),)
			if slice_interval is not None:
				partial_stroke={"edges": target_edges,"slice_interval": list(slice_interval), "taper_vertices": [],}
				source_obj=self._source_object(context)
				if source_obj is not None:
					new_taper_vertices=sorted(set(new_taper_vertices)
						| self._stroke_taper_tip_vertices(source_obj,partial_stroke,) 	)
			if success:
				merged_name,new_stroke_vertices=self._merge_new_decal_into_master(generated_name,vertex_count_before=vertex_count_before,)
				if slice_interval is not None:
					final_master=bpy.data.objects.get(merged_name or generated_name)
					uv_success,uv_reason=(self._finalize_interactive_partial_uvs( context,final_master,))
					if not uv_success:
						if deleted_backup:
							self._delete_generated_object(merged_name or generated_name)
							restored_name=self._restore_object_backup(deleted_backup)
							self._interactive_layer_name=(restored_name or old_generated_name)
							self._last_generated_object_name=(self._interactive_layer_name)
							self._last_stroke_vertex_indices=(previous_last_stroke_vertices)
						GR(self,"WARNING",f"Could not finalize partial UVs: {uv_reason}",)
						return {"RUNNING_MODAL"}
				self._action_history.append({"created_name": merged_name or generated_name,"deleted_backup": deleted_backup,"previous_pending": previous_pending,"previous_anchor": previous_anchor, "previous_last_name": old_generated_name, "previous_last_stroke_vertices": previous_last_stroke_vertices,})
				if slice_interval is not None and event.ctrl and not event.shift:
					...
					if event.shift and self._shift_terminal_edge_index >=0:
						...
						self._pending_partial_edge_index=(self._shift_terminal_edge_index if self._shift_fraction < 1.0 - EPSILON else -1)
						self._pending_partial_fraction=self._shift_fraction
					elif auto_connect_plan is not None:
						self._path_anchor_edge_index=int(auto_connect_plan["anchor_edge"])
						self._pending_partial_edge_index=-1
						self._pending_partial_fraction=1.0
					else:
				...
				self._register_interactive_stroke(self._last_generated_object_name,target_edges,new_stroke_vertices,force_connected=force_connected,slice_interval=slice_interval,face_width=self._interactive_face_width,taper_vertices=new_taper_vertices,edge_local_slice_interval=click_edge_local_slice,edge_local_slice_edge_index=(hovered_edge_index if click_edge_local_slice is not None else None),preserve_stroke_ids=preserve_stroke_ids,)
				self._last_click_edge_index=hovered_edge_index
				self._last_click_time=time.monotonic()
				if auto_connect_plan is not None:
					GR(self,"INFO", "Merged adjacent decal strips")
				elif merge_blocked_info:
					GR(self,"INFO",merge_blocked_info)
			else:
				if deleted_backup:
					...
					self._last_stroke_vertex_indices=previous_last_stroke_vertices
				GR(self,"WARNING",f"Could not generate decal: {reason}",)
		...
		if event.type in {"DEL", "X", "BACK_SPACE"} and event.value== "PRESS":
			if self._pending_edge_indices:
				self._clear_pending_path(context)
				GR(self,"INFO", "Cleared shortest-path continuation")
				return {"RUNNING_MODAL"}
