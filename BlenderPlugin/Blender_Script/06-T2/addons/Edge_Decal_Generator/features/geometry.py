...
EDGEDECAL_PREPARED_SOURCE_CACHE_STATS={"prepared_hits": 0,"prepared_misses": 0, "chain_hits": 0, "chain_misses": 0,}
def prepared_source_cache_key(source_obj,angle_limit=radians(5.0),apply_limited_dissolve=True,):
	if (source_obj is None
		or getattr(source_obj, "type",None) != "MESH"
		or getattr(source_obj, "data",None) is None):
		return None
	return (int(source_obj.data.as_pointer()),round(max(0.0,float(angle_limit)),10),bool(apply_limited_dissolve),)
def clear_prepared_source_cache(source=None,reset_stats=False):
	if source is None:
		...
		mesh=(source.data
			if getattr(source, "type",None)== "MESH" else source)
...
def _cache_matrix_signature(matrix):
	return tuple(round(float(value),10) for row in matrix for value in row)
...
def source_mesh_prepared_cache_signature(mesh):
	...
	digest.update((f"{len(mesh.vertices)}:{len(mesh.edges)}:"
			f"{len(mesh.loops)}:{len(mesh.polygons)}").encode("ascii"))
	_update_mesh_cache_digest(digest,mesh.vertices, "co", "f",len(mesh.vertices) * 3)
	_update_mesh_cache_digest(digest,mesh.edges, "vertices", "i",len(mesh.edges) * 2)
	_update_mesh_cache_digest(digest,mesh.edges, "use_seam", "b",len(mesh.edges))
	if len(mesh.edges):
		try:
			_update_mesh_cache_digest(digest,mesh.edges,"use_edge_sharp","b",len(mesh.edges),)
		except (AttributeError,TypeError):
			pass
	_update_mesh_cache_digest(digest,mesh.loops, "vertex_index", "i",len(mesh.loops))
	_update_mesh_cache_digest(digest,mesh.polygons, "loop_start", "i",len(mesh.polygons))
	_update_mesh_cache_digest(digest,mesh.polygons, "loop_total", "i",len(mesh.polygons))
	_update_mesh_cache_digest(digest,mesh.polygons,"material_index","i",len(mesh.polygons),)
	for uv_layer in mesh.uv_layers:
		digest.update(uv_layer.name.encode("utf-8",errors="replace"))
		_update_mesh_cache_digest(digest,uv_layer.data,"uv","f",len(uv_layer.data) * 2,)
...
def invalidate_prepared_source_cache_if_changed(mesh):
	...
	pending_entries=[entry for entry in entries if entry.get("source_signature") is None]
	for entry in pending_entries:
		entry["source_signature"]=current_signature
	if all(entry.get("source_signature", "")==current_signature
...
def resolve_random_face_width_bounds(minimum_face_width,maximum_face_width,all_source_edges,world_matrix,):
	width_a=resolve_relative_face_width(minimum_face_width,all_source_edges,world_matrix,)
	width_b=resolve_relative_face_width(maximum_face_width,all_source_edges,world_matrix,)
	return min(width_a,width_b),max(width_a,width_b)
def randomized_face_width(minimum_face_width,maximum_face_width,seed,signature,):
	...
	if maximum - minimum <=EPSILON:
		return minimum
	rng=random.Random(chain_random_stream_seed(seed,signature,0xD1CEB00C))
...
def partition_selected_edge_graph_by_angle(selected_edges,world_matrix,split_angle,):
	...
	for vertex,incident_edges in graph.items():
		...
		for edge in incident_edges:
			...
			directions[edge]=safe_normalized((world_matrix @ other.co) - vertex_point,Vector((1.0,0.0,0.0)),)
		for index_a in range(len(incident_edges) - 1):
			...
			for index_b in range(index_a + 1,len(incident_edges)):
				...
				turn_cosine=max(-1.0,min(1.0,-directions[edge_a].dot(directions[edge_b])),)
	...
	groups=[sorted(group,key=lambda edge: edge.index) for group in grouped.values()]
...
def extract_edge_chains(selected_edges,world_matrix):
	...
	while unvisited:
		...
		closed=(len(verts) > 2 and verts[-1]==verts[0])
		if closed:
			verts=verts[:-1]
		if edges:
...
def topological_edge_loop_continuation(current_edge,vertex):
	if (current_edge is None
		...
		return None
	quad_faces=[face for face in current_edge.link_faces if len(face.verts)==4]
	if not quad_faces:
		return None
	...
	candidates=[edge for edge in vertex.link_edges if (edge !=current_edge and edge not in adjacent_in_quads and not edge.hide and len(edge.link_faces)==2)]
...
def walk_topological_edge_loop(bm,start_edge_index):
	...
	def walk_from(start_vertex):
		...
		current_vertex=start_vertex
		while True:
			next_edge=topological_edge_loop_continuation(current_edge,current_vertex,)
...
def filter_automatic_edges_by_angle(bm,minimum_face_angle,candidate_edge_indices=None,):
	bm.edges.ensure_lookup_table()
	threshold=max(0.0,min(pi,float(minimum_face_angle)))
	candidates=(range(len(bm.edges)) if candidate_edge_indices is None else candidate_edge_indices)
	qualified=set()
...
def split_chain_by_angle(chain_verts,chain_edges,closed,world_matrix,split_angle):
	...
	if not closed:
		...
		for vertex_index in range(1,len(chain_verts) - 1):
			angle=traversal_turn_angle(world_points[vertex_index - 1],world_points[vertex_index],world_points[vertex_index + 1],)
	...
	for vertex_index in range(len(chain_verts)):
		...
		angle=traversal_turn_angle(world_points[previous_index],world_points[vertex_index],world_points[next_index],)
		if angle > split_angle:
	...
	for break_list_index,start_vertex in enumerate(break_indices):
		...
		if sub_edges:
			result.append((sub_verts,sub_edges,False))
	return result
def edge_surface_bend_class(edge,world_matrix,normal_matrix,flat_angle=1.0e-4,):
	...
	try:
		...
		return 0
def extract_edge_chains_by_surface_bend(selected_edges,world_matrix,normal_matrix,):
	grouped_edges={-1: [],0: [],1: []}
	for edge in selected_edges:
		grouped_edges[edge_surface_bend_class(edge,world_matrix,normal_matrix)].append(edge)
	chains=[]
	for bend_class in (-1,0,1):
		if grouped_edges[bend_class]:
			chains.extend(extract_edge_chains(grouped_edges[bend_class],world_matrix))
	return chains
def extract_cached_edge_chains_by_surface_bend(work_bmesh,selected_edges,world_matrix,normal_matrix,cache_key=None,):
	entry=(EDGEDECAL_PREPARED_SOURCE_CACHE.get(cache_key) if cache_key is not None else None)
	if entry is None:
		return extract_edge_chains_by_surface_bend(selected_edges,world_matrix,normal_matrix,)
	...
	chain_key=(tuple(sorted(int(edge.index) for edge in selected_edges)),_cache_matrix_signature(world_matrix),)
	cached_chains=entry["chains"].get(chain_key)
	if cached_chains is not None:
		try:
			result=[([work_bmesh.verts[index] for index in vert_indices],[work_bmesh.edges[index] for index in edge_indices],bool(closed),) for vert_indices,edge_indices,closed in cached_chains]
	...
	result=extract_edge_chains_by_surface_bend(selected_edges,world_matrix,normal_matrix,)
	entry["chains"][chain_key]=tuple((tuple(int(vert.index) for vert in chain_verts),tuple(int(edge.index) for edge in chain_edges),bool(closed),) for chain_verts,chain_edges,closed in result)
	return result
def build_limited_dissolve_generation_bmesh(source_bmesh,angle_limit=radians(5.0),apply_limited_dissolve=True,cache_source=None,use_cache=False,):
	cache_key=(prepared_source_cache_key(cache_source,angle_limit,apply_limited_dissolve,) if use_cache else None)
	cached_entry=(EDGEDECAL_PREPARED_SOURCE_CACHE.get(cache_key) if cache_key is not None else None)
	cached_bmesh=(cached_entry.get("bmesh") if cached_entry is not None else None)
	if cached_bmesh is not None and cached_bmesh.is_valid:
		...
		original_layer=work_bmesh.edges.layers.int.get("edge_decal_original_index")
		original_index_by_edge={edge: int(edge[original_layer]) - 1 for edge in work_bmesh.edges if original_layer is not None and int(edge[original_layer]) > 0}
	...
	original_layer=None
	if cache_key is not None:
		original_layer=work_bmesh.edges.layers.int.new("edge_decal_original_index")
		for edge in work_bmesh.edges:
			edge[original_layer]=int(edge.index) + 1
	original_index_by_edge={edge: edge.index for edge in work_bmesh.edges}
	dissolve_candidates=list(work_bmesh.edges)
	if apply_limited_dissolve and dissolve_candidates:
		bmesh.ops.dissolve_limit(work_bmesh,angle_limit=max(0.0,float(angle_limit)),use_dissolve_boundaries=False,verts=list(work_bmesh.verts),edges=dissolve_candidates,delimit={"MATERIAL", "SEAM", "SHARP", "UV"},)
	work_bmesh.normal_update()
	work_bmesh.verts.ensure_lookup_table()
	work_bmesh.edges.ensure_lookup_table()
	work_bmesh.faces.ensure_lookup_table()
	if cache_key is not None:
		...
		work_bmesh.edges.index_update()
		work_bmesh.faces.index_update()
		original_index_by_edge={edge: int(edge[original_layer]) - 1 for edge in work_bmesh.edges if int(edge[original_layer]) > 0}
		previous_entry=EDGEDECAL_PREPARED_SOURCE_CACHE.pop(cache_key,None)
		previous_bmesh=(previous_entry.get("bmesh") if previous_entry is not None else None)
		if previous_bmesh is not None and previous_bmesh.is_valid:
			previous_bmesh.free()
		EDGEDECAL_PREPARED_SOURCE_CACHE[cache_key]={"bmesh": work_bmesh.copy(),"chains": {},"source_signature": (None if cache_source.data.is_editmode else source_mesh_prepared_cache_signature(cache_source.data)),}
	return work_bmesh,original_index_by_edge
...
def face_inward_direction(face,edge,world_matrix,normal_matrix,tangent):
	...
	loop_direction=safe_normalized(loop_end_world - loop_start_world,tangent,)
	inward=face_normal_world.cross(loop_direction)
	inward -=face_normal_world * inward.dot(face_normal_world)
	return safe_normalized(inward,face_normal_world.cross(tangent),)
...
def assign_face_sides(face_data,previous_pair=None):
	...
	direct_score=((10.0
			if (a0["face"]==p0["face"]
				or faces_are_coplanar(a0["face"],p0["face"])) else 0.0)
		+ (10.0
			if (a1["face"]==p1["face"]
				or faces_are_coplanar(a1["face"],p1["face"])) else 0.0)
		+ a0["normal"].dot(p0["normal"])
		+ a1["normal"].dot(p1["normal"]))
	swapped_score=((10.0
			if (a1["face"]==p0["face"]
				or faces_are_coplanar(a1["face"],p0["face"])) else 0.0)
		+ (10.0
			if (a0["face"]==p1["face"]
				or faces_are_coplanar(a0["face"],p1["face"])) else 0.0)
...
def oriented_quad(indices,vertices,target_normal):
	oriented_indices,_=oriented_face_with_uvs(indices,tuple((0.0,0.0) for _ in indices),vertices,target_normal,)
	return oriented_indices
def ray_segment_distance_in_plane(ray_origin,ray_direction,segment_start,segment_end,plane_normal,):
...
def _segment_point_distance_sq(point,seg_start,seg_end):
	...
	factor=max(0.0,min(1.0,(point - seg_start).dot(segment) / length_sq),)
	closest=seg_start + segment * factor
	delta=point - closest
	return delta.length_squared
def build_adaptive_width_search_context(all_source_edges,selected_edge_set,world_matrix,clamp_boundaries,clamp_selected_overlaps,search_radius,):
	...
	for path_index,(_verts,path_edges,path_closed) in enumerate(extract_edge_chains(selected,world_matrix)):
		edge_lengths=[]
		...
		for edge_index,path_edge in enumerate(path_edges):
			...
			for candidate_index,candidate_edge in enumerate(path_edges):
				path_distance=abs(edge_centers[edge_index] - edge_centers[candidate_index])
				if path_closed and total_length > EPSILON:
					path_distance=min(path_distance,total_length - path_distance,)
	...
	def add_edge(edge,is_selected):
		...
		entry={"edge": edge,"p0": p0,"p1": p1,"is_selected": is_selected, "verts": frozenset(edge.verts), "selected_path": selected_path_by_edge.get(edge),}
		entries.append(entry)
		bounds_min=Vector((min(p0.x,p1.x) - search_radius,min(p0.y,p1.y) - search_radius,min(p0.z,p1.z) - search_radius,))
		bounds_max=Vector((max(p0.x,p1.x) + search_radius,max(p0.y,p1.y) + search_radius,max(p0.z,p1.z) + search_radius,))
		...
		for cell_i in range(i0,i1 + 1):
			for cell_j in range(j0,j1 + 1):
				for cell_k in range(k0,k1 + 1):
					grid.setdefault((cell_i,cell_j,cell_k),[],).append(entry)
	if clamp_boundaries:
	...
	if not entries:
		return None
	return {"grid": grid,"inv_cell_size": inv_cell_size, "search_radius_sq": search_radius_sq,"selected_path_by_edge": selected_path_by_edge, "selected_local_path_edges": selected_local_path_edges,}
def _adaptive_width_nearby_entries(width_search_context,sample_point):
	...
	for entry in grid.get((cell_i,cell_j,cell_k),()):
		...
		if edge_index in seen:
			continue
		seen.add(edge_index)
		if (_segment_point_distance_sq(sample_point,entry["p0"],entry["p1"],)
			> search_radius_sq):
			continue
		nearby.append(entry)
	return nearby
def adaptive_width_for_edge_side(edge,side,tangent,requested_width,clearance,sample_count,selected_edge_set,all_source_edges,world_matrix,clamp_boundaries=True,clamp_selected_overlaps=False,overlap_clearance=0.98,width_search_context=None,):
	...
	sample_sets={1: (0.5,),2: (0.25,0.75),3: (0.15,0.5,0.85),4: (0.1,0.35,0.65,0.9),5: (0.08,0.25,0.5,0.75,0.92),}
	sample_factors=sample_sets.get(max(1,min(5,int(sample_count))),sample_sets[3],)
	edge_vertices=set(edge.verts)
	use_spatial_index=width_search_context is not None
	current_selected_path=(width_search_context.get("selected_path_by_edge",{}).get(edge) if use_spatial_index else None)
	for factor in sample_factors:
		sample_point=p0.lerp(p1,factor)
		if use_spatial_index:
			candidate_entries=_adaptive_width_nearby_entries(width_search_context,sample_point,)
		else:
			candidates=set()
			if clamp_boundaries:
			...
			candidate_entries=[{"edge": candidate,"p0": world_matrix @ candidate.verts[0].co,"p1": world_matrix @ candidate.verts[1].co,"is_selected": candidate in selected_edge_set, "verts": frozenset(candidate.verts),"selected_path": None,} for candidate in candidates]
		for entry in candidate_entries:
			...
			if candidate_is_selected:
				...
				if (current_selected_path is not None
					...
					and candidate in width_search_context.get("selected_local_path_edges",{}).get(edge,())):
					continue
			...
			if abs(face_normal.dot(candidate_end - sample_point)) > plane_tolerance:
				continue
			distance=ray_segment_distance_in_plane(sample_point,inward,candidate_start,candidate_end,face_normal,)
			if distance is None:
				continue
			if candidate_is_selected:
	...
	if nearest_safe_width is None:
		return requested_width
	return max(1.0e-5,min(requested_width,nearest_safe_width),)
def stabilize_width_sequence(widths,edge_lengths,closed,maximum_slope=1.0):
	...
	def center_distance(index_a,index_b):
		return max(1.0e-6,(lengths[index_a] + lengths[index_b]) * 0.5,)
	...
	for _pass in range(passes):
		...
		if not changed:
			break
	return stabilized
def stabilize_selected_graph_overlap_widths(edge_data,selected_edges,world_matrix,):
	selected_edge_set=set(selected_edges)
	slots={(edge,face) for edge in selected_edge_set for face in edge.link_faces if edge in edge_data and face in edge_data[edge]["sides"]}
	adjacency={slot: set() for slot in slots}
	for source_vertex in {vertex for edge in selected_edge_set for vertex in edge.verts}:
		...
		for radial_edge in source_vertex.link_edges:
			if radial_edge in selected_edge_set:
				continue
			radial_faces=[face for face in radial_edge.link_faces if face in face_adjacency]
		...
		for face_component in face_components:
			vertex_slots=[(edge,face) for edge in source_vertex.link_edges if edge in selected_edge_set and edge in edge_data for face in edge.link_faces if (face in face_component and (edge,face) in slots) 	]
	...
	widths={slot: max(1.0e-5,float(edge_data[slot[0]]["sides"][slot[1]]["width"]),) for slot in slots}
	for edge,face in slots:
		...
		for source_vertex in edge.verts:
			...
			current_tangent=safe_normalized((world_matrix @ edge.other_vert(source_vertex).co) - origin,data["tangent"],)
			neighbor_tangent=safe_normalized((world_matrix
					@ neighbor_edge.other_vert(source_vertex).co) - origin,edge_data[neighbor_edge]["tangent"],)
			unit_miter=planar_line_intersection(origin,current_tangent,neighbor_tangent,side["inward"],neighbor_side["inward"],side["normal"],1.0,1.0,1.0e6,)
			consumption +=max(0.0,(unit_miter - origin).dot(current_tangent),)
		if consumption > EPSILON:
			widths[(edge,face)]=min(widths[(edge,face)],data["length"] * 0.9 / consumption,)
	maximum_slope=0.75
	ordered_links=sorted({tuple(sorted((slot,neighbor),
				key=lambda item: (item[0].index,item[1].index),))
			for slot in slots
			for neighbor in adjacency[slot]
			if slot !=neighbor},
		key=lambda pair: (pair[0][0].index,pair[0][1].index,pair[1][0].index,pair[1][1].index,),)
	for _pass in range(max(1,len(slots))):
		changed=False
		for first,second in ordered_links:
			travel=max(1.0e-6,(edge_data[first[0]]["length"] + edge_data[second[0]]["length"]) * 0.5,)
...
def planar_line_intersection(vertex_point,previous_tangent,next_tangent,previous_inward,next_inward,face_normal,previous_width,next_width,miter_limit,):
	...
	if offset.length > maximum:
		offset=safe_normalized(previous_inward + next_inward,next_inward,) * maximum
		intersection=vertex_point + offset
	return intersection
def bound_overlap_miter_point(origin,point,incident_segments):
	offset=point - origin
	scale=1.0
	radial_limit=max(float(segment["width"])
		for segment in incident_segments) * 1.5
...
def offset_point_from_face_normals(point,normals,distance,):
	if distance <=EPSILON:
		return point.copy()
	unique_normals=[]
	for normal in normals:
		...
		if candidate.length_squared <=EPSILON:
			continue
		if any(candidate.dot(existing) > 0.999999
	...
	def solve_pair(normal_a,normal_b):
		...
		return (normal_a + normal_b) * (distance / denominator)
	if len(unique_normals)==2:
		return point + solve_pair(unique_normals[0],unique_normals[1],)
	best_matrix=None
	...
	for index_a in range(len(unique_normals) - 2):
		for index_b in range(index_a + 1,len(unique_normals) - 1):
			for index_c in range(index_b + 1,len(unique_normals)):
				matrix=Matrix((tuple(unique_normals[index_a]),tuple(unique_normals[index_b]),tuple(unique_normals[index_c]),))
	...
	if best_matrix is not None and best_determinant > 1.0e-8:
		displacement=best_matrix.inverted() @ Vector((distance,distance,distance,))
		residual=max(abs(normal.dot(displacement) - distance) for normal in unique_normals)
	...
	for normal in unique_normals:
		...
		for row in range(3):
			for column in range(3):
				matrix_values[row][column] +=(normal[row] * normal[column])
	...
	if displacement.length_squared <=EPSILON:
		...
		for index_a in range(len(unique_normals) - 1):
			for index_b in range(index_a + 1,len(unique_normals)):
				pair_dot=abs(unique_normals[index_a].dot(unique_normals[index_b]))
				if pair_dot < lowest_dot:
					lowest_dot=pair_dot
					best_pair=(unique_normals[index_a],unique_normals[index_b],)
		displacement=solve_pair(*best_pair)
	return point + displacement
def build_selected_edge_graph_strip(selected_edges,all_source_edges,world_matrix,normal_matrix,face_width,surface_offset,miter_limit,vertices_out,faces_out,face_uvs_out,center_vertices_out,auto_face_width=False,auto_width_samples=1,auto_width_clearance=0.85,clamp_edge_overlaps=False,overlap_clearance=0.98,width_search_context=None,face_width_resolved=False,uv_scale=1.0,face_source_edges_out=None,face_width_by_edge=None,fold_nonplanar_sectors=False,):
	selected_edges=[edge for edge in selected_edges if len(edge.link_faces)==2]
	if not selected_edges:
		return 0
	selected_edge_set=set(selected_edges)
	if not face_width_resolved:
		face_width=resolve_relative_face_width(face_width,all_source_edges,world_matrix,)
	else:
	...
	for edge in selected_edges:
		...
		sides={}
		edge_face_width=max(float(face_width_by_edge.get(edge,face_width) if face_width_by_edge is not None else face_width),1.0e-8,)
		for face in edge.link_faces:
			normal=transform_normal(normal_matrix,face.normal)
			inward=face_inward_direction(face,edge,world_matrix,normal_matrix,tangent,)
			side={"face": face,"normal": normal, "inward": inward,}
			width=edge_face_width
			if auto_face_width or clamp_edge_overlaps:
				width=adaptive_width_for_edge_side(edge,side,tangent,edge_face_width,auto_width_clearance,auto_width_samples,selected_edge_set,all_source_edges,world_matrix,clamp_boundaries=auto_face_width,clamp_selected_overlaps=clamp_edge_overlaps,overlap_clearance=overlap_clearance,width_search_context=width_search_context,)
			sides[face]={**side, "width": width,}
		edge_data[edge]={"verts": (v0,v1),"points": (p0,p1), "tangent": tangent,"length": (p1 - p0).length, "sides": sides,}
	if clamp_edge_overlaps:
		stabilize_selected_graph_overlap_widths(edge_data,selected_edges,world_matrix,)
	center_by_vertex={}
	outer_by_slot={}
	...
	def slot_outer_point(source_vertex,edge,face):
		...
		return (source_point + side["inward"] * side["width"] + side["normal"] * surface_offset)
	for source_vertex,incident_edges in graph.items():
		source_point=world_matrix @ source_vertex.co
		incident_normals=[edge_data[edge]["sides"][face]["normal"] for edge in incident_edges for face in edge.link_faces]
		center_index=append_vertex(offset_point_from_face_normals(source_point,incident_normals,surface_offset,))
		...
		if len(incident_edges)==1:
			...
			for face in edge.link_faces:
				outer_by_slot[(source_vertex,edge,face)]=append_vertex(slot_outer_point(source_vertex,edge,face))
			continue
		incident_faces=list(source_vertex.link_faces)
		face_adjacency={face: set() for face in incident_faces}
		for radial_edge in source_vertex.link_edges:
		...
		for component in components:
			...
			component_normals=[transform_normal(normal_matrix,face.normal) for face in component]
			support_normal=Vector((0.0,0.0,0.0))
			for normal in component_normals:
				support_normal +=normal
			support_normal=safe_normalized(support_normal,component_normals[0],)
			sector_origin=offset_point_from_face_normals(source_point,component_normals,surface_offset,)
			def projected_slot_vectors(edge,face):
				...
				tangent=safe_normalized(tangent,edge_data[edge]["tangent"],)
				raw_inward=edge_data[edge]["sides"][face]["inward"]
				inward=raw_inward - support_normal * raw_inward.dot(support_normal)
				inward=safe_normalized(inward,support_normal.cross(tangent),)
			...
			if (fold_nonplanar_sectors
				and len(component)==2
				and component_normals[0].dot(component_normals[1]) < 0.9999):
				fold_edges=[radial_edge for radial_edge in source_vertex.link_edges if (radial_edge not in selected_edge_set and face_a in radial_edge.link_faces and face_b in radial_edge.link_faces) 	]
				if len(fold_edges)==1:
					fold_other=fold_edges[0].other_vert(source_vertex)
					fold_direction=safe_normalized((world_matrix @ fold_other.co) - source_point,Vector((1.0,0.0,0.0)),)
					fold_length=((world_matrix @ fold_other.co) - source_point).length
					def rail_fold_intersection(edge,face,width):
						side=edge_data[edge]["sides"][face]
						edge_other=edge.other_vert(source_vertex)
						tangent=safe_normalized((world_matrix @ edge_other.co) - source_point,edge_data[edge]["tangent"],)
						rail_origin=(source_point + side["inward"] * width + side["normal"] * surface_offset)
						denominator=side["normal"].dot(tangent.cross(fold_direction))
						if abs(denominator) <=EPSILON:
							return None
						parameter=side["normal"].dot((sector_origin - rail_origin).cross( fold_direction)) / denominator
						point=rail_origin + tangent * parameter
						fold_distance=(point - sector_origin).dot(fold_direction)
						fold_distance=max(0.0,min(fold_length,fold_distance),)
						point=(sector_origin + fold_direction * fold_distance)
						reach_limit=width * max(1.0,miter_limit)
						if clamp_edge_overlaps:
							reach_limit=min(reach_limit,width * 1.5)
						if (reach_limit <=EPSILON
							or (point - sector_origin).length
							> reach_limit + EPSILON):
							return None
						return point
					fold_point_a=rail_fold_intersection(edge_a,face_a,width_a,)
					fold_point_b=rail_fold_intersection(edge_b,face_b,width_b,)
					if fold_point_a is not None and fold_point_b is not None:
						sector_index=append_vertex((fold_point_a + fold_point_b) * 0.5)
						outer_by_slot[(source_vertex,edge_a,face_a)]=sector_index
						outer_by_slot[(source_vertex,edge_b,face_b)]=sector_index
						continue
			sector_point=planar_line_intersection(sector_origin,tangent_a,tangent_b,inward_a,inward_b,support_normal,width_a,width_b,miter_limit,)
			if clamp_edge_overlaps:
				sector_offset=sector_point - sector_origin
				radial_limit=max(width_a,width_b) * 1.5
				if (radial_limit > EPSILON
					and sector_offset.length > radial_limit):
					sector_point=(sector_origin
	...
	for edge in selected_edges:
		...
		u_length=max(data["length"] * max(float(uv_scale),1.0e-8),1.0e-8,)
		for side_index,face in enumerate(edge.link_faces):
			...
			indices,uvs=oriented_face_with_uvs((center0,center1,outer1,outer0),((0.0,0.5),(u_length,0.5),(u_length,outer_v),(0.0,outer_v),),vertices_out,edge_data[edge]["sides"][face]["normal"],)
...
def build_partitioned_selected_edge_graph_strip(selected_edges,edge_groups,vertices_out,faces_out,face_uvs_out,center_vertices_out,**builder_options,):
	...
	created=build_selected_edge_graph_strip(selected_edges=selected_edges,vertices_out=temporary_vertices,faces_out=temporary_faces,face_uvs_out=temporary_uvs,center_vertices_out=temporary_centers,face_source_edges_out=temporary_face_edges,**builder_options,)
	if created==0:
		return 0
	center_set=set(temporary_centers)
	vertex_maps={}
	published_centers=set()
	for temporary_face,face_uvs,source_edge in zip(temporary_faces,temporary_uvs,
		temporary_face_edges,):
		...
		for temporary_index in temporary_face:
			...
			if output_index is None:
				...
				if (temporary_index in center_set
...
def endpoint_miter_point(source_vertex,current_edge,current_tangent_away,side,selected_edge_set,world_matrix,normal_matrix,vertex_point,face_width,miter_limit,):
	fallback=(vertex_point + side["inward"] * face_width)
	support_face=side["face"]
	incident_selected=[edge for edge in source_vertex.link_edges if (edge in selected_edge_set and len(edge.link_faces)==2)]
	if current_edge not in incident_selected:
		return fallback
	neighbor_edges=[edge for edge in incident_selected if edge !=current_edge and support_face in edge.link_faces]
	...
	for candidate_face in neighbor_edge.link_faces:
		if (candidate_face==support_face
			or faces_are_coplanar(candidate_face,support_face,)):
	...
	current_length=(current_other_world - vertex_point).length
	neighbor_length=(neighbor_other_world - vertex_point).length
	if (current_length <=EPSILON
		or neighbor_length <=EPSILON):
		return fallback
	neighbor_tangent_away=safe_normalized(neighbor_other_world - vertex_point,current_tangent_away,)
	neighbor_inward=face_inward_direction(neighbor_support_face,neighbor_edge,world_matrix,normal_matrix,neighbor_tangent_away,)
	miter_point=planar_line_intersection(vertex_point,current_tangent_away,neighbor_tangent_away,side["inward"],neighbor_inward,side["normal"],face_width,face_width,miter_limit,)
	miter_reach=(miter_point - vertex_point).length
	local_reach_limit=min(face_width * max(1.0,miter_limit),current_length * 0.75,neighbor_length * 0.75,)
	if local_reach_limit <=EPSILON or miter_reach > local_reach_limit:
		return fallback
	return miter_point
def selected_face_junction_neighbor(source_vertex,current_edge,support_face,selected_edge_set,):
	candidates=[edge for edge in source_vertex.link_edges if (edge !=current_edge and edge in selected_edge_set and len(edge.link_faces)==2 and support_face in edge.link_faces)]
	return candidates[0] if len(candidates)==1 else None
def source_vertex_at_world_point(chain_verts,world_matrix,point,tolerance=1.0e-7,):
	for source_vertex in chain_verts:
		if ((world_matrix @ source_vertex.co) - point).length <=tolerance:
			return source_vertex
	return None
def selected_junction_center_point(source_vertex,selected_edge_set,world_matrix,normal_matrix,surface_offset,):
	incident_faces=[]
	seen_faces=set()
	...
	normals=[transform_normal(normal_matrix,face.normal) for face in incident_faces]
	return offset_point_from_face_normals(point,normals,surface_offset)
def support_faces_share_edge_at_vertex(face_a,face_b,source_vertex,):
	if face_a is None or face_b is None or face_a==face_b:
		return False
	return any(edge in face_b.edges and source_vertex in edge.verts for edge in face_a.edges)
def append_cached_junction_vertex(vertices_out,junction_vertex_cache,key,point,):
	...
	return index
def append_endpoint_junction_vertex(vertices_out,junction_vertex_cache,source_vertex,current_edge,side,selected_edge_set,point,surface_offset,):
	incident_selected=[edge for edge in source_vertex.link_edges if edge in selected_edge_set and len(edge.link_faces)==2]
	neighbor=selected_face_junction_neighbor(source_vertex,current_edge,side["face"],selected_edge_set,)
	cache_key=(("outer",source_vertex.index,side["face"].index)
		if len(incident_selected) >=3 and neighbor is not None else None)
	return append_cached_junction_vertex(vertices_out,junction_vertex_cache,cache_key,point + side["normal"] * surface_offset,)
def topology_sector_endpoint_indices(source_vertex,previous_edge,next_edge,previous_tangent_away,next_tangent_away,previous_side,next_side,previous_width,next_width,selected_edge_set,world_matrix,normal_matrix,vertex_point,miter_limit,surface_offset,vertices_out,junction_vertex_cache,):
	incident_faces=set(source_vertex.link_faces)
	face_adjacency={face: set() for face in incident_faces}
	for radial_edge in source_vertex.link_edges:
		if radial_edge in selected_edge_set:
			continue
		radial_faces=[face for face in radial_edge.link_faces if face in incident_faces]
		for face_a in radial_faces:
			for face_b in radial_faces:
				if face_a is not face_b:
					face_adjacency[face_a].add(face_b)
	...
	for normal in component_normals:
		support_normal +=normal
	support_normal=safe_normalized(support_normal,component_normals[0],)
	sector_origin=offset_point_from_face_normals(vertex_point,component_normals,surface_offset,)
	def projected_vectors(tangent,side):
		...
		inward=side["inward"] - support_normal * side["inward"].dot(support_normal)
		inward=safe_normalized(inward,support_normal.cross(tangent),)
		return tangent,inward
	previous_tangent,previous_inward=projected_vectors(previous_tangent_away,previous_side,)
	next_tangent,next_inward=projected_vectors(next_tangent_away,next_side,)
	sector_point=planar_line_intersection(sector_origin,previous_tangent,next_tangent,previous_inward,next_inward,support_normal,previous_width,next_width,miter_limit,)
	component_signature=tuple(sorted(face.index for face in component))
	sector_index=append_cached_junction_vertex(vertices_out,junction_vertex_cache,("sector",source_vertex.index,component_signature),sector_point,)
	return sector_index,sector_index
def connected_face_loop_slide_direction(source_vertex,boundary_edges,support_faces,world_matrix,side_normal,side_inward,required_distance,):
	...
	for edge in source_vertex.link_edges:
		...
		if inward_score <=0.25:
			continue
		face_normal=transform_normal(world_matrix.inverted_safe().transposed().to_3x3(),matching_face.normal,)
...
def build_corner_strip(chain_verts,chain_edges,closed,selected_edge_set,all_source_edges,world_matrix,normal_matrix,face_width,decal_amount,slice_positions,forced_slice_interval,auto_face_width,auto_width_samples,auto_width_clearance,clamp_edge_overlaps,overlap_clearance,surface_offset,miter_limit,use_face_loop_slide,vertices_out,faces_out,face_uvs_out,center_vertices_out,width_search_context=None,face_width_resolved=False,junction_vertex_cache=None,**_ignored_taper_options,):
	if not face_width_resolved:
		face_width=resolve_relative_face_width(face_width,all_source_edges,world_matrix)
	else:
		face_width=max(float(face_width),1.0e-8)
	...
	if closed:
		...
		(work_world_points,work_chain_edges,start_trimmed,end_trimmed,)=trim_open_chain_by_amount(chain_verts,chain_edges,world_matrix,decal_amount,slice_positions,forced_interval=forced_slice_interval,)
	...
	for segment_index,edge in enumerate(work_chain_edges):
		...
		for face in linked_faces:
			normal=transform_normal(normal_matrix,face.normal)
			inward=face_inward_direction(face,edge,world_matrix,normal_matrix,tangent,)
			face_data.append({"face": face,"normal": normal, "inward": inward,})
		side_a,side_b=assign_face_sides(face_data,previous_pair)
		previous_pair=(side_a,side_b)
		width_a=face_width
		width_b=face_width
		if auto_face_width or clamp_edge_overlaps:
			width_a=adaptive_width_for_edge_side(edge,side_a,tangent,face_width,auto_width_clearance,auto_width_samples,selected_edge_set,all_source_edges,world_matrix,clamp_boundaries=auto_face_width,clamp_selected_overlaps=clamp_edge_overlaps,overlap_clearance=overlap_clearance,width_search_context=width_search_context,)
			width_b=adaptive_width_for_edge_side(edge,side_b,tangent,face_width,auto_width_clearance,auto_width_samples,selected_edge_set,all_source_edges,world_matrix,clamp_boundaries=auto_face_width,clamp_selected_overlaps=clamp_edge_overlaps,overlap_clearance=overlap_clearance,width_search_context=width_search_context,)
		segment_data.append({"tangent": tangent,"length": (p1 - p0).length,"a": side_a,"b": side_b, "a_width": width_a, "b_width": width_b,})
	if clamp_edge_overlaps:
	...
	for vertex_index,point in enumerate(world_points):
		...
		if closed or vertex_index > 0:
			...
			incident_normals.extend((previous_segment["a"]["normal"],previous_segment["b"]["normal"],))
		if closed or vertex_index < point_count - 1:
			next_segment=segment_data[vertex_index % edge_count]
			incident_normals.extend((next_segment["a"]["normal"],next_segment["b"]["normal"],))
		source_vertex=source_vertex_at_world_point(chain_verts,world_matrix,point,)
		incident_selected=([edge for edge in source_vertex.link_edges if edge in selected_edge_set and len(edge.link_faces)==2] if source_vertex is not None else [])
		center_key=None
		center_point=offset_point_from_face_normals(point,incident_normals,surface_offset,)
		if source_vertex is not None and len(incident_selected) >=3:
			center_key=("center",source_vertex.index)
			center_point=selected_junction_center_point(source_vertex,selected_edge_set,world_matrix,normal_matrix,surface_offset,)
		center_index=append_cached_junction_vertex(vertices_out,junction_vertex_cache,center_key,center_point,)
		center_indices.append(center_index)
		center_vertices_out.append(center_index)
	outer=[{"a_start": None,"a_end": None, "b_start": None,"b_end": None,} for _ in range(edge_count)]
	corner_patches=[]
	def selected_boundary_direction_count(source_vertex,boundary_edges):
		...
		for edge in boundary_edges:
			try:
				...
				continue
			direction=safe_normalized((world_matrix @ other_vertex.co) - source_point,Vector((1.0,0.0,0.0)),)
			if not any(direction.dot(existing) > 0.9995
				for existing in unique_directions):
				unique_directions.append(direction)
		return len(unique_directions)
	def loop_slide_point(source_vertex,boundary_edges,side,width,point,):
		if (not use_face_loop_slide
			or source_vertex is None):
			return None
		if selected_boundary_direction_count(source_vertex,boundary_edges,) !=1:
			return None
		slide_direction=connected_face_loop_slide_direction(source_vertex,boundary_edges,[side["face"]],world_matrix,side["normal"],side["inward"],required_distance=width,)
		if slide_direction is None:
			return None
		return point + slide_direction * width
	def make_outer_vertex(point,side,width):
		index=len(vertices_out)
		vertices_out.append(point + side["inward"] * width + side["normal"] * surface_offset)
		return index
	def make_endpoint_outer_vertex(source_vertex,current_edge,side,point,):
		return append_endpoint_junction_vertex(vertices_out,junction_vertex_cache,source_vertex,current_edge,side,selected_edge_set,point,surface_offset,)
	def assign_topology_sector_endpoints(vertex_index,side_key,previous_segment_index,next_segment_index,previous_segment,next_segment,previous_side,next_side,previous_slot,next_slot,):
		source_vertex=source_vertex_at_world_point(chain_verts,world_matrix,world_points[vertex_index],)
		if (source_vertex is None
			or not support_faces_share_edge_at_vertex(previous_side["face"],next_side["face"],
				source_vertex,)):
			return False
		previous_edge=work_chain_edges[previous_segment_index]
		next_edge=work_chain_edges[next_segment_index]
		previous_index,next_index=topology_sector_endpoint_indices(source_vertex=source_vertex,previous_edge=previous_edge,next_edge=next_edge,previous_tangent_away=-previous_segment["tangent"],next_tangent_away=next_segment["tangent"],previous_side=previous_side,next_side=next_side,previous_width=previous_segment[f"{side_key}_width"],next_width=next_segment[f"{side_key}_width"],selected_edge_set=selected_edge_set,world_matrix=world_matrix,normal_matrix=normal_matrix,vertex_point=world_points[vertex_index],miter_limit=miter_limit,surface_offset=surface_offset,vertices_out=vertices_out,junction_vertex_cache=junction_vertex_cache,)
		outer[previous_segment_index][previous_slot]=previous_index
		outer[next_segment_index][next_slot]=next_index
		return True
	def assign_local_bevel_join(vertex_index,side_key,previous_segment_index,next_segment_index,previous_segment,next_segment,previous_side,next_side,previous_slot,next_slot,):
		previous_width=previous_segment[f"{side_key}_width"]
		next_width=next_segment[f"{side_key}_width"]
		previous_point=(world_points[vertex_index] + previous_side["inward"] * previous_width + previous_side["normal"] * surface_offset)
		next_point=(world_points[vertex_index] + next_side["inward"] * next_width + next_side["normal"] * surface_offset)
		merge_tolerance=max(1.0e-6,surface_offset * 4.0,min(previous_width,next_width) * 0.12,)
		if (previous_point - next_point).length <=merge_tolerance:
			shared_index=len(vertices_out)
			vertices_out.append((previous_point + next_point) * 0.5)
			outer[previous_segment_index][previous_slot]=shared_index
			outer[next_segment_index][next_slot]=shared_index
			return
		...
		outer[previous_segment_index][previous_slot]=previous_index
		outer[next_segment_index][next_slot]=next_index
		corner_patches.append({"side": side_key,"previous_index": previous_index,"next_index": next_index,"center_index": center_indices[vertex_index],"normal": safe_normalized(previous_side["normal"] + next_side["normal"],next_side["normal"],), "vertex_index": vertex_index,})
	def solve_side_at_vertex(vertex_index,side_key):
		if not closed and vertex_index==0:
			...
			if start_trimmed:
				...
				boundary_edges=[edge for edge in source_vertex.link_edges if edge in selected_edge_set]
				point=loop_slide_point(source_vertex,boundary_edges,side,side_width,world_points[0],)
				if point is None:
					point=endpoint_miter_point(source_vertex=source_vertex,current_edge=work_chain_edges[0],current_tangent_away=segment_data[0]["tangent"],side=side,selected_edge_set=selected_edge_set,world_matrix=world_matrix,normal_matrix=normal_matrix,vertex_point=world_points[0],face_width=side_width,miter_limit=miter_limit,)
			if start_trimmed:
				index=len(vertices_out)
				vertices_out.append(point + side["normal"] * surface_offset)
			else:
				index=make_endpoint_outer_vertex(source_vertex,work_chain_edges[0],side,point,)
		...
		if not closed and vertex_index==point_count - 1:
			...
			if end_trimmed:
				...
				boundary_edges=[edge for edge in source_vertex.link_edges if edge in selected_edge_set]
				point=loop_slide_point(source_vertex,boundary_edges,side,side_width,world_points[-1],)
				if point is None:
					point=endpoint_miter_point(source_vertex=source_vertex,current_edge=work_chain_edges[-1],current_tangent_away=-segment_data[-1]["tangent"],side=side,selected_edge_set=selected_edge_set,world_matrix=world_matrix,normal_matrix=normal_matrix,vertex_point=world_points[-1],face_width=side_width,miter_limit=miter_limit,)
			if end_trimmed:
				index=len(vertices_out)
				vertices_out.append(point + side["normal"] * surface_offset)
			else:
				index=make_endpoint_outer_vertex(source_vertex,work_chain_edges[-1],side,point,)
		...
		if (previous_side["face"]==next_side["face"]
			or faces_are_coplanar(previous_side["face"],next_side["face"],)):
			previous_width=previous_segment[f"{side_key}_width"]
			next_width=next_segment[f"{side_key}_width"]
			corner_point=planar_line_intersection(world_points[vertex_index],previous_segment["tangent"],next_segment["tangent"],previous_side["inward"],next_side["inward"],previous_side["normal"],previous_width,next_width,miter_limit,)
			if clamp_edge_overlaps:
				corner_point=bound_overlap_miter_point(world_points[vertex_index],corner_point,
					({"tangent": previous_segment["tangent"],"length": previous_segment["length"], "width": previous_width,},{"tangent": next_segment["tangent"],"length": next_segment["length"], "width": next_width,},),)
			source_vertex=source_vertex_at_world_point(chain_verts,world_matrix,world_points[vertex_index],)
			incident_selected=([edge for edge in source_vertex.link_edges if edge in selected_edge_set and len(edge.link_faces)==2] if source_vertex is not None else [])
			cache_key=(("outer",source_vertex.index,previous_side["face"].index)
				if source_vertex is not None and len(incident_selected) >=3 else None)
			index=append_cached_junction_vertex(vertices_out,junction_vertex_cache,cache_key,offset_point_from_face_normals(corner_point,(previous_side["normal"],next_side["normal"],),surface_offset,),)
			outer[previous_segment_index][previous_slot]=index
			outer[next_segment_index][next_slot]=index
			return
		if assign_topology_sector_endpoints(vertex_index,side_key,previous_segment_index,next_segment_index,previous_segment,next_segment,previous_side,next_side,
			previous_slot,next_slot,):
			return
		assign_local_bevel_join(vertex_index,side_key,previous_segment_index,next_segment_index,previous_segment,next_segment,previous_side,next_side,previous_slot,next_slot,)
	...
	for segment_index,segment in enumerate(segment_data):
		...
		side_a_indices,side_a_uvs=oriented_face_with_uvs((a0,c0,c1,a1),((0.0,v0),(center_u,v0),(center_u,v1),(0.0,v1)),vertices_out,segment["a"]["normal"],)
		side_b_indices,side_b_uvs=oriented_face_with_uvs((c0,b0,b1,c1),((center_u,v0),(outer_b_u,v0),(outer_b_u,v1),(center_u,v1)),vertices_out,segment["b"]["normal"],)
	...
	for patch in corner_patches:
		...
		if patch["side"]== "a":
			indices=(patch["previous_index"],patch["center_index"],patch["next_index"],)
			uvs=((0.0,patch_v),(0.5,patch_v),(0.0,patch_v),)
		else:
			indices=(patch["previous_index"],patch["next_index"],patch["center_index"],)
			uvs=((1.0,patch_v),(1.0,patch_v),(0.5,patch_v),)
		indices,uvs=oriented_face_with_uvs(indices,uvs,vertices_out,patch["normal"],)
		faces_out.append(indices)
		face_uvs_out.append(uvs)
		created_faces +=1
	return created_faces
def build_corner_strip_tapered(chain_verts,chain_edges,closed,selected_edge_set,all_source_edges,world_matrix,normal_matrix,face_width,decal_amount,slice_positions,forced_slice_interval,taper_sliced_ends,force_taper_start,force_taper_end,slice_taper_length,auto_face_width,auto_width_samples,auto_width_clearance,clamp_edge_overlaps,overlap_clearance,surface_offset,miter_limit,use_face_loop_slide,vertices_out,faces_out,face_uvs_out,center_vertices_out,width_search_context=None,face_width_resolved=False,junction_vertex_cache=None,):
	if not face_width_resolved:
		face_width=resolve_relative_face_width(face_width,all_source_edges,world_matrix)
	else:
		face_width=max(float(face_width),1.0e-8)
	...
	def solve_side_at_vertex(vertex_index,side_key):
		if not closed and vertex_index==0:
			...
			if start_trimmed:
				...
				index=append_endpoint_junction_vertex(vertices_out,junction_vertex_cache,source_vertex,work_chain_edges[0],side,selected_edge_set,point,surface_offset,)
			outer[0][f"{side_key}_start"]=index
			return
		if not closed and vertex_index==point_count - 1:
			side=segment_data[-1][side_key]
			...
			if end_trimmed:
				...
				index=append_endpoint_junction_vertex(vertices_out,junction_vertex_cache,source_vertex,work_chain_edges[-1],side,selected_edge_set,point,surface_offset,)
			outer[-1][f"{side_key}_end"]=index
			return
		previous_segment_index=(vertex_index - 1) % edge_count
		next_segment_index=vertex_index % edge_count
		...
		if (source_vertex is not None
			and support_faces_share_edge_at_vertex(previous_side["face"],next_side["face"],
				source_vertex,)):
			previous_index,next_index=topology_sector_endpoint_indices(source_vertex=source_vertex,previous_edge=work_chain_edges[previous_segment_index],next_edge=work_chain_edges[next_segment_index],previous_tangent_away=-previous_segment["tangent"],next_tangent_away=next_segment["tangent"],previous_side=previous_side,next_side=next_side,previous_width=previous_segment[f"{side_key}_width"],next_width=next_segment[f"{side_key}_width"],selected_edge_set=selected_edge_set,world_matrix=world_matrix,normal_matrix=normal_matrix,vertex_point=world_points[vertex_index],miter_limit=miter_limit,surface_offset=surface_offset,vertices_out=vertices_out,junction_vertex_cache=junction_vertex_cache,)
			outer[previous_segment_index][previous_slot]=previous_index
			outer[next_segment_index][next_slot]=next_index
			return
		assign_local_bevel_join(vertex_index,side_key,previous_segment_index,next_segment_index,previous_segment,next_segment,previous_side,next_side,previous_slot,next_slot,)
	...
	for segment_index,segment in enumerate(segment_data):
		...
		side_a_indices,side_a_uvs=oriented_face_with_uvs(side_a_indices,side_a_uvs,vertices_out,segment["a"]["normal"],)
		side_b_indices,side_b_uvs=oriented_face_with_uvs(side_b_indices,side_b_uvs,vertices_out,segment["b"]["normal"],)
		faces_out.append(side_a_indices)
		face_uvs_out.append(side_a_uvs)
		faces_out.append(side_b_indices)
		face_uvs_out.append(side_b_uvs)
	...
	for patch in corner_patches:
		...
		if patch["side"]== "a":
			indices=(patch["previous_index"],patch["center_index"],patch["next_index"],)
			uvs=((patch_v,0.0),(patch_v,0.5),(patch_v,0.0),)
		else:
			indices=(patch["previous_index"],patch["next_index"],patch["center_index"],)
			uvs=((patch_v,1.0),(patch_v,1.0),(patch_v,0.5),)
		indices,uvs=oriented_face_with_uvs(indices,uvs,vertices_out,patch["normal"],)
		faces_out.append(indices)
		face_uvs_out.append(uvs)
		created_faces +=1
...
def build_world_bvh_from_bmesh(bm,world_matrix):
	...
	vertices=[world_matrix @ vert.co for vert in bm.verts]
	polygons=[tuple(vert.index for vert in face.verts) for face in bm.faces if len(face.verts) >=3]
	if not vertices or not polygons:
		return None
	return BVHTree.FromPolygons(vertices,polygons,all_triangles=False,)
def resolved_crevice_ao_distance(requested_distance,face_width,):
	if requested_distance > EPSILON:
		return requested_distance
	return max(face_width * 4.0,1.0e-4,)
def crevice_ao_threshold(removal_amount):
	amount=max(0.0,min(1.0,removal_amount))
	return 0.78 - amount * 0.63
def local_cross_section_ao_score(bvh,sample_frames,distance,ray_samples,bias,):
	...
	for point,open_normal,tangent in sample_frames:
		tangent=safe_normalized(tangent,Vector((1.0,0.0,0.0)),)
		open_normal=(open_normal
			- tangent * open_normal.dot(tangent))
		open_normal=safe_normalized(open_normal,Vector((0.0,0.0,1.0)),)
		side=safe_normalized(tangent.cross(open_normal),Vector((0.0,1.0,0.0)),)
		open_normal=safe_normalized(side.cross(tangent),open_normal,)
		origin=point + open_normal * bias
		for sample_index in range(ray_samples):
			factor=((sample_index + 0.5) / ray_samples)
			angle=(-fan_limit + factor * fan_limit * 2.0)
			direction=safe_normalized(open_normal * cos(angle)
				+ side * sin(angle),open_normal,)
			location,_normal,_index,hit_distance=bvh.ray_cast(origin,direction,distance,)
			ray_count +=1
...
def edge_ao_score(edge,world_matrix,normal_matrix,bvh,distance,ray_samples,bias,):
	if len(edge.link_faces) !=2:
		return 0.0
	point_a=world_matrix @ edge.verts[0].co
	point_b=world_matrix @ edge.verts[1].co
	tangent=safe_normalized(point_b - point_a,Vector((1.0,0.0,0.0)),)
	normal_a=transform_normal(normal_matrix,edge.link_faces[0].normal,)
	normal_b=transform_normal(normal_matrix,edge.link_faces[1].normal,)
	open_normal=safe_normalized(normal_a + normal_b,normal_a,)
	sample_frames=[(point_a.lerp(point_b,factor),open_normal,tangent,) for factor in (0.2,0.5,0.8)]
	return local_cross_section_ao_score(bvh,sample_frames,distance,ray_samples,bias,)
def edge_is_concave_geometric(edge,world_matrix,normal_matrix,):
	if len(edge.link_faces) !=2:
		return False
	...
	normal_a=transform_normal(normal_matrix,face_a.normal,)
	normal_b=transform_normal(normal_matrix,face_b.normal,)
	score=(normal_a.dot(direction_b)
	...
	try:
		...
		try:
			...
			return False
def filter_edges_by_crevice(edges,removal_amount,world_matrix,normal_matrix,detection_mode="AO",bvh=None,ao_distance=0.0,ao_samples=8,face_width=0.06,surface_offset=0.002,):
	amount=max(0.0,min(1.0,removal_amount))
	if amount <=EPSILON:
		return list(edges)
	angle_threshold=pi * (1.0 - amount)
	ao_threshold=crevice_ao_threshold(amount)
	resolved_distance=resolved_crevice_ao_distance(ao_distance,face_width,)
	bias=max(surface_offset * 2.0,resolved_distance * 0.005,1.0e-5,)
	kept=[]
	for edge in edges:
		if len(edge.link_faces) !=2:
			kept.append(edge)
		...
		if detection_mode== "AO" and bvh is not None:
			if face_angle < angle_threshold:
				kept.append(edge)
				continue
			if not edge_is_concave_geometric(edge,world_matrix,
				normal_matrix,):
				kept.append(edge)
				continue
			score=edge_ao_score(edge,world_matrix,normal_matrix,bvh,resolved_distance,ao_samples,bias,)
			should_remove=score >=ao_threshold
		else:
			should_remove=(edge_is_concave_geometric(edge,world_matrix,normal_matrix,) and face_angle >=angle_threshold)
		if should_remove:
			continue
		kept.append(edge)
	return kept
def select_edges_by_crevice_mask(edges,world_matrix,normal_matrix,detection_mode="AO",bvh=None,ao_distance=0.0,ao_samples=8,face_width=0.06,surface_offset=0.002,):
	candidates=list(edges)
	kept=set(filter_edges_by_crevice(candidates,1.0,world_matrix,normal_matrix,detection_mode,bvh,ao_distance,ao_samples,face_width,surface_offset,))
	return [edge for edge in candidates if edge not in kept]
def filter_edges_by_minimum_length(edges,world_matrix,minimum_length,):
	...
	for edge in edges:
		...
		if (p1 - p0).length >=minimum_length:
			kept.append(edge)
	return kept
def filter_edge_chains_by_minimum_length(edges,world_matrix,minimum_length,):
	if minimum_length <=0.0:
		return list(edges)
	kept=[]
	seen=set()
	for _chain_verts,chain_edges,_closed in extract_edge_chains(edges,world_matrix,):
		total_length=0.0
...
def chain_world_length(chain_verts,chain_edges,closed,world_matrix):
	...
	total=sum((points[index + 1] - points[index]).length
...
def chain_random_signature(chain_verts,chain_edges,world_matrix):
	...
	for edge_index in sorted(edge.index for edge in chain_edges):
		signature=mix_random_u64(signature ^ mix_random_u64(edge_index + 1))
	quantized_points=sorted(tuple(int(round(component * 100000.0)) for component in point)
		for point in (world_matrix @ vertex.co for vertex in chain_verts))
	for point in quantized_points:
		for component in point:
			signature=mix_random_u64(signature ^ mix_random_u64(component))
	return signature
def chain_random_stream_seed(seed,signature,stream):
	return mix_random_u64(mix_random_u64(seed)
		^ mix_random_u64(signature)
		^ mix_random_u64(stream))
def randomized_face_widths_for_edge_groups(edge_groups,minimum_face_width,maximum_face_width,seed,world_matrix,):
	widths={}
	all_edges=list(dict.fromkeys(edge
		for group in (edge_groups or ()) for edge in group))
	...
	while remaining:
		...
		component_vertices=list({vertex for edge in component_edges for vertex in edge.verts})
		width=randomized_face_width(minimum_face_width,maximum_face_width,seed,chain_random_signature(component_vertices,component_edges,world_matrix,),)
		for edge in component_edges:
			widths[edge]=width
	return widths
def select_chains_by_global_amount(source_chains,amount,seed,world_matrix,pins,minimum_fragment_length=0.0,edge_slice=0.0,maximum_decal_length=0.0,decal_layer=None,minimum_gap_length=0.0,):
	...
	for source_index,(verts,edges,closed) in enumerate(source_chains):
		...
		if length <=EPSILON:
			continue
		records.append({"verts": verts,"edges": edges,"closed": closed,"length": length,"source_index": source_index,"random_signature": chain_random_signature(verts,edges,
				world_matrix,),"pin_index": pin_index_for_layer_cycle(source_index,pins,decal_layer,seed,),})
	if not records:
		return []
	selected=[]
	...
	for record in records:
		rng=random.Random(chain_random_stream_seed(seed,record["random_signature"],0x5F3759DF,))
	...
	for _iteration in range(48):
		...
		allocated=sum(record["length"] * min(1.0,middle * weight)
			for record,weight in zip(records,density_weights))
		if allocated < target_keep_length:
			low_scale=middle
		else:
			high_scale=middle
	chain_keep_lengths=[record["length"] * min(1.0,high_scale * weight) for record,weight in zip(records,density_weights)]
	def randomized_partition(total,count,rng,minimum_value=0.0,maximum_value=None):
		...
		while remaining > EPSILON and active:
			weights={index: 0.08 + rng.random() ** 2.2 for index in active}
	...
	for record,keep_length in zip(records,chain_keep_lengths):
		...
		if minimum > EPSILON and keep_length >=minimum:
			fragment_count=min(fragment_count,max(1,int(keep_length / minimum)),)
		elif keep_length < minimum:
			fragment_count=1
		removed_length=max(0.0,length - keep_length)
		if fragment_count > 1 and minimum_gap > EPSILON:
			maximum_fragments_for_visible_gaps=(1 + int((removed_length + EPSILON) / minimum_gap))
			fragment_count=min(fragment_count,max(1,maximum_fragments_for_visible_gaps),)
		rng=random.Random(chain_random_stream_seed(seed,record["random_signature"],0xA511E9B3,))
		strict_fragment_maximum=(maximum_decal_length if maximum_decal_length > EPSILON else None)
		fragment_lengths=randomized_partition(keep_length,fragment_count,rng,minimum_value=(minimum if minimum > EPSILON and keep_length >=minimum * fragment_count else 0.0),maximum_value=strict_fragment_maximum,)
		internal_gap_total=minimum_gap * max(0,fragment_count - 1)
		remaining_gap_length=max(0.0,removed_length - internal_gap_total)
		gap_lengths=randomized_partition(remaining_gap_length,fragment_count + 1,rng,)
		...
		for fragment_index in range(fragment_count):
			...
			if kept["closed"]:
				...
				kept["closed"]=False
			selected.append(kept)
	selected.sort(key=lambda record: (record["source_index"],record.get("fragment_index",-1),))
	return selected
...
def current_uv_slice_positions(pin_index=None):
	...
	return sorted({round(max(0.0,min(1.0,float(slice_pin.u))),6)
...
def snapped_slice_interval(amount,slice_positions):
	...
	candidates=sorted(set([0.0,1.0]
			+ [round(max(0.0,min(1.0,position)),6) for position in slice_positions] ))
	best=None
...
def trim_open_chain_by_amount(chain_verts,chain_edges,world_matrix,decal_amount,slice_positions=None,forced_interval=None,):
	world_points=[world_matrix @ vert.co for vert in chain_verts]
	edge_count=len(chain_edges)
	...
	if forced_interval is None and amount >=1.0 - EPSILON:
		return world_points,chain_edges,False,False
	edge_lengths=[(world_points[index + 1] - world_points[index]).length for index in range(edge_count)]
	...
	if forced_interval is not None:
		...
		start_fraction,end_fraction=snapped_slice_interval(amount,slice_positions or [],)
	...
	def sample_at_distance(distance_along_chain):
		...
		for edge_index,edge_length in enumerate(edge_lengths):
			if distance_remaining <=edge_length + EPSILON:
				...
				point=world_points[edge_index].lerp(world_points[edge_index + 1],factor,)
				return edge_index,factor,point
			distance_remaining -=edge_length
		return edge_count - 1,1.0,world_points[-1]
	start_edge_index,start_factor,start_point=sample_at_distance(start_distance)
	end_edge_index,end_factor,end_point=sample_at_distance(end_distance)
	...
	if start_edge_index==end_edge_index:
		if (end_point - start_point).length <=EPSILON:
			tangent=safe_normalized(world_points[start_edge_index + 1] - world_points[start_edge_index],Vector((1.0,0.0,0.0)),)
