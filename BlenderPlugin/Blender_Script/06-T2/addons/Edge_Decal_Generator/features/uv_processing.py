...
def call_operator_by_idname(bl_idname):
	...
	if operator_group is None:
		raise RuntimeError(f"找不到运算符类别: {category}")
	operator=getattr(operator_group,operator_name,None)
	if operator is None:
		raise RuntimeError(f"找不到运算符: {bl_idname}")
...
class _BufferedUVLayer:
	...
	def __init__(self,coordinates):
		self.data=[_BufferedUVLoop(Vector(coordinate)) for coordinate in coordinates]
def buffered_uv_layer(uv_layer):
	...
	try:
		...
		coordinates=((flat_coordinates[index],flat_coordinates[index + 1])
			for index in range(0,len(flat_coordinates),2))
	except (AttributeError,RuntimeError,TypeError,ValueError):
		coordinates=(item.uv.copy() for item in uv_layer.data)
	return _BufferedUVLayer(coordinates)
def flush_buffered_uv_layer(buffered_layer,uv_layer):
	flat_coordinates=[component for item in buffered_layer.data for component in item.uv]
	try:
		uv_layer.data.foreach_set("uv",flat_coordinates)
	except (AttributeError,RuntimeError,TypeError,ValueError):
		for target,source in zip(uv_layer.data,buffered_layer.data):
			target.uv=source.uv
def collect_selected_uv_islands(mesh,selected_only=True,uv_layer=None,):
	if uv_layer is None:
	...
	if selected_only:
		participating_polygons=[polygon for polygon in polygons if polygon.select]
	else:
		participating_polygons=list(polygons)
	if not participating_polygons:
		return []
	adjacency={polygon.index: set() for polygon in participating_polygons}
	edge_users={}
	for polygon in participating_polygons:
		...
		for local_index,loop_index in enumerate(loop_indices):
			...
			edge_users.setdefault(edge_key,[]).append((polygon.index,vertex_a,uv_data[loop_index].uv.copy(),vertex_b,uv_data[next_loop_index].uv.copy(),))
	uv_tolerance_sq=1.0e-12
	for edge_key,users in edge_users.items():
		if len(users) !=2:
			continue
		(polygon_a,vertex_a_0,uv_a_0,vertex_a_1,uv_a_1,)=users[0]
		(polygon_b,vertex_b_0,uv_b_0,vertex_b_1,uv_b_1,)=users[1]
		uv_map_a={vertex_a_0: uv_a_0,vertex_a_1: uv_a_1,}
		uv_map_b={vertex_b_0: uv_b_0,vertex_b_1: uv_b_1,}
		edge_vertex_a,edge_vertex_b=edge_key
		if ((uv_map_a[edge_vertex_a] - uv_map_b[edge_vertex_a]).length_squared
			<=uv_tolerance_sq
			and
	...
	while unvisited:
		...
		if loop_indices:
			islands.append({"polygons": sorted(polygon_group), "loops": loop_indices,})
	return islands
def rotate_uv_island(uv_layer,loop_indices,angle):
	...
	for loop_index in loop_indices:
		...
		relative=uv - center
		rotated=Vector((relative.x * cosine - relative.y * sine,relative.x * sine + relative.y * cosine,))
...
def centerline_uv_axis_angle(decal_obj,uv_layer,loop_indices,center_vertices=None,):
...
def align_uv_island_horizontally(decal_obj,uv_layer,loop_indices,center_vertices=None,):
	axis_angle=centerline_uv_axis_angle(decal_obj,uv_layer,loop_indices,center_vertices=center_vertices,)
	if axis_angle is None:
		axis_angle=principal_uv_axis_angle(uv_layer,loop_indices,)
	rotate_uv_island(uv_layer,loop_indices,-axis_angle,)
...
def average_uv_island_scales(mesh,uv_layer,islands):
	...
	for island in islands:
		mesh_area=sum(mesh.polygons[index].area for index in island["polygons"])
		uv_area=sum(uv_polygon_area(uv_layer,mesh.polygons[index]) for index in island["polygons"])
...
def set_uv_texel_density(mesh,uv_layer,islands,pixels_per_centimeter,texture_resolution,scene_scale_length,):
	if texture_resolution <=0 or pixels_per_centimeter <=0.0:
		return
	unit_scale=scene_scale_length if scene_scale_length > 0.0 else 1.0
	target_uv_per_blender_unit=(pixels_per_centimeter * 100.0 * unit_scale) / float(texture_resolution)
	for island in islands:
		mesh_area=sum(mesh.polygons[index].area for index in island["polygons"])
		uv_area=sum(uv_polygon_area(uv_layer,mesh.polygons[index]) for index in island["polygons"])
		if mesh_area <=EPSILON or uv_area <=EPSILON:
		...
		if current_uv_per_blender_unit <=EPSILON:
			continue
		scale_uv_island_about_center(uv_layer,island["loops"],target_uv_per_blender_unit / current_uv_per_blender_unit,)
def apply_uv_scale_multiplier(uv_layer,islands,scale):
	if abs(scale - 1.0) <=EPSILON:
		return
	for island in islands:
		scale_uv_island_about_center(uv_layer,island["loops"],scale,)
def measured_texel_density(mesh,uv_layer,islands,texture_resolution,scene_scale_length,):
	total_mesh_area=0.0
	total_uv_area=0.0
	for island in islands:
		total_mesh_area +=sum(mesh.polygons[index].area for index in island["polygons"])
		total_uv_area +=sum(uv_polygon_area(uv_layer,mesh.polygons[index]) for index in island["polygons"])
	if total_mesh_area <=EPSILON or total_uv_area <=EPSILON:
		return 0.0
	unit_scale=scene_scale_length if scene_scale_length > 0.0 else 1.0
...
def translate_uv_island_to_center(uv_layer,loop_indices,target_center,scale=1.0,):
	min_u,max_u,min_v,max_v=uv_bounds(uv_layer,loop_indices)
	current_center=Vector(((min_u + max_u) * 0.5,(min_v + max_v) * 0.5,))
	for loop_index in loop_indices:
		uv=uv_layer.data[loop_index].uv
		uv_layer.data[loop_index].uv=(target_center + (uv - current_center) * scale)
def place_exact_density_islands_in_quarter_strips(uv_layer,islands,strip_indices,):
	strip_height=0.25
	for island,strip_index in zip(islands,strip_indices):
		min_u,max_u,min_v,max_v=uv_bounds(uv_layer,island["loops"],)
		current_center=Vector(((min_u + max_u) * 0.5,(min_v + max_v) * 0.5,))
		target_center=Vector((0.5,strip_index * strip_height + strip_height * 0.5,))
		translation=target_center - current_center
		for loop_index in island["loops"]:
			uv_layer.data[loop_index].uv +=translation
def place_averaged_islands_in_quarter_strips(uv_layer,islands,strip_indices,padding,):
	strip_height=0.25
	...
	for island in islands:
		...
		common_scale=min(common_scale,available_width / width,available_height / height,)
	if common_scale==float("inf"):
		common_scale=1.0
	common_scale=min(1.0,common_scale)
	for island,strip_index in zip(islands,strip_indices):
		target_center=Vector((0.5,strip_index * strip_height + strip_height * 0.5,))
		translate_uv_island_to_center(uv_layer,island["loops"],target_center,common_scale,)
...
def randomize_uv_islands_horizontally(uv_layer,islands,rng,padding,amount=1.0,):
	if not islands:
		return
	ordered_islands=sorted(islands,
	...
	if horizontal_span <=EPSILON:
		return
	phase=rng.random()
	target_centers=[(((slot + phase) / island_count) - 0.5) * horizontal_span for slot in range(island_count)]
...
def randomize_decal_uv_islands_horizontally(decal_obj,random_seed,padding=0.0,amount=1.0,):
	...
	islands=collect_selected_uv_islands(mesh,selected_only=False,uv_layer=uv_layer,)
	if not islands:
		return 0
	randomize_uv_islands_horizontally(uv_layer,islands,random.Random(random_seed),padding,amount,)
	...
	for vertex in decal_obj.data.vertices:
		for membership in vertex.groups:
			if membership.group==group_index and membership.weight > 0.0:
				...
				break
	return result
def align_uv_centerlines_to_strips(decal_obj,uv_layer,islands,strip_indices,center_vertices=None,):
	if center_vertices is None:
		center_vertices=get_center_vertex_indices(decal_obj)
	if not center_vertices:
		return
	...
	mesh=decal_obj.data
	for island,strip_index in zip(islands,strip_indices):
		center_loop_indices=[loop_index
			for loop_index in island["loops"]
			if mesh.loops[loop_index].vertex_index in center_vertices]
		if not center_loop_indices:
			continue
		min_center_v=min(uv_layer.data[index].uv.y for index in center_loop_indices)
		max_center_v=max(uv_layer.data[index].uv.y for index in center_loop_indices)
		current_center_v=(min_center_v + max_center_v) * 0.5
		target_center_v=(strip_index * strip_height + strip_height * 0.5)
		offset_v=target_center_v - current_center_v
		for loop_index in island["loops"]:
			uv_layer.data[loop_index].uv.y +=offset_v
def postprocess_generated_uvs(decal_obj,uv_scale,set_target_density,target_texel_density,texture_resolution,scene_scale_length,average_island_scale,align_horizontally,place_in_quarter_strips,randomize_quarter_strip,randomize_horizontal_offset,horizontal_randomize_amount,random_seed,strip_padding,):
	mesh=decal_obj.data
	blender_uv_layer=mesh.uv_layers.active
	if blender_uv_layer is None:
		return 0,0.0
	...
	try:
		...
		pins_are_authoritative=bool(getattr(scene.edge_decal_settings, "auto_use_uv_pins",False)
	...
	center_vertices=(get_center_vertex_indices(decal_obj) if align_horizontally or place_in_quarter_strips else None)
	for island in islands:
		if align_horizontally:
			align_uv_island_horizontally(decal_obj,uv_layer,island["loops"],center_vertices=center_vertices,)
	if average_island_scale and not set_target_density:
		average_uv_island_scales(mesh,uv_layer,islands,)
	if set_target_density:
		set_uv_texel_density(mesh,uv_layer,islands,target_texel_density,texture_resolution,scene_scale_length,)
	rng=(random.Random(random_seed) if randomize_quarter_strip or randomize_horizontal_offset else None)
	strip_indices=None
	if place_in_quarter_strips:
		...
		if average_island_scale and not set_target_density:
			place_averaged_islands_in_quarter_strips(uv_layer,islands,strip_indices,strip_padding,)
		else:
			place_exact_density_islands_in_quarter_strips(uv_layer,islands,strip_indices,)
	apply_uv_scale_multiplier(uv_layer,islands,uv_scale,)
	if randomize_horizontal_offset:
		randomize_uv_islands_horizontally(uv_layer,islands,rng,strip_padding,horizontal_randomize_amount,)
	if strip_indices is not None:
		align_uv_centerlines_to_strips(decal_obj,uv_layer,islands,strip_indices,center_vertices=center_vertices,)
	density=0.0
	if set_target_density:
		density=measured_texel_density(mesh,uv_layer,islands,texture_resolution,scene_scale_length,)
	flush_buffered_uv_layer(uv_layer,blender_uv_layer)
	mesh.update()
...
def edge_decal_quad_components(bm):
	unvisited={face for face in bm.faces if len(face.verts)==4}
	components=[]
	while unvisited:
...
def edge_decal_walk_quad_edgeloop(start_loop,allowed_faces):
	...
	while True:
		...
		if (radial_loop.face not in allowed_faces
		...
		for base_loop in (face.loops[0],face.loops[1]):
			...
			for start_loop in (base_loop,opposite_loop):
				if start_loop.edge in edge_lengths:
					continue
				collected_edges.update(edge_decal_walk_quad_edgeloop( start_loop,allowed_faces,))
			if not collected_edges:
				continue
			average_length=sum(edge.calc_length()
				for edge in collected_edges) / len(collected_edges)
			for edge in collected_edges:
				edge_lengths[edge]=average_length
	return edge_lengths
def edge_decal_choose_quadrify_start_face(faces,uv_layer):
	face_centers={face: edge_decal_uv_centroid(face.loops,uv_layer) for face in faces}
	...
	component_center=Vector(((min_u + max_u) * 0.5,(min_v + max_v) * 0.5,))
	return min(faces,
		key=lambda face: (face_centers[face] - component_center).length_squared,)
def edge_decal_assign_uv_to_matching_loops(loop,uv_layer,original_uv,target_uv,):
	tolerance_sq=1.0e-12
	for linked_loop in loop.vert.link_loops:
		if (linked_loop[uv_layer].uv - original_uv).length_squared <=tolerance_sq:
			linked_loop[uv_layer].uv=target_uv
def edge_decal_align_quadrify_seed_face(face,uv_layer,edge_lengths,even_shape=False,center_vertex_indices=None,):
	loops=list(face.loops)
	center_vertex_indices=center_vertex_indices or set()
	semantic_start=None
	for index,loop in enumerate(loops):
		next_loop=loop.link_loop_next
		if (loop.vert.index in center_vertex_indices
	...
	if semantic_start is not None:
		...
		first_uv_length=(loops[1][uv_layer].uv - loops[0][uv_layer].uv).length
		second_uv_length=(loops[2][uv_layer].uv - loops[1][uv_layer].uv).length
		start_index=0 if first_uv_length >=second_uv_length else 1
	ordered=[loops[(start_index + offset) % 4] for offset in range(4)]
	master_loop=ordered[0]
	ortho_loop=ordered[1]
	master_mesh_length=edge_lengths.get(master_loop.edge,master_loop.edge.calc_length(),)
	ortho_mesh_length=edge_lengths.get(ortho_loop.edge,ortho_loop.edge.calc_length(),)
	master_uv_vector=(ordered[1][uv_layer].uv - ordered[0][uv_layer].uv)
	...
	if even_shape:
		...
		ortho_uv_length=(master_uv_length * ortho_mesh_length / master_mesh_length)
	else:
		ortho_uv_length=master_uv_length
	if abs(master_uv_vector.x) >=abs(master_uv_vector.y):
		axis=Vector((1.0 if master_uv_vector.x >=0.0 else -1.0,0.0,))
	else:
		axis=Vector((0.0,1.0 if master_uv_vector.y >=0.0 else -1.0,))
	master_vector=axis * master_uv_length
	ortho_vector=master_vector.orthogonal().normalized() * ortho_uv_length
	current_ortho=(ordered[3][uv_layer].uv - ordered[0][uv_layer].uv)
	if ortho_vector.dot(current_ortho) < 0.0:
		ortho_vector.negate()
	new_coordinates=[Vector((0.0,0.0)),master_vector,master_vector + ortho_vector,ortho_vector,]
	...
	for loop,coordinate in zip(ordered,new_coordinates):
		...
		edge_decal_assign_uv_to_matching_loops(loop,uv_layer,original_uv,target_uv,)
def edge_decal_yield_adjacent_quad_loops(start_face,faces):
	...
	while queue:
		...
		for master_loop in face.loops:
			...
			if (next_face not in allowed_faces
			...
			yield next_loop
def edge_decal_quadrify_component(bm,faces,uv_layer,average_shape=True,even_shape=False,center_vertex_indices=None,):
	if not faces:
		return 0
	edge_lengths=(edge_decal_average_quad_edge_lengths(bm,faces) if average_shape else {})
	start_face=edge_decal_choose_quadrify_start_face(faces,uv_layer,)
	edge_decal_align_quadrify_seed_face(start_face,uv_layer,edge_lengths,even_shape=even_shape,center_vertex_indices=center_vertex_indices,)
	processed_faces=1
	for loop in edge_decal_yield_adjacent_quad_loops(start_face,faces,):
		master_loop=loop.link_loop_radial_next
		master_vector=(master_loop[uv_layer].uv - master_loop.link_loop_next[uv_layer].uv)
		...
		if even_shape:
			...
			master_mesh_length=edge_lengths.get(master_edge,master_edge.calc_length(),)
			ortho_mesh_length=edge_lengths.get(ortho_edge,ortho_edge.calc_length(),)
			if master_mesh_length > EPSILON:
				ortho_uv_length=(master_vector.length * ortho_mesh_length / master_mesh_length)
			else:
				ortho_uv_length=master_vector.length
		ortho_vector=(master_vector.orthogonal().normalized() * ortho_uv_length)
		master_shared_mid=(master_loop[uv_layer].uv
			+ master_loop.link_loop_next[uv_layer].uv) * 0.5
		master_opposite_mid=(master_loop.link_loop_next.link_loop_next[uv_layer].uv
			+ master_loop.link_loop_prev[uv_layer].uv) * 0.5
		master_interior_vector=(master_opposite_mid - master_shared_mid)
		if (master_interior_vector.length_squared > EPSILON
			and ortho_vector.dot(master_interior_vector) > 0.0):
			ortho_vector.negate()
		target_coordinates=(master_loop.link_loop_next[uv_layer].uv.copy(),master_loop[uv_layer].uv.copy(),master_loop[uv_layer].uv + ortho_vector,master_loop.link_loop_next[uv_layer].uv + ortho_vector,)
		target_loops=(loop,loop.link_loop_next,loop.link_loop_next.link_loop_next,loop.link_loop_prev,)
		for target_loop,coordinate in zip(target_loops,target_coordinates,):
...
def capture_taper_triangle_uv_bindings(bm,uv_layer):
	...
	for face in bm.faces:
		...
		bindings.append({"face_index": face.index,"quad_index": quad_face.index,"a_index": a_vert.index,"b_index": b_vert.index, "tip_index": tip_vert.index,"along": along, "signed_height": signed_height,})
	return bindings
...
def closest_point_on_triangle(point,point_a,point_b,point_c):
	...
	if area_a <=0.0 and (dot_ac_b - dot_ab_b) >=0.0 and (dot_ab_c - dot_ac_c) >=0.0:
...
def triangle_interpolated_uv(point,triangle_points,triangle_uvs):
	...
	inverse=1.0 / denominator
	weight_b=(dot_ac_ac * dot_offset_ab - dot_ab_ac * dot_offset_ac) * inverse
	weight_c=(dot_ab_ab * dot_offset_ac - dot_ab_ac * dot_offset_ab) * inverse
	weight_a=1.0 - weight_b - weight_c
	return (triangle_uvs[0] * weight_a + triangle_uvs[1] * weight_b + triangle_uvs[2] * weight_c)
def transfer_source_first_uv_to_decal(source_obj,decal_obj):
	...
	if source_obj.mode== "EDIT":
		try:
			bmesh.update_edit_mesh(source_mesh,loop_triangles=True,destructive=False,)
		except RuntimeError:
			pass
	...
	source_vertices=[source_world @ vertex.co for vertex in source_mesh.vertices]
	triangle_records=[]
	triangle_indices=[]
	...
	for loop_triangle in source_mesh.loop_triangles:
		...
		if (points[1] - points[0]).cross(points[2] - points[0]).length_squared <=(EPSILON * EPSILON):
			continue
		record={"polygon_index": int(loop_triangle.polygon_index),"points": points, "uvs": tuple(source_uv_layer.data[loop_index].uv.copy() for loop_index in loop_triangle.loops),}
		record_index=len(triangle_records)
		triangle_records.append(record)
		triangle_indices.append(vertex_indices)
		triangles_by_polygon.setdefault(record["polygon_index"],[]).append(record_index)
	if not triangle_records:
		return None
	source_bvh=BVHTree.FromPolygons(source_vertices,triangle_indices,all_triangles=True,)
	...
	if second_uv_layer is None:
		if len(decal_mesh.uv_layers) >=2:
			...
			second_uv_layer=decal_mesh.uv_layers.new(name=SECOND_UV_LAYER_NAME)
		for primary_uv,second_uv in zip(primary_uv_layer.data,second_uv_layer.data,):
	...
	for polygon in decal_mesh.polygons:
		...
		if triangle_index is None or triangle_index >=len(triangle_records):
			failed_polygons +=1
			continue
		source_polygon_index=triangle_records[triangle_index]["polygon_index"]
		candidate_indices=triangles_by_polygon.get(source_polygon_index,(triangle_index,),)
		for loop_index in polygon.loop_indices:
			...
			for candidate_index in candidate_indices:
				...
				projected=closest_point_on_triangle(point_world,*record["points"],)
			...
			if best_record is None:
				continue
			second_uv_layer.data[loop_index].uv=triangle_interpolated_uv(best_point,best_record["points"],best_record["uvs"],)
			transferred_loops +=1
...
def decal_uses_source_matched_uv(decal_obj):
	...
	if data is not None and bool(getattr(data, "match_source_material",False)):
		return True
	mesh=getattr(decal_obj, "data",None)
	return bool(mesh is not None
		and any(material is not None
			and material.get("edge_decal_source_matched") for material in mesh.materials))
def generate_conformal_second_uv(context,decal_obj,target_texel_density,texture_resolution,scene_scale_length,):
	if decal_obj is None or decal_obj.data is None:
		return 0,0.0
	mesh=decal_obj.data
	...
	try:
		...
		temporary_mesh=bpy.data.meshes.new(f"{mesh.name}_SecondUVUnwrap")
		temporary_mesh.from_pydata([tuple(vertex.co) for vertex in mesh.vertices],[],[tuple(polygon.vertices) for polygon in mesh.polygons],)
		temporary_mesh.update(calc_edges=True)
		seam_keys={frozenset(edge.vertices) for edge in mesh.edges if edge.use_seam}
		for edge in temporary_mesh.edges:
			edge.use_seam=frozenset(edge.vertices) in seam_keys
		temporary_mesh.update()
		temporary_object=bpy.data.objects.new(f"{decal_obj.name}_SecondUVUnwrap",temporary_mesh,)
		...
		conformal_face_uvs={face.index: [(loop.vert.index,loop[temporary_bmesh_uv].uv.copy()) for loop in face.loops] for face in temporary_bmesh.faces}
		force_object_mode(context)
		for polygon in mesh.polygons:
			...
			for loop_index in polygon.loop_indices:
				...
				second_uv_layer.data[loop_index].uv=candidates[min(used_index,len(candidates) - 1)]
		...
		mesh.update()
def generate_decal_second_uv(context,source_obj,decal_obj,target_texel_density,texture_resolution,scene_scale_length,):
	if source_obj is not None and decal_uses_source_matched_uv(decal_obj):
		transferred=transfer_source_first_uv_to_decal(source_obj,decal_obj,)
		if transferred is not None:
			return 0,0.0
	return generate_conformal_second_uv(context,decal_obj,target_texel_density,texture_resolution,scene_scale_length,)
def integrated_quadrify_decal_mesh(decal_obj,average_shape=True,even_shape=False,):
	mesh=decal_obj.data
	...
	try:
		...
		for component in components:
			count=edge_decal_quadrify_component(bm,component,uv_layer,average_shape=average_shape,even_shape=even_shape,center_vertex_indices=center_vertex_indices,)
			if count:
				processed_components +=1
				processed_faces +=count
		restore_taper_triangle_uv_bindings(bm,uv_layer,taper_triangle_bindings,)
		...
		for face in bm.faces:
			...
			if len(face.loops) !=len(polygon.loop_indices):
				continue
			for source_loop,loop_index in zip(face.loops,polygon.loop_indices,):
...
def follow_active_quads_per_component(decal_obj):
	...
	for component in components:
		...
		for face in component:
			face.select=True
		active_face=max(component,
			key=lambda face: face.calc_area(),)
		bm.faces.active=active_face
		bmesh.update_edit_mesh(decal_obj.data,loop_triangles=False,destructive=False,)
		try:
	...
	bmesh.update_edit_mesh(decal_obj.data,loop_triangles=False,destructive=False,)
	return processed
def unwrap_generated_decal(context,source_obj,decal_obj,use_integrated_quadrify,integrated_average_shape,integrated_even_shape,use_follow_active_quads,uv_scale,set_target_density,target_texel_density,texture_resolution,scene_scale_length,generate_second_uv,average_island_scale,align_horizontally,place_in_quarter_strips,randomize_quarter_strip,randomize_horizontal_offset,horizontal_randomize_amount,random_seed,strip_padding,):
	...
	if use_integrated_quadrify:
		component_count,face_count=integrated_quadrify_decal_mesh(decal_obj,average_shape=integrated_average_shape,even_shape=integrated_even_shape,)
		if component_count:
		...
		if use_follow_active_quads:
			follow_count=follow_active_quads_per_component(decal_obj,)
			if follow_count > 0:
				quadrify_result= "FOLLOW_ACTIVE_QUADS"
		bpy.ops.object.mode_set(mode="OBJECT")
	processed_islands,measured_density=postprocess_generated_uvs(decal_obj,uv_scale,set_target_density,target_texel_density,texture_resolution,scene_scale_length,average_island_scale,align_horizontally,place_in_quarter_strips,randomize_quarter_strip,randomize_horizontal_offset,horizontal_randomize_amount,random_seed,strip_padding,)
	if generate_second_uv:
		generate_decal_second_uv(context,source_obj,decal_obj,target_texel_density,texture_resolution,scene_scale_length,)
...
class EDGEDECAL_OT_apply_uvs(Operator):
	bl_idname= "object.edge_decal_apply_uvs";bl_label= g("Apply UVs");bl_description=("运行UV贴花管道，无再生\n 或替换其手动编辑的拓扑");bl_options={"REGISTER", "UNDO"}
	@classmethod
	def poll(cls,context):
		decal_obj=resolved_editable_decal_layer(context)
		return bool(decal_obj is not None
	...
	def execute(self,context):
		...
		source_obj=getattr(decal_obj.edge_decal_object_settings,"source_object",None,)
		...
		selected_before=[obj.name_full for obj in context.selected_objects]
		edit_obj_before=getattr(context, "edit_object",None)
		started_in_edit_mode=context.mode== "EDIT_MESH"
		force_object_mode(context)
		vertex_selection=tuple(vertex.select for vertex in decal_obj.data.vertices)
		edge_selection=tuple(edge.select for edge in decal_obj.data.edges)
		polygon_selection=tuple(polygon.select for polygon in decal_obj.data.polygons)
		vertex_count=len(decal_obj.data.vertices)
		edge_count=len(decal_obj.data.edges)
		polygon_count=len(decal_obj.data.polygons)
		polygon_topology=tuple(tuple(polygon.vertices) for polygon in decal_obj.data.polygons)
		quadrify_result= "DIRECT_STRIP"
		processed_islands=0
		...
		try:
			...
			GR(self,"ERROR",f"Apply UVs failed: {error}")
			return {"CANCELLED"}
		finally:
			force_object_mode(context)
			for vertex,was_selected in zip(decal_obj.data.vertices,vertex_selection,):
				vertex.select=was_selected
			for edge,was_selected in zip(decal_obj.data.edges,edge_selection,):
				edge.select=was_selected
			for polygon,was_selected in zip(decal_obj.data.polygons,polygon_selection,):
		...
		GR(self,"INFO", "; ".join(details))
		return {"FINISHED"}
def chain_endpoint_corner_trim_interval(chain_verts,chain_edges,closed,world_matrix,bevel_width,multiplier=1.0,):
	if closed or not chain_edges or len(chain_verts) < 2 or bevel_width <=EPSILON:
		return None
	total_length=sum(((world_matrix @ chain_verts[index + 1].co) -
