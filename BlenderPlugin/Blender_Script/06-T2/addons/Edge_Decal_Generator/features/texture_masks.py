...
def generation_mask_layer(source_obj,context=None):
	...
	if target_name:
		...
		if decal_layer_is_valid(target,source_obj):
			return target
	return resolve_generation_target_layer(source_obj,context=context,include_locked=False,)
...
def texture_mask_paint_is_active(context,layer_obj=None):
	...
	return bool(image_paint.mode== "IMAGE" and image_paint.canvas==image)
def create_black_generation_mask(layer_obj,resolution=EDGEDECAL_MASK_DEFAULT_RESOLUTION):
	...
	image=bpy.data.images.new(name=f"{display_name}_GenerationMask",width=resolution,height=resolution,alpha=False,float_buffer=False,is_data=True,)
...
def generation_mask_sampler(source_bm,layer_obj):
	...
	return {"image": image,"uv_layer": uv_layer,"pixels": generation_mask_pixel_buffer(image),"width": int(image.size[0]), "height": int(image.size[1]), "threshold": max(1.0e-6,min(1.0,float(data.texture_mask_threshold)),),}
...
def generation_mask_value_on_edge(uv_pairs,sampler,factor):
	...
	for uv_a,uv_b in uv_pairs:
		...
		value=max(value,sample_generation_mask_value(sampler["pixels"],sampler["width"],sampler["height"],u,v,),)
	return value
def generation_mask_edge_intervals(edge,sampler):
	uv_pairs=generation_mask_edge_uv_pairs(edge,sampler["uv_layer"])
	if not uv_pairs:
		return [],[]
	pixel_length=max((((uv_b.x - uv_a.x) * sampler["width"]) ** 2
			+ ((uv_b.y - uv_a.y) * sampler["height"]) ** 2) ** 0.5 for uv_a,uv_b in uv_pairs)
	sample_count=max(2,min(EDGEDECAL_MASK_MAX_EDGE_SAMPLES,int(pixel_length / EDGEDECAL_MASK_SAMPLE_SPACING_PIXELS) + 2,),)
	threshold=sampler["threshold"]
	factors=[index / max(1,sample_count - 1) for index in range(sample_count)]
	states=[generation_mask_value_on_edge(uv_pairs,sampler,factor) >=threshold for factor in factors]
	transitions=[]
	for index in range(len(factors) - 1):
		...
		for _step in range(EDGEDECAL_MASK_TRANSITION_REFINEMENT_STEPS):
			middle=(low + high) * 0.5
			middle_state=(generation_mask_value_on_edge(uv_pairs,sampler,middle) >=threshold)
...
def split_bmesh_edges_by_generation_mask(work_bmesh,edges,layer_obj,):
	sampler=generation_mask_sampler(work_bmesh,layer_obj)
	if sampler is None:
		return list(edges),{"cut_count": 0,"partial_edge_count": 0, "cut_vertex_indices": set(),}
	selected_segments=[]
	cut_count=0
	...
	for edge in list(edges):
		if not edge.is_valid:
			continue
		white_intervals,transitions=generation_mask_edge_intervals(edge,sampler,)
		if not transitions:
			if white_intervals:
		...
		for start,end in zip(boundaries[:-1],boundaries[1:]):
			middle=(start + end) * 0.5
			enabled=any(white_start - EPSILON <=middle <=white_end + EPSILON for white_start,white_end in white_intervals)
		...
		for transition in transitions:
			...
			prefix_edge,new_vertex=bmesh.utils.edge_split(current_edge,current_start_vertex,max(EPSILON,min(1.0 - EPSILON,local_factor)),)
		...
		for segment,(_start,_end,enabled) in zip(split_segments,interval_records,):
			if enabled:
	...
	return selected_segments,{"cut_count": cut_count,"partial_edge_count": partial_edge_count,"cut_vertex_indices": {int(vertex.index) for vertex in cut_vertices if vertex.is_valid},}
class EDGEDECAL_OT_texture_mask_add(Operator):
	bl_idname= "object.edge_decal_texture_mask_add";bl_label= g("Add Layer Mask");bl_description=("为这个层创建一个实心黑色遮罩；绘制白色贴花\n 允许生成");bl_options={"REGISTER", "UNDO"}
	def draw(S,_):L=S.layout;[GP(L,S,n) for n in S.__annotations__]
	resolution: IntProperty(name="Resolution",default=EDGEDECAL_MASK_DEFAULT_RESOLUTION,min=64,max=8192,subtype="PIXEL",)
	def execute(self,context):
		layer_obj=active_texture_mask_layer(context)
		if layer_obj is None:
			GR(self,"ERROR", "No active decal layer")
			return {"CANCELLED"}
		data=layer_obj.edge_decal_object_settings
		image=getattr(data, "texture_mask",None)
		if image is None:
		...
		GR(self,"INFO","Black mask created. Paint white where this layer may generate.",)
		return {"FINISHED"}
class EDGEDECAL_OT_texture_mask_remove(Operator):
	bl_idname= "object.edge_decal_texture_mask_remove";bl_label= g("Remove Layer Mask");bl_description= "分离纹理遮罩并恢复无遮罩生成";bl_options={"REGISTER", "UNDO"}
	def execute(self,context):
		layer_obj=active_texture_mask_layer(context)
		if layer_obj is None:
			return {"CANCELLED"}
...
class EDGEDECAL_OT_texture_mask_reset(Operator):
	bl_idname= "object.edge_decal_texture_mask_reset";bl_label= g("Reset Mask to Black");bl_description= "擦除整个遮罩，这样层就不会生成任何内容";bl_options={"REGISTER", "UNDO"}
	def execute(self,context):
		layer_obj=active_texture_mask_layer(context)
		image=layer_generation_mask_image(layer_obj)
		if image is None or not reset_generation_mask_black(image):
			return {"CANCELLED"}
		GR(self,"INFO", "Layer mask reset to black")
		return {"FINISHED"}
class EDGEDECAL_OT_texture_mask_paint(Operator):
	bl_idname= "object.edge_decal_texture_mask_paint";bl_label= g("Paint Layer Mask");bl_description=("使用该遮罩作为源网格上Blender的纹理绘制画布")
	def execute(self,context):
		layer_obj=active_texture_mask_layer(context)
		image=layer_generation_mask_image(layer_obj)
		if layer_obj is None or image is None:
			GR(self,"ERROR", "Add and enable a layer mask first")
			return {"CANCELLED"}
		if texture_mask_paint_is_active(context,layer_obj):
			try:
				bpy.ops.object.mode_set(mode="OBJECT")
			except RuntimeError as error:
				GR(self,"ERROR",f"Could not exit Texture Paint: {error}")
				return {"CANCELLED"}
			GR(self,"INFO", "Exited layer mask Texture Paint")
			return {"FINISHED"}
		data=layer_obj.edge_decal_object_settings
		...
		if source_obj is None or source_obj.type != "MESH":
			GR(self,"ERROR", "The layer's source mesh is missing")
			return {"CANCELLED"}
		if source_obj.data.uv_layers.active is None:
			GR(self,"ERROR","Texture painting the layer mask needs an active source UV map",)
			return {"CANCELLED"}
		if context.mode != "OBJECT":
			try:
				bpy.ops.object.mode_set(mode="OBJECT")
			except RuntimeError:
				GR(self,"ERROR", "Could not leave the current mode")
				return {"CANCELLED"}
		...
		try:
			...
			GR(self,"ERROR",f"Could not start Texture Paint: {error}")
			return {"CANCELLED"}
		GR(self,"INFO", "Texture Paint ready: paint white to generate and black to block",)
		return {"FINISHED"}
