...
def edge_decal_default_user_library_root(create=False):
	return bpy.utils.user_resource("CONFIG",path="lazy_edge_decals",create=create,)
def edge_decal_user_library_root(create=False):
	preferences=edge_decal_addon_preferences()
	configured=str(getattr(preferences, "user_library_directory", "") or "").strip()
	if configured:
...
class EDGEDECAL_AP_preferences(AddonPreferences):
	bl_idname=AD
	user_library_directory: StringProperty(name="User Preset Library",description=("包含用户预设、材质资产库和拷贝的文件夹\n 纹理；保留空物体以使用Blender的配置目录"),subtype="DIR_PATH",default=JP(FP,"assets",""),update=update_edge_decal_user_library_directory,)
	...
	def draw(self,context):
		layout=self.layout.column_flow(columns=2,align=1);DH(self,context,layout);layout.template_icon(icon_value=I("1"),scale=12);GP(layout,self,"C");X(self,context,layout);layout=self.layout
		GP(layout,self, "user_library_directory")
		layout.label(text=f"{g('Active')}: {edge_decal_user_library_root(create=False)}",icon="FILE_FOLDER",)
		GL(layout,"Changing folders switches libraries; files are not moved automatically.",icon="INFO",)
...
def _edge_decal_legacy_user_asset_root():
	return bpy.utils.user_resource("CONFIG",path=os.path.join("lazy_edge_decals", "assets"),create=False,)
def _edge_decal_safe_user_asset_path(relative_path,create_root=False):
	...
	if (not create_root
		...
		and not str(getattr(edge_decal_addon_preferences(), "user_library_directory", "")
			or "").strip()):
		legacy_root=_edge_decal_legacy_user_asset_root()
		if legacy_root:
			legacy_root=os.path.realpath(legacy_root)
			legacy_candidate=os.path.realpath(os.path.join(legacy_root,str(relative_path or "")))
			try:
				if (os.path.commonpath((legacy_root,legacy_candidate))==legacy_root
...
def _edge_decal_external_image_source(image):
	...
	if not os.path.isfile(source_path):
		raise ValueError(f"图像 '{image.name_full}' 找不到文件: {source_path}")
	return source_path
def _edge_decal_copy_user_texture(source_path):
	digest=_edge_decal_file_sha256(source_path)
	texture_directory=_edge_decal_safe_user_asset_path("textures",create_root=True,)
	if not texture_directory:
		raise OSError("无法解析用户纹理目录")
...
def export_edge_decal_user_material_asset(material,preset_identifier):
	...
	for image in _edge_decal_material_images(material):
		...
		if not source_path:
			continue
		texture_paths[image.name_full]=_edge_decal_copy_user_texture(source_path)
	safe_identifier=_edge_decal_safe_asset_filename(preset_identifier)
	library_relative=f"materials/{safe_identifier}.blend"
	library_path=_edge_decal_safe_user_asset_path(library_relative,create_root=True,)
	if not library_path:
		raise OSError("无法解析用户材质资产库目录")
	os.makedirs(os.path.dirname(library_path),exist_ok=True)
	temporary_path=f"{library_path}.tmp.blend"
	try:
		if os.path.exists(temporary_path):
			os.remove(temporary_path)
		bpy.data.libraries.write(temporary_path,{material},fake_user=True,compress=True,)
		...
		if os.path.exists(temporary_path):
			try:
				...
				pass
	return {"schema_version": EDGEDECAL_USER_ASSET_SCHEMA_VERSION,"material_name": material.name_full, "library": library_relative, "textures": texture_paths,}
...
def ensure_user_edge_decal_material(material_name,asset_record):
	...
	try:
		with bpy.data.libraries.load(library_path,link=False,
			assets_only=False,) as (data_from,data_to):
...
def edge_decal_bundled_asset_manifest():
	...
	try:
		...
		EDGEDECAL_BUNDLED_ASSET_MANIFEST_MTIME=None
		return {}
	if (EDGEDECAL_BUNDLED_ASSET_MANIFEST_CACHE is not None
...
def edge_decal_bundled_material_library_path():
	manifest=edge_decal_bundled_asset_manifest()
	relative=manifest.get("material_library","materials/edge_decal_materials.blend",)
	return _edge_decal_safe_bundled_path(relative)
...
def ensure_bundled_edge_decal_material(material_name):
	...
	loaded_material["edge_decal_bundled_library"]=os.path.basename(library_path)
...
def ensure_bundled_edge_decal_uv_pins(scene,material):
	...
	if not isinstance(material_pins,list) or not material_pins:
		return 0
	existing_entries=[(index,pin) for index,pin in enumerate(scene.edge_decal_uv_pins) if getattr(pin, "material",None)==material]
	if (material.name== "M_EdgeDecal_Welds_01"
		and len(existing_entries)==1):
		legacy_index,legacy_pin=existing_entries[0]
		if (not str(getattr(legacy_pin, "pin_name", "")).strip()
