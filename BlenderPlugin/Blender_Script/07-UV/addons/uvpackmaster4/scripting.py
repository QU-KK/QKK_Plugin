...
from .grouping_scheme import UVPM4_GroupingSchemeAccessDescriptor
from .grouping_scheme_access import GroupingSchemeAccess
from .pack_context import PackContext
from .operator_misc import UVPM4_OT_ConfirmBase
...
class UVPM4_OT_ScriptSet(Operator):

    bl_idname = 'uvpackmaster4.script_set'
    bl_label = 'Set String'
    bl_description = ''
    ...
    def execute(self, context):
        context.uvpm4_script_entry.script = str(self.script)
        return {'FINISHED'}
    

class UVPM4_MT_ScriptSet(Menu):

    bl_label = "Set"
    bl_idname = "UVPM4_MT_ScriptSet"

    def draw(self, context):
        layout = self.layout
        layout.context_pointer_set('uvpm4_script_entry', context.uvpm4_script_entry)
        ...
        for text in _get_script_data().keys():
            op = layout.operator(UVPM4_OT_ScriptSet.bl_idname, text=text)
            op.script = text


@standalone_property_group
class UVPM4_ScriptEntry:
    ...
    g_scheme_access_desc : PointerProperty(type=UVPM4_GroupingSchemeAccessDescriptor)


@standalone_property_group
class UVPM4_ScriptCollection:

    scripts : CollectionProperty(type=UVPM4_ScriptEntry)
...
@standalone_property_group
class UVPM4_ScriptContainer:

    enabled : BoolProperty(name='Scripting Enabled', default=False)

    before_op : PointerProperty(type=UVPM4_ScriptCollection)
    after_op : PointerProperty(type=UVPM4_ScriptCollection)


class UVPM4_UL_ScriptCollection(UIList):

    bl_idname = 'UVPM4_UL_ScriptCollection'
    ...
    def draw_item(self, _context, layout, _data, item, icon, _active_data, _active_propname, _index):
        ...
        if not s_entry.script:
            ...
            UVPM4_OT_WarningPopup.draw_operator(row, text='Script name not set')
        elif s_entry.script not in _get_script_data().keys():
            from .help import UVPM4_OT_WarningPopup
            UVPM4_OT_WarningPopup.draw_operator(row, text='Script not found in the blend file')

        set_row = row.row(align=True)
        set_row.alignment = 'RIGHT'
        set_row.context_pointer_set('uvpm4_script_entry', s_entry)
        set_row.menu(UVPM4_MT_ScriptSet.bl_idname)


class UVPM4_ScriptCollectionWrapper:
    ...
    def __op_init(self, op):
        ...
        op.script_event_val = self.s_event.value()
    ...
    def draw(self, layout):
        ...
        row = main_col.row()
        row.template_list(UVPM4_UL_ScriptCollection.bl_idname, "", self.c, 'scripts',
                            self.c,
                            "active_script_entry_idx", rows=3)

        col = row.column(align=True)
        op_row = col.row(align=True)
        op = op_row.operator(UVPM4_OT_ScriptAddEntry.bl_idname, icon='ADD', text="")
        self.__op_init(op)
        op_row = col.row(align=True)
        op = op_row.operator(UVPM4_OT_ScriptRemoveEntry.bl_idname, icon='REMOVE', text="")
        self.__op_init(op)
        ...
        col.separator()
        op_row = col.row(align=True)
        op = op_row.operator(UVPM4_OT_ScriptMoveActiveEntry.bl_idname, icon='TRIA_UP', text="")
        self.__op_init(op)
        op.direction = 'UP'

        op_row = col.row(align=True)
        op = op_row.operator(UVPM4_OT_ScriptMoveActiveEntry.bl_idname, icon='TRIA_DOWN', text="")
        self.__op_init(op)
        op.direction = 'DOWN'
    ...
    def exec_script_entry(self, s_entry_idx):
        ...
        if s_entry.use_g_scheme:
            gs_access = GroupingSchemeAccess(self.container.context, desc=s_entry.g_scheme_access_desc)

            g_scheme = gs_access.active_g_scheme
            if not g_scheme:
...
class UVPM4_ScriptContainerWrapper:
    ...
    def get_collection(self, s_event):
        return UVPM4_ScriptCollectionWrapper(self, s_event, getattr(self.c, s_event.value()))
    ...
    def exec_scripts(self, s_event):
        ...
        collection = self.get_collection(s_event)
        collection.exec_scripts()

    def draw(self, layout):
        if not self.prefs.script_allow_execution:
            layout.operator(UVPM4_OT_ScriptAllowExecution.bl_idname)
            return
...
@standalone_property_group
class UVPM4_Scripting:

    packing : PointerProperty(type=UVPM4_ScriptContainer)

    @staticmethod
    def get_container(context, container_id):
        return UVPM4_ScriptContainerWrapper(context, container_id, getattr(get_main_props(context).scripting, container_id))
    
    @staticmethod
    def exec_scripts(context, s_event, container_id):
        container = UVPM4_Scripting.get_container(context, container_id)
        container.exec_scripts(s_event)



class UVPM4_PT_ScriptingBase():

    def get_main_property(self):
        return UVPM4_Scripting.get_container(self.context, self.SCRIPT_CONTAINER_ID).get_enabled_property()

    def draw_impl(self, context):
        s_container = UVPM4_Scripting.get_container(context, self.SCRIPT_CONTAINER_ID)
...
class ScriptAttrMixin:
    ...
    script_event_val : StringProperty(name='', default='')
    script_entry_idx : IntProperty(name='', default=-1)


class UVPM4_OT_ScriptBase(Operator):
    
    def execute(self, context):
        self.s_container = UVPM4_Scripting.get_container(context, self.script_container_id)
        self.s_collection = self.s_container.get_collection(ScriptEvent.item_by_value(self.script_event_val))
        return self.execute_impl(context)


class UVPM4_OT_ScriptAddEntry(UVPM4_OT_ScriptBase, ScriptAttrMixin):

    bl_idname = 'uvpackmaster4.script_add_entry'
    bl_label = 'Add Script'
...
class UVPM4_OT_ScriptRemoveEntry(UVPM4_OT_ScriptBase, ScriptAttrMixin):

    bl_idname = 'uvpackmaster4.script_remove_entry'
    bl_label = 'Add Script'
...
class UVPM4_OT_ScriptMoveActiveEntry(UVPM4_OT_ScriptBase, ScriptAttrMixin):

    bl_idname = "uvpackmaster4.script_move_active_entry"
    bl_label = "Move"
...
class UVPM4_OT_ScriptAllowExecution(UVPM4_OT_ConfirmBase):

    bl_idname = 'uvpackmaster4.script_allow_execution'
