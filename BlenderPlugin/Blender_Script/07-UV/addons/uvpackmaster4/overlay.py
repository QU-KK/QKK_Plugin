...
from .spipeline.engine.types import OperationStatus, UvpmLogType
...
class EngineOverlayManager(OverlayManager):
    ...
    @classmethod
    def format_string(cls, op_status, str):
        return '<t color="{}">{}</t>'.format(cls.OPSTATUS_TO_COLOR[op_status], str)

    def __init__(self, op, dev_array, p_context):
        super().__init__(op.context, engine_overlay_manager_draw_callback)
...
def engine_overlay_manager_draw_callback(self, context):
    try:
        ...
        if self.p_context and self.p_context.p_islands is not None:
