...
class SNA_PT_collision_configuration_35989(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        op = split_B43CD.operator('sna.my_generic_operator_58b2a', text='_autophy', icon_value=0, emboss=True, depress=False)
        op.sna_name = '_autophy'
        op = split_B43CD.operator('sna.my_generic_operator_58b2a', text='_autophy_s', icon_value=0, emboss=True, depress=False)
...
def sna_get_phymat_list_9D092():
    phymat_list = None
    phymat_list = [
    ['默认','default_phymat', [0.0, 0.49, 0.06, 1.0]], 
    ['金属','metal_phymat', [0.23, 0.23, 0.23, 1.0]], 
    ['水泥','build_phymat', [0.1, 0.1, 0.1, 1.0]], 
    ['岩石','stone_phymat', [0.26, 0.23, 0.09, 1.0]], 
    ['木头','wood_phymat', [0.24, 0.12, 0.04, 1.0]], 
    ['布料','cloth_phymat', [0.7, 0.46, 0.08, 1.0]], 
    ['玻璃','glass_phymat', [0.24, 0.34, 0.53, 1.0]],
    ['皮革','leather_phymat', [0.11, 0.01, 0.23, 1.0]], 
    ['橡胶','rubber_phymat', [0.8, 0.22, 0.45, 1.0]], 
    ['泥土','soil_phymat', [0.12, 0.07, 0.0, 1.0]], 
    ['冰    ','ice_phymat', [0.02, 0.05, 0.6, 1.0]],
    ['水    ','water_phymat', [0.03, 0.3, 0.6, 1.0]], 
    ['无弹孔','nonreactivity_phymat', [0.06, 0.17, 0.2, 1.0]]
    ]
    #子弹碰撞颜色设置
...
class SNA_OT_My_Generic_Operator_58B2A(bpy.types.Operator):
    ...
    def execute(self, context):
        for i_CAF60 in range(len(bpy.context.view_layer.objects.selected)):
            bpy.context.view_layer.objects.selected[i_CAF60].name = str(bpy.context.view_layer.objects.selected[i_CAF60].name).replace('_autophy_s', '').replace('_autophy', '') + self.sna_name
        return {"FINISHED"}
