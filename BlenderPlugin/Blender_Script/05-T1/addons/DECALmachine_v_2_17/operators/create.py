...
from .. utils.assets import get_ambiguous_libs, get_invalid_libs, get_corrupt_libs, get_skipped_update_path, declutter_assetspath, reload_all_assets
...
class UpdateDecalLibrary(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if not self.batch:
            DM.get_legacy_materials(force=True)
...
class BatchUpdate(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        ...
        assets_dict = DM.get_assets_dict(force=True)
...
class UpdateBlendFile(bpy.types.Operator):
    ...
    @classmethod
    def poll(cls, context):
        legacy_dict = DM.get_legacy_materials(force=False, debug=False)
    ...
    def execute(self, context):
        ...
        legacy_dict = DM.get_legacy_materials(force=True, debug=False)
