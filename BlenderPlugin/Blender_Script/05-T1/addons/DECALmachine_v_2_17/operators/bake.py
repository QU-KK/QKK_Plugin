...
class Bake(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if targets:
            ...
            print(f"\nInfo: Starting to bake decals to {', '.join([target.name for target in targets])}, at {width}x{height} with {samples} samples and {supersample}x Anti Aliasing using the {device} --------------------")
            ...
            ray_distance = dm.export_bake_ray_distance
            ...
            if baketypes:
                ...
                for target in targets:
                    ...
                    target_mask = bake_target_mask(bakescene, bakebasepath, target.name, 'mask', bakeimg, bakemat, margin=0, extrusion_distance=extrusion_distance, ray_distance=ray_distance)
                    if bake_masks:
                    ...
                    if combine_bakes:
                        combine_mask = bake_target_mask(bakescene, bakebasepath, target.name, 'combine', bakeimg, bakemat, margin=margin, extrusion_distance=extrusion_distance, ray_distance=ray_distance)
                        ...
                        if decals:
                            if baketype in ['NORMAL', 'AO_CURV_HEIGHT', 'SUBSET', 'EMISSION_NORMAL']:
                                if not normal_decals_mask:
                                    normal_margin_0_mask = bake_decals_margin_mask(bakescene, bakebasepath, target.name, 'normal', bakeimg, decals, margin=0, extrusion_distance=extrusion_distance, ray_distance=ray_distance)
                                    normal_margin_3_mask = bake_decals_margin_mask(bakescene, bakebasepath, target.name, 'normal', bakeimg, decals, margin=margin, extrusion_distance=extrusion_distance, ray_distance=ray_distance)
                            ...
                            if baketype in ['COLOR', 'EMISSION_COLOR']:
                                if not color_decals_mask:
                                    color_margin_0_mask = bake_decals_margin_mask(bakescene, bakebasepath, target.name, 'color', bakeimg, decals, margin=0, extrusion_distance=extrusion_distance, ray_distance=ray_distance)
                                    color_margin_3_mask = bake_decals_margin_mask(bakescene, bakebasepath, target.name, 'color', bakeimg, decals, margin=margin, extrusion_distance=extrusion_distance, ray_distance=ray_distance)
                            ...
                            bake_map = bake(bakescene, bakebasepath, decals, target.name, baketype, bakeimg, margin=margin, extrusion_distance=extrusion_distance, ray_distance=ray_distance)
