...
def get_ambiguous_libs():
    ...
    if DM.assets:
        return sorted(set(DM.assets['AMBIGUOUS']) - set(DM.assets['CORRUPT']))
...
def get_invalid_libs():
    ...
    if DM.assets:
        return sorted(set(DM.assets['INVALID']) - set(DM.assets['OBSOLETE']))
...
def get_corrupt_libs():
    ...
    if DM.assets:
        return sorted(set(DM.assets['CORRUPT']) - set(DM.assets['OBSOLETE']) - set(DM.assets['INVALID']))
    return []
...
def declutter_assetspath(force=True, debug=False):
    ...
    clutter = DM.get_assets_dict(force=force)['CLUTTER']
