...
from . collection import sort_into_collections, unlink_object_from_all_collections
...
def create_panel_uvs(bm, geo_sequences, panel, width=None):
    ...
    panel.data.uv_layers[0].name = 'UVMap'
...
def finish_panel_decal(depsgraph, context, panel, target, cutter, smooth=True):
    ...
    attribute_names = [attr.name for attr in panel.data.attributes]

    for name in attribute_names:
        if name.startswith('.') or name in ['position', 'sharp_face', 'material_index', 'UVMap']:
            continue

        panel.data.attributes.remove(panel.data.attributes[name])
    ...
    if cutter:
        ...
        unlink_object_from_all_collections(cutter)
