...
bl_info = {
    "name" : "Click to put",
    "author" : "QKK", 
    "description" : "Click to put",
    "blender" : (4, 2, 0),
    "version" : (1, 1, 0),
    "location" : "",
    "warning" : "",
    "doc_url": "https://superhivemarket.com/creators/qkk", 
...
class SNA_OT_Place_Put_Dfe71(bpy.types.Operator):
    bl_idname = "sna.place_put_dfe71"
    bl_label = "Place_put"
    bl_description = "Select an object"
    bl_options = {"REGISTER", "UNDO"}
    sna_copy: bpy.props.BoolProperty(name='copy', description='', options={'HIDDEN'}, default=False)
    cursor = "PAINT_CROSS"
    _handle = None
    _event = {}
        ...
        if event.keys():
            ...
            try:
                ...
                if font_id == -1:
                    ...
                    blf.enable(font_id, blf.WORD_WRAP)
                    blf.draw(font_id, ('Click to position' if (bpy.context.preferences.view.language != 'zh_HANS') else '点击放置'))
    ...
    def execute(self, context):
        ...
        exec("bpy.context.object.display_type = 'TEXTURED'")
        exec('bpy.context.space_data.show_gizmo_tool = True')
        exec("bpy.context.scene.transform_orientation_slots[0].type = 'LOCAL'")
    ...
    def modal(self, context, event):
        ...
        try:
            if (event.type == 'LEFTMOUSE' and event.value == 'PRESS' and event.alt == False and event.shift == False and event.ctrl == False):
                ...
                bpy.context.scene.sna_controls_place_copy = False
    ...
    def invoke(self, context, event):
        ...
        if _DFE71_running:
            ...
            if self.sna_copy:
                ...
                exec("bpy.context.object.display_type = 'WIRE'")
                exec('bpy.context.space_data.show_gizmo_tool = False')
                exec('bpy.context.space_data.overlay.show_cursor = False')
                bpy.context.scene.sna_controls_place_copy = True
            else:
                exec("bpy.context.object.display_type = 'WIRE'")
                exec('bpy.context.space_data.show_gizmo_tool = False')
                exec('bpy.context.space_data.overlay.show_cursor = False')
...
def sna_add_to_view3d_pt_tools_active_8902E(self, context):
    if not (False):
        layout = self.layout
        box_06D00 = layout.box()
        box_06D00.alert = False
        box_06D00.enabled = ('OBJECT'==bpy.context.mode and (len(bpy.context.view_layer.objects.selected) == 1))
        ...
        op = box_06D00.operator('sna.place_put_dfe71', text='', icon_value=string_to_icon('STYLUS_PRESSURE'), emboss=bpy.context.scene.sna_controls_place, depress=bpy.context.scene.sna_controls_place)
        op.sna_copy = False
        op = box_06D00.operator('sna.place_put_dfe71', text='', icon_value=string_to_icon('ADD'), emboss=bpy.context.scene.sna_controls_place_copy, depress=bpy.context.scene.sna_controls_place_copy)
        op.sna_copy = True
...
def register():
    ...
    bpy.types.Scene.sna_controls_place_copy = bpy.props.BoolProperty(name='controls_place_copy', description='', default=False)
    bpy.utils.register_class(SNA_OT_Place_Put_Dfe71)
    bpy.types.VIEW3D_PT_tools_active.prepend(sna_add_to_view3d_pt_tools_active_8902E)
...
def unregister():
    ...
    addon_keymaps.clear()
    del bpy.types.Scene.sna_controls_place_copy
    del bpy.types.Scene.sna_controls_place
    bpy.utils.unregister_class(SNA_OT_Place_Put_Dfe71)
    bpy.types.VIEW3D_PT_tools_active.remove(sna_add_to_view3d_pt_tools_active_8902E)
