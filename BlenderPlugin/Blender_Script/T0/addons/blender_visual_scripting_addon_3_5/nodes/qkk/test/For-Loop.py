...
class SN_ForExecuteNode(SN_ScriptingBaseNode, bpy.types.Node):
    ...
    bl_idname = "SN_For_LoopExecuteNode"
    bl_label = "For-Loop (Execute)"
    ...
    def evaluate(self, context):
        if self.inputs[1].is_linked:
            collection_var = self.inputs[1].python_value
            index_var = f"i_{self.static_uid}"
            item_var = f"item_{self.static_uid}"
            
            order = f"reversed({collection_var})" if self.reverse else collection_var
            
            self.outputs["Item"].python_value = item_var
            
            self.code = f"""
                        for {index_var}, {item_var} in enumerate({order}):
