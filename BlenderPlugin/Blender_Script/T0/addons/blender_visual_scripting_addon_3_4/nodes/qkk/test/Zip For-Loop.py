...
class SN_ForExecuteNode(SN_ScriptingBaseNode, bpy.types.Node):
    ...
    bl_idname = "SN_Zip_For_LoopExecuteNode"
    bl_label = "Zip For-Loop (Execute)"
    ...
    def evaluate(self, context):
        if self.inputs[1].is_linked:
            collection = self.inputs[1].python_value
            self.outputs["Index"].python_value = f"i_{self.static_uid}"
            self.outputs["Item"].python_value = f"item_{self.static_uid}"
            
            collection_access = f"reversed({collection})" if self.reverse else collection
            
            self.code = f"""
                        for i_{self.static_uid}, item_{self.static_uid} in zip(range(len({collection})), {collection_access}):
