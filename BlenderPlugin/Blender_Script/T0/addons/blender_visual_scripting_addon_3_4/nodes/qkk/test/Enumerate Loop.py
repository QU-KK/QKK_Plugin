...
class SN_ForExecuteNode(SN_ScriptingBaseNode, bpy.types.Node):
    ...
    bl_idname = "SN_Enumerate_Loop_ExecuteNode"
    bl_label = "Enumerate Loop (Execute)"
    ...
    def evaluate(self, context):
        if self.inputs[1].is_linked:
            ...
            reverse_flag = "[::-1]" if self.reverse else ""
            self.outputs["Index"].python_value = f"i_{self.static_uid}"
            self.outputs["Item"].python_value = f"item_{self.static_uid}"
            
            self.code = f'''
                        for i_{self.static_uid}, item_{self.static_uid} in enumerate({collection}{reverse_flag}):
