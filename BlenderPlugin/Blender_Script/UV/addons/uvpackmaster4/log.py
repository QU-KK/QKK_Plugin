...
from .spipeline.engine.types import UvpmLogFlags, UvpmLogType, OperationStatus, RETCODE_METADATA
...
class LogManager:
    ...
    OPSTATUS_TO_LOGTYPE = reverse_dict(LOGTYPE_TO_OPSTATUS)

    INFO_MAX_LOG_COUNT = 10

    def __init__(self, post_log_op=None, info_max_log_count=INFO_MAX_LOG_COUNT, op_status_from_last_logged=True):
        self.post_log_op = post_log_op
        self.engine_retcode = None
        self.op_status_from_last_logged = op_status_from_last_logged
        self.op_status = None

        self.log_lists = {
            UvpmLogType.STATUS :     LogList(max_log_count=0),
            UvpmLogType.INFO :       LogList(max_log_count=info_max_log_count),
    ...
    def log_engine_retcode(self, engine_retcode):
        ...
        self.op_status = RETCODE_METADATA[self.engine_retcode].op_status
    ...
    def operation_status(self):
        for op_status in self.OPSTATUS_PRIORITY:
            if self.op_status is not None and self.op_status == op_status:
                return op_status

            if self.op_status_from_last_logged:
