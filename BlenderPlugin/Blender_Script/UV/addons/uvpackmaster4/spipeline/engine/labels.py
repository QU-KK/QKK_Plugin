...
class Labels:
    ...
    PIXEL_BORDER_MARGIN_ENABLE_NAME='Enable Border Margin'
    PIXEL_BORDER_MARGIN_ENABLE_DESC='Border Margin allows to explicitly define the distance in pixels between UV islands and the UV target box border. When Border Margin is disabled, the distance is determined by the island margin value'

    PIXEL_BORDER_MARGIN_NAME='Border Margin (px)'
    PIXEL_BORDER_MARGIN_DESC="Distance in pixels between UV islands and the UV target box border"
    ...
    PIXEL_MARGIN_TEX_SIZE_DESC="Texture size in pixels used for pixel margin calculation as well as in texel density operations. If the 'Non-Square Packing' option is enabled, then this property is ignored and the dimensions of the active texture are used for calculations"
    ...
    @classmethod
    def TEXEL_DENSITY_VALUE_NAME(cls, add_prefix=False):
        from ...app_iface import AppInterface
        return '{}Value ({})'.format('TD ' if add_prefix else '', AppInterface.TEXEL_DENSITY_UNIT)
    
    TEXEL_DENSITY_SET_BEFORE_PACK_NAME='Set TD Before Packing'
    
    TDENSITY_TO_SET_NAME="TD To Set"
    TDENSITY_PACKING_NAME="TD (Packing)"

    TEXEL_DENSITY_SET_BEFORE_PACK_DESC = "Enable automatic texel density setting for the input islands before packing. Press the Help button on the right side of the header for a detailed description of the functionality"
    ...
    NON_SQUARE_PACKING_NAME='Non-Square Packing'
    NON_SQUARE_PACKING_DESC='Take into consideration the ratio of the active texture dimensions during packing which enables packing for a non-square texture'
    ...
    PIXEL_PERFECT_ALIGN_TARGET_NAME = 'Align Target'
    PIXEL_PERFECT_ALIGN_TARGET_DESC = 'Defines the pixel alignment target'
    ...
    GROUP_METHOD_MESH_DESC = 'Islands being a part of adjacent geometry will belong to the same group'
    GROUP_METHOD_OBJECT_DESC = 'Islands being a part of the same object will belong to the same group'
    ...
    GROUP_METHOD_VERTEX_COLOR_DESC='Islands with the same vertex color assigned will belong to the same group (the active vertex color is always used)'
    GROUP_METHOD_COLLECTION_DESC='Islands being a part of objects from the same outliner collection will belong to the same group'
    ...
    GROUP_LAYOUT_MODE_TEXTURE_ATLAS_DESC='Groups will be packed into subtiles. The number of subtiles in a single tile is determined by the atlas size properties. When a given tile is out of subtiles, the packer will use subtiles of the next tile and so on'
    ...
    DYNAMIC_TILES_DESC='When UVs are packed with Dynamic Tiles on, the packer will automatically determine the number of target tiles to use - it will use as many tiles as possible to pack the UVs efficiently'

    DYNAMIC_TILES_GROUP_NAME='Dynamic Tiles'
    DYNAMIC_TILES_GROUP_DESC='Enable the Dynamic Tiles functionality for the given group.\n\n{}'.format(DYNAMIC_TILES_DESC)

    DYNAMIC_TILES_ALL_NAME='Dynamic Tiles (All)'
    DYNAMIC_TILES_ALL_DESC='Enable the Dynamic Tiles functionality for all groups.\n\n{}'.format(DYNAMIC_TILES_DESC)
    ...
    GROUPS_ENABLE_DESC="Enable the given group functionality"
    ...
    USE_EDITOR_GRID_NAME="Use Editor Grid"
    USE_EDITOR_GRID_DESC="If enabled, the tile grid shape is determined by the UV editor settings (N-panel -> View tab). Otherwise, the shape is configured using the properties below"
    ...
    START_TILE_X_NAME='Start Tile (X)'
    START_TILE_Y_NAME='Start Tile (Y)'
    ...
    LAST_GROUP_COMPLEMENTARY_DESC="Automatically pack the last group on the top of all other groups"
    ...
    VERT_MULTI_PANEL_TOGGLES_NAME='Vertical Multi Panel Toggles'
    VERT_MULTI_PANEL_TOGGLES_DESC='Draw multi panel toggles vertically in the UI'
    ...
    HIGHPREC_TOPO_ANALYSIS_NAME='High-Prec Topology Analysis'
    HIGHPREC_TOPO_ANALYSIS_DESC='Enabling this option may solve packer hangs during topology analysis of a very dense UV map. The option should only be enabled when encountering such a situation, not during the standard packer usage'
    ...
    OTHER_ISLANDS_DESC = "'Other' islands are: 1. unselected islands 2. pinned islands if the '{}' option is enabled (in the Preferences multi panel)".format(PINNED_UVS_AS_OTHERS_NAME)
