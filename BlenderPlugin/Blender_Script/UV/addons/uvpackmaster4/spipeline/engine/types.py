...
TopoAnalysisConfig = namedtuple('TopoAnalysisConfig', ['uv_coord_prec', 'uv_connect_limit'])
...
class EnumValue:
    ...
    def to_blend_items(cls, enum_values):
        return [enum_val.to_blend_item for enum_val in enum_values]

    def __init__(self, value, name, desc='', req_feature=''):
        self.v = value
        self.name = name
        self.desc = desc
    ...
    def to_blend_item(self, supported=True, name_mod=None):
        ...
        return (self.v, name, self.desc, icon, int(self.v))
...
class EnumBase:
    ...
    @classmethod
    def item_by_value(cls, value):
        return cls.ITEM_BY_VALUE[str(value)]
...
def enum_decorator(new_cls: T) -> T:
    items = tuple(attr for attr in new_cls.__dict__.values() if isinstance(attr, EnumValue))
    item_by_value = {item.value(): item for item in items}
    
    cls = type(new_cls.__name__, (EnumBase,) + new_cls.__bases__, dict(new_cls.__dict__, ITEMS=items, ITEM_BY_VALUE=item_by_value))
    return cls
...
class RetCodeMetadata:
    ...
    def __init__(self, op_status, desc=None):
...
RETCODE_METADATA = {
    ...
    UvpmRetCode.NO_SPACE : RetCodeMetadata(
        ...
        desc='No space to pack all islands'
    ),
    ...
    UvpmRetCode.INVALID_ISLANDS : RetCodeMetadata(
        op_status=OperationStatus.ERROR,
        desc='Invalid islands encountered'
    ),
    UvpmRetCode.NO_SIUTABLE_DEVICE : RetCodeMetadata(
        op_status=OperationStatus.ERROR,
        desc='No suitable device to run the operation'
    ),
    ...
    UvpmRetCode.INVALID_INPUT : RetCodeMetadata(
        op_status=OperationStatus.ERROR,
        desc='Invalid operation input'
    ),
    UvpmRetCode.WARNING : RetCodeMetadata(
        op_status=OperationStatus.WARNING,
        desc='Operation completed with warning'
    )
}
...
@enum_decorator
class UvpmPixelPerfectAlignTarget:

    CORNER = EnumValue('0', 'Corner')
    CENTER = EnumValue('1', 'Center')
...
class PackOpType:
    ...
    def send_unselected_islands(cls, op_type):
        return cls.static_islands_enable(op_type)
    

@enum_decorator
class TileTargetMode:

    TILE_GRID = EnumValue('0', 'Tile Grid')
    TILE_RANGE = EnumValue('1', 'Tile Range')
...
class GroupingMethod:
    ...
    VERTEX_COLOR = EnumValue('6', 'Vertex Color', Labels.GROUP_METHOD_VERTEX_COLOR_DESC)
    COLLECTION = EnumValue('7', 'Collection', Labels.GROUP_METHOD_COLLECTION_DESC)
    ...
    def to_blend_items(cls):
        return (cls.MATERIAL.to_blend_item(),
                ...
                cls.VERTEX_COLOR.to_blend_item(),
                cls.COLLECTION.to_blend_item(),
                cls.MANUAL.to_blend_item())

    @classmethod
    def auto_grouping_enabled(cls, g_method):
        return g_method != cls.MANUAL
...
class GroupLayoutMode:
    ...
    TEXTURE_ATLAS = EnumValue('5', 'Texture Atlas', Labels.GROUP_LAYOUT_MODE_TEXTURE_ATLAS_DESC)
    ...
    @classmethod
    def is_automatic(cls, mode):
        return mode in cls.automatic_modes()

    @classmethod
    def supports_tiles_in_row(cls, mode):
        return mode in (cls.AUTOMATIC, cls.MANUAL, cls.TEXTURE_ATLAS)

    @classmethod
    def supports_tile_count(cls, mode):
        return cls.is_automatic(mode) and (not cls.supports_tile_count_xy(mode))
    
    @classmethod
    def supports_tile_count_xy(cls, mode):
        return mode == cls.TILE_GRID
    
    @classmethod
    def supports_dynamic_tiles(cls, mode):
        return mode not in {cls.TILE_GRID, cls.TEXTURE_ATLAS}


@enum_decorator
class ChildPanelMode:

    STANDARD = EnumValue('0', 'Standard')
    ENABLE_MENU = EnumValue('1', 'Enable Menu')
