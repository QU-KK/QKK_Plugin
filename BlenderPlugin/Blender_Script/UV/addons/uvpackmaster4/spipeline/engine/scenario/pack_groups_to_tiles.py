...
from ..pack_utils.pack_manager import TaskData
...
class Scenario(GSchemeDistinctStagesPackScenario):
    ...
    def island_overlaps_op_target(self, island):
        return self.g_scheme.island_overlaps_op_target(island)
    ...
    def init_tasks(self):
        ...
        for group in self.g_scheme.groups:
            ...
            if group.dt_config is not None:
                self.pack_manager.add_task(TaskData(self.pack_params, [group.to_stage()], dt_config=group.dt_config))
                continue
            ...
            if td_cluster is None:
                ...
                if td_cluster.scale_mode != group.scale_mode:
                    uc.packer.send_log(uc.LogType.ERROR, "Inconsistent 'Scale Mode' setting for groups packed with uniform texel density")
                    uc.packer.send_log(uc.LogType.ERROR, "Groups with inconsistent setting: {}, {}".format(td_cluster.group_name, group.name))
                    raise uc.InputError()
        ...
        for g_cluster in group_clusters:
            ...
            for group in g_cluster.groups:
                if group.scale_mode != uc.ScaleMode.FIXED_SCALE:
                    continue
        ...
        for g_cluster in group_clusters:
            ...
            self.pack_manager.add_task(TaskData(self.pack_params, stages))
