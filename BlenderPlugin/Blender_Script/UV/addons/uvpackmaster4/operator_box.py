...
from .spipeline.engine.types import OpFinishedException, UvpmRetCode
from .spipeline.engine.labels import Labels
from .grouping_scheme_access import GSAccessDescIdAttrMixin, GroupingSchemeAccess
from .box import UVPM4_Box
from .box_utils import BoxRenderer, BoxRenderAccess, CustomTargetBoxAccess, TileTargetBoxAccess, disable_box_rendering
from .operator import UVPM4_OT_Engine, UVPM4_OT_GenericHandler, ModeIdAttrMixin
...
class RenderBoxesOverlayManager(OverlayManager):
    ...
    IDLE_HEADER_MSG_EDIT = '[BOX EDIT]'
    IDLE_HEADER_MSG_DISPLAY = '[BOX DISPLAY]'
    ...
    def idle_overlay_msg_array(self):
        msg_array = []
        msg_array.append(self.IDLE_HEADER_MSG_EDIT if self.op.edit_enable() else self.IDLE_HEADER_MSG_DISPLAY)

        if self.op.edit_enable():
...
def render_boxes_overlay_manager_draw_callback(self, context):
    try:
        ...
        print_backtrace_if_debug(ex, debug_lvl=2)


class UVPM4_OT_RenderBoxesGeneric(UVPM4_OT_GenericHandler):
    ...
    def handle_draw(self, event, view_coords):
        ...
        if self.draw_begin is not None and self.draw_end is not None:
            ...
            self.active_box_info.box = UVPM4_Box.SA(*draw_coords)
    ...
    def handle_box_selection(self, event, view_coords):
        ...
        if self.non_trivial_trans:
            self.non_trivial_trans = False
            return

        if self.box_info_array is None:
            return

        if not (event.type == 'LEFTMOUSE' and event.value == 'RELEASE'):
            return
        ...
        self.box_access.select_box_impl(box_to_select)
    ...
    def handle_resize(self, event, view_coords):
        ...
        if self.render_mode == RenderBoxMode.RESIZE:
            ...
            if event.type == 'LEFTMOUSE' and event.value == 'RELEASE':
                ...
                self.active_box_info.box = UVPM4_Box.SA(*new_coords)
                self.prefs.boxes_dirty = True
            return
...
class UVPM4_OT_FinishBoxRendering(Operator):

    bl_idname = 'uvpackmaster4.finish_box_rendering'
...
class GroupingSchemeRenderAccess(BoxRenderAccess):
    ...
    def render_group_impl(self, group):
        return True
    ...
    def box_info_array_impl(self):
        ...
        for group_idx, group in enumerate(self.g_scheme.groups):
            if not self.render_group_impl(group):
                continue
...
class ActiveGroupingSchemeRenderAccess(GroupingSchemeRenderAccess):

    ACTIVE_Z_COORD = 10.0

    def __init__(self, gs_access):
        self.gs_access = gs_access
        super().__init__(self.gs_access.active_g_scheme)

    def active_box_impl(self):
        return self.gs_access.active_target_box


class ActiveModeGroupingSchemeRenderAccess(ActiveGroupingSchemeRenderAccess):

    def __init__(self, active_mode, ui_drawing=False):
        self.active_mode = active_mode
        gs_access = self.active_mode.grouping_config.get_scheme_access(ui_drawing=ui_drawing)
        super().__init__(gs_access)

    def render_group_impl(self, group):
        return self.active_mode.target_boxes_editable(group)

        

class UVPM4_OT_RenderGroupingSchemeBoxes(UVPM4_OT_RenderBoxesGeneric, GSAccessDescIdAttrMixin, ModeIdAttrMixin):

    bl_idname = 'uvpackmaster4.render_grouping_scheme_boxes'
    ...
    def get_box_access(self, context):
        box_access = ActiveModeGroupingSchemeRenderAccess(self.get_mode())
        return box_access if box_access.g_scheme is not None else None


class UVPM4_OT_ModifyTargetBox(Operator):
...
class UVPM4_OT_SetTargetBoxToTile(UVPM4_OT_ModifyTargetBox):
...
class GroupingSchemeGetterMixin:

    def get_box_access(self, context):
        return ActiveGroupingSchemeRenderAccess(GroupingSchemeAccess(context, **GroupingSchemeAccess.get_desc_kwargs(self, context)))


class UVPM4_OT_SetGroupingSchemeBoxToTile(GroupingSchemeGetterMixin, UVPM4_OT_SetTargetBoxToTile, GSAccessDescIdAttrMixin):

    bl_idname = 'uvpackmaster4.set_grouping_scheme_box_to_tile'


class CustomTargetBoxGetterMixin:

    def get_box_access(self, context):
        return CustomTargetBoxAccess(context)
    

class UVPM4_OT_SetCustomTargetBoxToTile(UVPM4_OT_SetTargetBoxToTile, CustomTargetBoxGetterMixin):

    bl_idname = 'uvpackmaster4.set_main_target_box_to_tile'


class UVPM4_OT_MoveTargetBox(UVPM4_OT_ModifyTargetBox):
...
class UVPM4_OT_ModifyTargetBoxEngine(UVPM4_OT_Engine):

    SCENARIO_ID = 'hidden_modify_box_with_islands'
    move_islands = False
    ...
    def pre_main(self):
        self.box_access = self.get_box_access(self.context)

        self.fully_inside = self.main_props.fully_inside
        self.active_box = self.box_access.active_box_impl()
...
class UVPM4_OT_MoveTargetBoxEngine(UVPM4_OT_ModifyTargetBoxEngine):
...
class UVPM4_OT_MoveGroupingSchemeBox(GroupingSchemeGetterMixin, UVPM4_OT_MoveTargetBoxEngine, MoveTargetBoxProperties, GSAccessDescIdAttrMixin):

    bl_idname = 'uvpackmaster4.move_grouping_scheme_box'


class UVPM4_OT_MoveCustomTargetBox(UVPM4_OT_MoveTargetBoxEngine, MoveTargetBoxProperties, CustomTargetBoxGetterMixin):

    bl_idname = 'uvpackmaster4.move_main_target_box'


class UVPM4_OT_RenderCustomTargetBox(UVPM4_OT_RenderBoxesGeneric, CustomTargetBoxGetterMixin):

    bl_idname = 'uvpackmaster4.render_main_target_box'
...
class UVPM4_OT_RenderTargetTiles(UVPM4_OT_RenderBoxesGeneric):

    bl_idname = 'uvpackmaster4.render_target_tiles'
    bl_label = 'Display Tiles'
    bl_description = ''
    ...
    def get_box_access(self, context):
        return TileTargetBoxAccess(context)
    

class UVPM4_OT_SelectIslandsInBox(UVPM4_OT_Engine):

    bl_label = 'Select Islands In Box'
    bl_description = 'Select/deselect islands in the active box'
    SCENARIO_ID = 'hidden_select_islands_in_box'
...
class UVPM4_OT_SelectIslandsInGroupingSchemeBox(GroupingSchemeGetterMixin, UVPM4_OT_SelectIslandsInBox, SelectIslandsInBoxProperties, GSAccessDescIdAttrMixin):

    bl_idname = 'uvpackmaster4.select_islands_in_grouping_scheme_box'


class UVPM4_OT_SelectIslandsInCustomTargetBox(UVPM4_OT_SelectIslandsInBox, SelectIslandsInBoxProperties, CustomTargetBoxGetterMixin):

    bl_idname = 'uvpackmaster4.select_islands_in_custom_target_box'
