...
class BakeWrangler_Socket_HighPolyMesh(NodeSocket, BakeWrangler_Tree_Socket):
    ...
    value: bpy.props.PointerProperty(name="从对象烘焙", description="进行“选定到活动”类型烘焙时,几何体将成为选择的一部件", type=bpy.types.ID, poll=value_prop_filter, update=value_prop_update)
    collection: bpy.props.BoolProperty(name="集合", description="启用后,将选择整个集合,而不是单个对象", update=collection_prop_update, default=False)
...
class BakeWrangler_Input_Mesh(Node, BakeWrangler_Tree_Node):
    ...
    def validate(self, check_materials=False):
        ...
        if not self.mesh_object:
            valid[0] = False
            valid.append(_print("未选择确认的栅格对象", node=self, ret=True))
        ...
        if self.multi_res and self.mesh_object:
            ...
            if not has_multi_mod:
                valid[0] = False
                valid.append(_print("启用了多级精度,但选定栅格对象上没有多级精度数据", node=self, ret=True))
        # Check cage if enabled
        if self.cage:
            if not self.cage_obj:
                valid[0] = False
                valid.append(_print("帧已启用,但未选择帧对象", node=self, ret=True))
            if self.mesh_object and self.cage_obj and len(self.mesh_object.data.polygons) != len(self.cage_obj.data.polygons):
                    valid[0] = False
                    valid.append(_print("帧对象面数量与栅格对象不匹配", node=self, ret=True))
            if self.mesh_object and len(self.get_objects()) < 2:
                valid[0] = False
                valid.append(_print("帧已启用,但未选择高多边形对象", node=self, ret=True))
        # Check valid UV Map
        if self.mesh_object and len(self.mesh_object.data.uv_layers) < 1:
            valid[0] = False
            valid.append(_print("栅格对象没有UV贴图)", node=self, ret=True))
        if self.mesh_object and len(self.mesh_object.data.uv_layers) > 1 and self.uv_map and self.uv_map not in self.mesh_object.data.uv_layers:
            valid[0] = False
            valid.append(_print("对象上不存在选定的UV贴图(它可能已被删除或重命名)", node=self, ret=True))
        # Validated?
        if not valid[0]:
            return valid
        ...
        if check_materials:
            ...
            for mat in mats:
                ...
                if not nodes or len(nodes) < 2:
                    valid.append(_print("'%s' 不基于节点或节点太少" % (mat.name), node=self, ret=True))
                    continue
                ...
                if not passed:
                    valid.append(_print("'%s' 输出似乎不是Principled和混合着色器的确认组合。烘焙值将不适用于此材质." % (mat.name), node=self, ret=True))
    ...
    mesh_object: bpy.props.PointerProperty(name="Bake Target", description="烘焙过程中将成为活动对象的栅格", type=bpy.types.Object, poll=mesh_object_filter)
    ...
    mask_margin: bpy.props.IntProperty(name="Mask Margin", description="为遮罩烘焙添加额外的填充。如果边缘细节被截断,则", default=0, min=0, subtype='PIXEL')
    multi_res: bpy.props.BoolProperty(name="Multires", description="直接从多区域对象烘焙。这将禁用或忽略其他烘焙设置。\n只能烘焙法向和置换")
    multi_res_pass: bpy.props.EnumProperty(name="Pass", description="选择要烘焙到图像中的着色信息。\nMultires过程将覆盖任何连接的烘焙过程", items=multi_res_passes, default='NORMALS')
    cage: bpy.props.BoolProperty(name="外壳", description="将光线从帧投射到活动对象。帧必须具有相同数量的面")
    cage_obj: bpy.props.PointerProperty(name="Cage Object", description="对象用作帧,而不是从活动对象计算帧", type=bpy.types.Object, poll=mesh_object_filter)
    uv_map: bpy.props.StringProperty(name="UV Map", description="若对象具有多个层,则拾取要烘焙的贴图。保留为空以活动层")
    ...
    def draw_buttons(self, context, layout):
        layout.label(text="栅格对象：")
        layout.prop_search(self, "mesh_object", context.scene, "objects", text="")
        layout.prop(self, "margin", text="边缘")
        layout.prop(self, "mask_margin", text="填充")
        if self.mesh_object and len(self.mesh_object.data.uv_layers) > 1:
        ...
        if not self.multi_res:
            if not self.cage:
                layout.prop(self, "cage", text="外壳")
            else:
                layout.prop(self, "cage", text="外壳")
                layout.prop_search(self, "cage_obj", context.scene, "objects", text="")
            layout.label(text="烘焙来源：")
            layout.prop(self, "ray_dist", text="射线距离")
        else:
