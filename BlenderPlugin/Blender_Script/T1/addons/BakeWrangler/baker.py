...
def process_tree(tree_name, node_name, socket):
    ...
    bpy.types.Object.bw_copy = bpy.props.PointerProperty(name="对象复制", description="应用修改器进行复制", type=bpy.types.Object)
    bpy.types.Object.bw_copy_frame = bpy.props.IntProperty(name="对象复制帧", description="创建时的帧数", default=current_frame)
    bpy.types.Object.bw_strip = bpy.props.PointerProperty(name="对象已剥离", description="剥离修改器进行复制", type=bpy.types.Object)
    bpy.types.Object.bw_strip_frame = bpy.props.IntProperty(name="对象条斑帧", description="创建时的帧数", default=current_frame)
    bpy.types.Object.bw_vcols = bpy.props.PointerProperty(name="对象VCol复制", description="完整复制修改器", type=bpy.types.Object)
    ...
    if debug: _print("> 调试输出已启用", tag=True)
    modify_recipe(tree)
    ...
    if node.bl_idname in ['BakeWrangler_Output_Image_Path', 'BakeWrangler_Output_Vertex_Cols']:
        ...
        if node.bl_idname == 'BakeWrangler_Output_Vertex_Cols':
            ...
            if len(frames) <= 1:
                ...
                pack_data()
                frame_range = frames
                if debug: _print("> 帧范围: %s" % (frame_range), tag=True)
                for frame in frames:
                    frames_itr += 1
...
def process_vcol_solution(solution):
    ...
    for key in solution.bakepass.keys():
        if not solution.bakepass[key].vcol_result:
            _print(">  通道: [%s] " % (solution.bakepass[key].get_name()), tag=True, wrap=False)
            err += process_bake_pass(solution, key, solution.split)
    _print(">", tag=True)
    
    _print(">    -导出顶点颜色:", tag=True, wrap=False)
...
def process_output(solution, split="", udim=None):
    ...
    _print(">  图像: [%s] " % (output_name), tag=True)
    post_alpha = None
    ...
    post_combined = None
    if post_alpha:
        _print(">   -创建alpha通道", tag=True, wrap=False)
    ...
    _print(">   -将更改写入 %s" % (output_file), tag=True)
    _print(">", tag=True)
...
def process_bake_pass(solution, key, split_list=None, exclude=True, imgs=None):
    ...
    def pbp_img_gen(imgs=None, node=None, bake_settings=None):
        if imgs is None or (len(imgs) and imgs[0] is None):
            if imgs is None: _print(" [栅格节点 (%i)]" % (len(bake_meshes)), tag=True)
    ...
    if not vcol:
        ...
        _print(" [栅格节点 (%i)]" % (len(bake_meshes)), tag=True)
    ...
    for mesh_dat in bake_meshes:
        ...
        if matbk:
            ...
            _print(">   公告牌: [%s]" % (mesh_settings['mesh_label']), tag=True)
        else:
        ...
        if mesh_settings['margin'] > 0 or mesh_settings['margin_auto']:
            ...
            if debug: _print(">   边缘 %s" % (mesh_settings["margin"]), tag=True)
        ...
        for active in active_meshes:
            ...
            if mesh_settings["cage"]:
                if len(target.data.polygons) != len(mesh_settings["cage_object"].data.polygons):
                    _print(">    !帧对无效 [%s]" % (active[0].name), tag=True)
                    mesh_settings["cage"] = False
                    mesh_settings["cage_obj_name"] = ""
            ...
            if udim:
                ...
                # Perform bake type needed
                if multi:
                    if vcol: _print(">    多级精度无法输出到顶点颜色,跳过过程", tag=True)
...
def bake(bake_cat, bake_type, bake_settings, mesh_settings, to_active, multi):
    ...
    try:
        ...
        _print("完成时间 %s" % (str(datetime.now() - start)), tag=True)
    return err
...
def bake_post_vcols(post_obj, post_mat, solution):
    ...
    # Generate output
    bpy.context.view_layer.update()
    _print(">   -执行烘焙后处理", tag=True, wrap=False)
    err = False
    try:
        bpy.ops.object.bake(
            type="EMIT",
...
def paint_margin(image, mask, margin, fill, step=12, samples=3):
    ...
    start = datetime.now()
    _print(">   -涂装余量: ", tag=True, wrap=False)
...
def prep_material_rec(node, socket, link_socket, bake_type, bake_settings, link_list, parent=None):
    ...
    if node.type == 'GROUP' and socket:
        ...
        if bake_settings['bbbk']:
            ...
            node.node_tree.interface.new_socket(name="BW_BBOut", in_out='OUTPUT', socket_type='NodeSocketColor')
            links.new(bake_settings['bbbk_sock'], node.outputs[-1])
        #node.node_tree.outputs.new('NodeSocketColor', 'BW_Out')
        node.node_tree.interface.new_socket(name="BW_Out", in_out='OUTPUT', socket_type='NodeSocketColor')
        ...
        if not gout or not gout.inputs[gsoc].is_linked:
            if debug: _print(">       错误：未关联组节点活动输出插槽", tag=True)
            return True
    ...
    if node.type == 'GROUP_INPUT' and socket and parent:
        ...
        if bake_settings['bbbk']:
            ...
            gout.node_tree.interface.new_socket(name="BW_BBIn", in_out='INPUT', socket_type='NodeSocketColor')
            links.new(bake_settings['bbbk_sock'], node.outputs[-2])
        #gout.node_tree.inputs.new('NodeSocketColor', 'BW_In')
        gout.node_tree.interface.new_socket(name="BW_In", in_out='INPUT', socket_type='NodeSocketColor')
        ...
        # Quick sanity check
        if not gout or not gout.inputs[gsoc].is_linked:
            if debug: _print(">       错误：未关联组节点输入插槽", tag=True)
            return True
    ...
    if node.type == 'BSDF_PRINCIPLED':
        ...
        if debug: _print(">       已到达分支端, ", tag=True, wrap=False)
        ...
        if bake_input:
            ...
            if bake_type in ['OSL_BENTNORM']:
                ...
                if bake_input.is_linked:
                    ...
                    if debug: _print("未关联法向,而是对象法向", tag=True)
            ...
            if bake_input.is_linked:
                ...
                if debug: _print("未关联,值将被复制", tag=True)
    ...
    if debug: _print(">       错误：已达到不支持的节点类型", tag=True)
    return False
...
def make_materials_unique_to_scene(scene, suffix, bake_settings):
    ...
    if bake_cat == 'WRANG':
        ...
        if len(mat_key) > 1 and bake_type != 'MATID': _print(">   Sanity错误：正在进行着色器烘焙,但对象具有多个材质", tag=True)
        for key in mat_key:
...
def free_data():
    has_refs = []
    if debug: _print("> 清除为上一次烘焙创建的数据)", tag=True)
    # Scenes first
