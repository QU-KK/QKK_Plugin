...
from . items import create_decaltype_items, create_infotype_items, create_infotext_align_items, create_trimtype_items
from . items import decaltype_items, texturetype_items, pack_images_items, trimtexturetype_items, atlastexturetype_items
from . items import create_atlas_type_items, create_atlas_creation_type_items, create_atlas_file_format_items, create_atlas_mode_items, create_atlas_prepack_items, create_atlas_trim_sort_items, exporttexturetype_items, create_atlas_size_mode_items, export_atlas_model_format_items, override_preset_items, override_preset_mapping, coat_items
...
class DecalSceneProperties(bpy.types.PropertyGroup):
    ...
    sidebar_show_creation: BoolProperty(name="Show Creation Panel", default=False)
    sidebar_show_export: BoolProperty(name="Show Export Panel", default=False)
    sidebar_show_update: BoolProperty(name="Show Update Panel", default=False)
    sidebar_show_resources: BoolProperty(name="Show Resources Panel", default=True)
    sidebar_show_help: BoolProperty(name="Show Help Panel", default=True)
    ...
    def update_collections(self, context):
        ...
        decals = [obj for obj in context.scene.objects if obj.DM.isdecal and obj.name in context.view_layer.objects]

        for obj in decals:
            sort_into_collections(context, obj, purge=False)

        purge_decal_collections(debug=False)
    ...
    export_bake_ray_distance: FloatProperty(name="Ray Distance", description="Max Ray Distance", default=0.01, min=0.0001, max=1, precision=4)
    ...
    export_bake_preview: BoolProperty(name="Preview Bakes", description="Preview Bakes directly on the Object\nThis creates a temporary Material and hides the Decals", default=True)
    export_bake_open_folder: BoolProperty(name="Open Bake Folder", description="After baking, open the location of the textures in the file browser", default=False)
