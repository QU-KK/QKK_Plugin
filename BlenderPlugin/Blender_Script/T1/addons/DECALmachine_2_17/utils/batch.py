...
from . collection import get_scene_collections
    ...
    for type in types:
        ...
        if state:
            if not type.name.startswith("."):
                if debug:
                    print(f"Hiding {type.name}.")
                type.name = f".{type.name}"

        else:
            if type.name.startswith("."):
                if debug:
                    print(f"Unhiding {type.name}.")
...
def toggle_decaltype_collection_visibility(state, debug=False):
    ...
    cols = [col for col in scene_collections if col.DM.isdecaltypecol]
...
def toggle_decalparent_collection_visibility(state, debug=False):
    ...
    cols = [col for col in scene_collections if col.DM.isdecalparentcol]
