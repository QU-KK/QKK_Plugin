...
def _get_boolean_solver(boolean, solver):
    if not solver:
        return solver

    try:
        identifiers = {item.identifier for item in boolean.bl_rna.properties['solver'].enum_items}
    except Exception:
        return solver

    if solver in identifiers:
        return solver

    if solver == 'FAST' and 'FLOAT' in identifiers:
        return 'FLOAT'

    for fallback in ('EXACT', 'MANIFOLD'):
        if fallback in identifiers:
            return fallback
...
def add_boolean(obj, target, method='UNION', solver='FAST'):
    ...
    boolean.solver = _get_boolean_solver(boolean, solver)
