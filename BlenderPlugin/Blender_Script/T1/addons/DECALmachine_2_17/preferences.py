...
from . utils.assets import declutter_assetspath, move_assets, get_ambiguous_libs, get_invalid_libs, get_corrupt_libs, verify_assetspath
from . utils.registration import get_path, get_name, reload_decal_libraries, get_version_filename_from_blender
...
class DECALmachinePreferences(bpy.types.AddonPreferences):
    ...
    def draw_assets_path(self, layout):
        ...
        assets_dict = DM.get_assets_dict(force=False)
    ...
    def draw_machin3(self, layout):
        ...
        row.scale_y = 1.2
        row.label(text="MESHmachine", icon_value=installed if DM.get_addon('MESHmachine') else missing)
        ...
        row = column.split(factor=0.3, align=True)
        row.scale_y = 1.2
        row.label(text="MACHIN3tools", icon_value=installed if DM.get_addon('MACHIN3tools') else missing)
        r = row.split(factor=0.2, align=True)
        ...
        row = column.split(factor=0.3, align=True)
        row.scale_y = 1.2
        row.label(text="PUNCHit", icon_value=installed if DM.get_addon('PUNCHit') else missing)
        r = row.split(factor=0.2, align=True)
        ...
        row = column.split(factor=0.3, align=True)
        row.scale_y = 1.2
        row.label(text="CURVEmachine", icon_value=installed if DM.get_addon('CURVEmachine') else missing)
        r = row.split(factor=0.2, align=True)
        ...
        row = column.split(factor=0.3, align=True)
        row.scale_y = 1.2
        row.label(text="HyperCursor", icon_value=installed if DM.get_addon('HyperCursor') else missing)
