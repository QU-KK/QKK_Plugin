...
def register_classes(classlists, debug=False):
    # Blender 5.1 / Python 3.13 compatibility fix:
    # avoid exec()-imported class names, because names such as DecalLibsUIList
    # may not be visible to later exec() calls under newer Python.
    import importlib
    ...
    for classlist in classlists:
        for fr, imps in classlist:
            module = importlib.import_module("..%s" % fr, package=__package__)
            for name, _ in imps:
...
def register_decals(library="ALL", default=None, reloading=False):
    ...
    decallibpaths = DM.get_assets_dict()['DECALS']
    ...
    for libname in decallibs:
        ...
        DM.decal_libraries.append((libname, 'decal', col))
    ...
    return DM.decal_libraries

def unregister_decals(library="ALL"):
    from .. import DECALmachineManager as DM

    if library == "ALL":
        decallibs = DM.decal_libraries

    else:
        decallibs = [(library, 'decal', [col for libname, libtype, col in DM.decal_libraries if libname == library][0])]
...
def register_trims(library="ALL", default=None, reloading=False):
    ...
    trimlibpaths = DM.get_assets_dict(force=reloading)['TRIMS']
    ...
    for libname in trimlibs:
        ...
        DM.trim_libraries.append((libname, 'trim sheet', col))
    ...
    return DM.trim_libraries

def unregister_trims(library="ALL"):
    from .. import DECALmachineManager as DM

    if library == "ALL":
        trimlibs = DM.trim_libraries

    else:
        trimlibs = [(library, 'trim sheet', [col for libname, libtype, col in DM.trim_libraries if libname == library][0])]
...
def register_atlases(reloading=False):
    ...
    assets_dict = DM.get_assets_dict(force=reloading)
...
def register_instant_decals(default=None, reloading=False):
    ...
    for decalname, decalpath in folders:
        ...
        if all([f in files for f in ["decal.blend", "decal.png", "uuid"]]):
            ...
            iconpath = os.path.join(decalpath, "decal.png")
            preview = DM.instantdecals.load(decalname, iconpath, 'IMAGE')
...
def unregister_instant_decals():
    ...
    previews.remove(DM.instantdecals)
...
def register_infotextures(default=None):
    ...
    for img in sorted(images):
        ...
        preview = DM.infotextures.load(imgname, imgpath, 'IMAGE')
...
def unregister_infotextures():
    ...
    previews.remove(DM.infotextures)
...
def register_infofonts(default=None):
    ...
    for font in sorted(fontfiles):
        ...
        preview = DM.infofonts.load(fontname, fontpath, 'FONT')
...
def register_trimtextures():
    ...
    for img in sorted(images):
        ...
        preview = DM.trimtextures.load(imgname, imgpath, 'IMAGE')
...
def unregister_trimtextures():
    ...
    previews.remove(DM.trimtextures)
