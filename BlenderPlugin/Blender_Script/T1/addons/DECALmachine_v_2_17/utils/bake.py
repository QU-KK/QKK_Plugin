...
def setup_baking(bakescene, baketype, margin=3, to_active=True, extrusion_distance=0, ray_distance=0):
    if baketype == 'NORMAL':
    ...
    bakescene.render.bake.max_ray_distance = ray_distance
    bakescene.render.bake.cage_extrusion = extrusion_distance

def bake_target_mask(bakescene, bakebasepath, targetname, bakename, bakeimg, bakemat, margin=3, extrusion_distance=0, ray_distance=0):
    setup_baking(bakescene, 'EMIT', margin=margin, to_active=False, extrusion_distance=extrusion_distance, ray_distance=ray_distance)
    bakepath = set_bakepath(bakeimg, bakebasepath, targetname, bakename)

    print(f"Info: Baking {targetname}'s {bakename}: {bakepath}")
...
def bake_decals_margin_mask(bakescene, bakebasepath, targetname, bakename, bakeimg, decals, margin=3, extrusion_distance=0, ray_distance=0):
    setup_baking(bakescene, 'EMIT', margin=margin, extrusion_distance=extrusion_distance, ray_distance=ray_distance)
    bakepath = set_bakepath(bakeimg, bakebasepath, f"{targetname}_{bakename}_margin_{margin}", 'mask')

    print(f"Info: Baking {targetname}'s {bakename} decals mask, with margin {margin}: {bakepath}")
...
def bake(bakescene, bakebasepath, decals, targetname, baketype, bakeimg, margin=3, extrusion_distance=0, ray_distance=0):
    if baketype in ['NORMAL']:
        setup_baking(bakescene, baketype, margin=margin, extrusion_distance=extrusion_distance, ray_distance=ray_distance)
    ...
    print(f"Info: Baking {targetname}'s {baketype.lower()} map: {path}")
...
def resample(bakes, factor):
    for target, baketypedict in bakes.items():
        for baketype, path in baketypedict.items():
            if baketype == 'MASKS':
                for p in path:
                    print(f"Info: Resampling bake: {p}")
                    scale_image(p, scale=factor)

            else:
                print(f"Info: Resampling bake: {path}")
