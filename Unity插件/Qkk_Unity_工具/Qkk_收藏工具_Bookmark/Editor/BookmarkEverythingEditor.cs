...
namespace BookmarkEverything
{
	public class BookmarkEverythingEditor : EditorWindow
    {
        ...
        private const string CATEGORY_SCENE = "收藏2";
        private const string CATEGORY_PREFAB = "收藏3";
        private const string CATEGORY_SCRIPT = "收藏4";
        private const string CATEGORY_SO = "收藏5";
        private const string CATEGORY_STARRED = "收藏1";
        ...
        [MenuItem("Qkk工具/收藏工具_Bookmark %h")]
        ...
        private void OnEnable() {
            titleContent =RetrieveGUIContent("资产收藏", "CustomSorting");
        ...
        {
            _headerContents.Add(RetrieveGUIContent("收藏夹", "UnityEditor.SceneHierarchyWindow"));
            _headerContents.Add(RetrieveGUIContent("设置", "SettingsIcon"));
        }
                ...
                case EventType.DragPerform:
                    ...
                    if (evt.type == EventType.DragPerform)
                    {
                        ...
                        foreach (UnityEngine.Object draggedObject in DragAndDrop.objectReferences)
                        {
                            ...
                            {
                                EditorUtility.DisplayDialog("收藏工具_Bookmark", "目前不支持层级对象。您是否希望我添加这个功能？ 可以联系我邮箱 dogukanerkut@gmail.com.", "好的");
                                return;
                            }
                        ...
                        if(duplicateList.Count > 0)
                        {
                            ...
                            if (EditorUtility.DisplayDialog("收藏工具_Bookmark", string.Format("发现重复条目：{0} 是否仍要添加？（非重复条目将被添加）)", sb.ToString()), "是", "否"))
                            {
        ...
        private bool DrawButton(string name, string iconName, ButtonTypes type)
        {
            ...
            switch (type)
            {
                ...
                case ButtonTypes.Big:
                    ...
                    options = new GUILayoutOption[]{ GUILayout.MaxHeight(EditorGUIUtility.singleLineHeight + (EditorGUIUtility.singleLineHeight * .05f)), GUILayout.MaxWidth(25) };
                    break;
...
#region MAIN HEADER DRAWERS
        ...
        private void DrawHeader()
        {
            ...
            {
                    bool save = EditorUtility.DisplayDialog("有未保存的收藏", "是否保存?", "保存", "不保存");
                    if (save)
                    {
                        SaveNewChanges();
                    }
        ...
        private void DrawProjectFinderEntries(string category)
        {
            ...
            for (int i = 0; i < _currentSettings.EntryData.Count; i++)
            {
                if (_currentSettings.EntryData[i].Category == category)
                {
                   ...
                   EditorGUILayout.BeginHorizontal();
                    if (GUILayout.Button(content, _buttonStyle, GUILayout.MaxHeight(EditorGUIUtility.singleLineHeight + (EditorGUIUtility.singleLineHeight * .05f))))
                    {
                        if (exists)
                        {
            ...
            {
                EditorGUILayout.LabelField("将文件拖进此处。", _boldLabelStyle);
            }
        ...
        private void DrawSettings()
        {
            ...
            EditorGUILayout.LabelField("管理收藏列表", _boldLabelStyle);
            ...
            {
                EditorGUILayout.LabelField(" ", _boldLabelStyle);
            }
            EditorGUILayout.EndVertical();
            EditorGUILayout.EndScrollView();
            ...
            if (_changesMade)
            {
                ...
                EditorGUILayout.HelpBox("发生更改，是否保存。", MessageType.Info);
        ...
        {
                EditorGUILayout.BeginVertical(_boxStyle);
            EditorGUILayout.LabelField("常规设置", _boldLabelStyle);
            EditorGUILayout.LabelField("", GUI.skin.horizontalSlider);
            EditorGUILayout.BeginHorizontal();
                string label = "定位模式（Both） :";
                EditorGUILayout.LabelField(label, GUILayout.MaxWidth(label.Length * 11.5f));
                if (_pingType == PingTypes.Ping)
                {
                ...
                _autoClose = EditorGUILayout.Toggle("自动关闭（不开启） : ", _autoClose);
